...
**Entry (Testing-harness convergence layout, retired)** · `entry:assayer:defer-harness-convergence-layout`

**RETIRED, decided.** The owning record resolves the layout at (`dec:harness:convergence-split`). The convergence subject splits by topic before the concentrated witnesses land, so no deferred choice remains here.
