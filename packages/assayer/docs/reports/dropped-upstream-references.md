...
| ``§ALGO S-8.3`` | The producing set, as the active analysis surface | Restore; closed by (`dec:ownership:producing-set-correspondence`) |
...
| ``§ALGO S-8`` | The head of the abstraction boundary's operation range | Defer; closed by (`dec:ownership:boundary-inventory`) |
...
The restore recommendation is now closed by the producing-set correspondence decision (`dec:ownership:producing-set-correspondence`).
...
The defer recommendation is now closed by the boundary-inventory decision (`dec:ownership:boundary-inventory`).
