...
1. **Read the stored operands:** $v = \Sigma \hat{\phi}$ and $h = \hat{\phi}^\top v$. For the elapsed-time factor $f_t = \gamma_t^{\Delta t}$, compute the policy leverage $h_\text{policy} = h/f_t$ from covariance as of the steward processing instant ($\Sigma/f_t$), using the same lazy time correction as assessment (`alg:temporal:lazy-application`).
2. **Cap the weight:** $w_\text{eff} = \min(w_\text{target},\, w_\text{ceiling},\, c/(h_\text{policy} + \varepsilon))$. Keep the stored $v$ and $h$ for the matrix and mean updates below; their single forgetting factor is $\gamma = \gamma_\text{label} f_t$.
...
The policy reads the current-time covariance while the matrix arithmetic retains its stored operands. Restore has already aged those operands for downtime, so its subsequent $f_t$ covers only elapsed time since construction; an uninterrupted model includes that interval in its next factor. Both policies therefore consult the same aged covariance (`dec:durability:decay-once`). The temporal factor must be finite and in $(0,1]$, and the corrected leverage must be finite and non-negative; otherwise the label follows the existing numeric refusal path before mutation. A factor of one preserves the policy reading bit for bit.
...
| The producing set | No counterpart | Competitive targets plus online ancestors: the online subsets of the investment set under the slot-vacancy invariant (`[SENTINEL-sec:sentinel:algorithm-producing-sets]`) | The reported cell set — the Core observes the reported set and never the contour, and the producing set is a strict superset of the contour cells selected for analysis |
