...
- **The abstraction-boundary enumeration.** The old interface map listed the Sentinel-side node state, depth and promotion events, investment-set and warm-up state, depth gates, contour and plateau organisation, and benchmark compounding that the Core cannot reach. The rewritten terminology convention kept the reading rule and lost the inventory, and this register did not record the deletion. **Restored into the ownership record** (`dec:ownership:boundary-inventory`).
