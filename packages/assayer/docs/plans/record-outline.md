...
Nineteen records against the current thirty-seven, all nineteen scoped here: eighteen are derived from the layers, and ``harness`` is the testing-only exception admitted by (´dec:assayer:record-harness-record´). "Serves" names the layer choices for the derived records and the governing testing policy and source concept for the exception. "Live" is the count of layer-projected live decisions from the audit's disposition tables (´sec:assayer:adr-disposition´), except for ``harness``, where it counts the shared test-contract decisions.

| Area | Scope in one line | Serves | Live | Deferrals |
| --- | --- | --- | --- | --- |
| ``ownership`` | What the package owns outright, what it merely observes, why the reported cell set has its shape, what stays on the Sentinel's side, and the lint that keeps the difference structural rather than conventional. | (`dec:structural:boundary-direction`), (`dec:structural:boundary-enforcement`), (`dec:structural:coordinate-width`) | 4 | — |
...
| ``harness`` | The shared testing contract: one scenario, declared barriers and playback, bounded probes, independent oracles, guarded fixtures, explicit side-harness limits, and performance evidence on that scenario. | (´dec:assayer:test-documentation-policy´), (´plan:assayer:testing-harness-concept´) | 12 | — |
...
| ``challenge`` | The conjugate challenge estimate, its host ownership, its final public arrangement, and the fact that it shipped before anything consumed it. | (`dec:numerics:challenge-model`), (`dec:contracts:companion-boundary`) | 5 | — |
...
The audit's five disposition tables carry 158 distinct layer-projected decisions, of which 142 are LIVE (`data:assayer:adr-disposition-totals`). Every one has exactly one home and no decision has two. By derived record: ``ownership`` 4, ``concurrency`` 7, ``durability`` 4, ``degradation`` 3, ``substrate`` 9, ``retention`` 10, ``memory`` 10, ``ordering`` 12, ``clock`` 3, ``vector`` 10, ``posterior`` 14, ``calibration`` 8, ``surface`` 17, ``construction`` 10, ``health`` 8, ``metrics`` 3, ``challenge`` 5, ``derivation`` 5. Their sum remains 142; the testing-only ``harness`` record adds 12 test-contract decisions, so the combined record-set total is 142 + 12 = 154.

Read by layer, the sums reconcile against the audit's tables: structural 21 live of 22 rows, representation 26 of 26, operational cycle 29 of 32, numerics 29 of 31, contracts 37 of 47 — totalling 142 live, 12 ABSORB, 1 SUPERSEDED, 3 DEAD against 158 rows. The 12 ABSORB rows are carried to the specification as candidates and are homed by no record here; the SUPERSEDED early-refit condition survives only as the history the unconditional trigger replaced (`dec:operational:convergence-diagnostic`); the 3 DEAD Companion rows remain the audit's historical disposition, while ``challenge`` closes their shared arrangement by retaining the two surfaces that since shipped and rejecting the mapper (`dec:challenge:arrangement-final`) rather than homing those rows.
...
The cost is honest and is recorded rather than hidden: three decisions and nine environments put ``clock`` at the floor of the band. Folding it into ``durability`` is nevertheless rejected at (`reg:assayer:record-outline-open`): the apparent economy would recreate split ownership for a discipline already cited across the record set and would strand its harness consequence outside durability.

**Decision (The testing-only harness record earns its place)** · `dec:assayer:record-harness-record`

The layer projection remains the rule (´dec:assayer:projection-principle´), and the coherent-home requirement remains the test for the exception (´req:assayer:record-derivation-rule´). The audit's projection shows that the harness changes no specification behaviour and that its behaviour-specific consequences belong in the records that govern those behaviours (´sec:assayer:testing-harness-record-audit-projection´); its collision check also leaves ``harness`` as a short, singular, unclaimed area (´tab:assayer:testing-harness-record-audit-area-collision´).

The shared testing contract is one discipline. Folding it into ``construction`` would make construction govern playback, oracle provenance, fixture validity, probes, and wait vocabulary, while splitting it among behaviour records would copy the common rule and leave each fragment under an owner that does not govern tests. Like the clock discipline (´dec:assayer:record-clock-record´), it therefore receives one owner of its own, and its body remains inside the derivation band.

The strongest alternative is to keep the test-only choices in the concept, add only their behaviour-specific consequences to existing records, and let the test documentation policy govern the resulting evidence. That alternative is rejected: the policy governs how test claims are documented (´dec:assayer:test-documentation-policy´), and the concept argues for a design (´plan:assayer:testing-harness-concept´), but neither gives the shared scenario, wait prohibition, probe boundary, oracle provenance, and fixture obligation a durable decision home.
...
**Caveat (The Companion cluster was scoped before its arrangement settled)** · `cav:assayer:record-challenge-scope`

The Companion cluster remains scoped here as ``challenge``. Its arrangement is now final: the published replacement trait and clocked health report are the whole surface, the challenge metric names remain host-emitted under the Assayer prefix, and no mapper is added (`dec:challenge:arrangement-final`). The record remains scoped to host ownership, infallible estimation, the conjugate model and that arrangement, and still owes its reader the fact it must not smooth over: the mathematics shipped before anything consumed it (`obs:assayer:adr-challenge-unwired`).
...
Thirteen environments, four layer-projected live decisions. The record carries six decisions in all: the producing-set correspondence and the Sentinel-side inventory answer to the Sentinel corpus rather than to a layer choice, so the register tracks them as mints while the coverage arithmetic counts only layer-projected homes (`data:assayer:record-coverage`) (`reg:assayer:decision-record-register`). The record fixes custody: what the package holds outright, what it only ever sees as a value already given to it, why the producing set shapes what it sees, what remains on the Sentinel's side, and the mechanical check that makes the difference a fact rather than a convention. It is the smallest structural record and the one most other records lean on, because the feed-forward direction is a premise everywhere downstream.
...
| ``dec`` | ``dec:ownership:producing-set-correspondence`` | the Sentinel's producing set explains the shape of the reported cell set without exposing the Sentinel's internal reasoning |
| ``dec`` | ``dec:ownership:boundary-inventory`` | the Sentinel-side inventory lives in the ownership record while the terminology convention carries only its upstream pointers |
| ``tab`` | ``tab:ownership:sentinel-side-inventory`` | the Sentinel-side node, depth, lifecycle, selection, contour, plateau and compounding material, followed by its owning upstream ranges |
...
**Entry (Record: harness)** · `entry:assayer:record-harness`

Sixteen environments, twelve test-contract decisions, four caveats. The record fixes the shared testing contract, assigns the concentrated convergence subject to topic binaries before its planned witnesses land, and places maintained performance evidence on the same scenario.

The testing-only entry is placed after ``clock`` and before ``vector`` because the operational section is the closest home for a contract governing time, queues, playback, state observation, and measurement, while the ruling above keeps its source explicitly outside the layer derivation. Its original projected inventory was fixed by (´tab:assayer:testing-harness-record-audit-environments´); the convergence split replaces one caveat with one decision and leaves the environment total unchanged (´dec:harness:convergence-split´), while the performance decision adds one test-contract environment (´dec:harness:performance-benchmarks´).

Record identity. Kind ``rec``; label ``rec:harness:shared-testing-contract``; scope: names the standing contract shared by Assayer's test caller surfaces.

| Kind | Label | Scope |
| --- | --- | --- |
| ``dec`` | ``dec:harness:single-scenario`` | one Assayer-owning scenario type carries the common instance, names, time, and barriers |
| ``dec`` | ``dec:harness:no-ad-hoc-waits`` | tests call a declared queue barrier and never author a sleep, poll, or wait |
| ``dec`` | ``dec:harness:declarative-playback`` | long stimuli are typed rows interpreted by one runner with declared barrier and checkpoint policy |
| ``dec`` | ``dec:harness:probe-contract`` | state readers are gated, single-load, owned-copy, read-only projections bounded by retention |
| ``dec`` | ``dec:harness:oracle-tier`` | a small shared tier recomputes published quantities by declared independent routes |
| ``dec`` | ``dec:harness:guarded-fixtures`` | a state-establishing fixture returns only after it has checked and reported its precondition |
| ``dec`` | ``dec:harness:separate-validation`` | fixture guards and result oracles remain separate because their failures mean different things |
| ``dec`` | ``dec:harness:real-engine`` | shared scenarios exercise a real Assayer instance rather than an engine mock or double |
| ``dec`` | ``dec:harness:specialised-side-harnesses`` | maintenance and multi-channel specialisation remain local while shared engine logic and barriers do not |
| ``dec`` | ``dec:harness:seeded-sweeps`` | invariant packs use declared reproducible seeds without adding a property-testing framework |
| ``dec`` | ``dec:harness:convergence-split`` | the convergence subject splits into topic binaries before its concentrated witnesses land |
| ``dec`` | ``dec:harness:performance-benchmarks`` | performance evidence is Criterion benchmarks that consume the shared scenario, with historical budgets documented rather than asserted in the ordinary suite |
| ``cav`` | ``cav:harness:limited-unblocking`` | the contract removes duplicated fixture work but does not resolve production or specification blockers |
| ``cav`` | ``cav:harness:progress-not-latency`` | playback progress diagnoses liveness and establishes no elapsed-time promise |
| ``cav`` | ``cav:harness:oracle-trust`` | route independence and fails-before evidence mitigate but cannot prove an oracle correct |
| ``cav`` | ``cav:harness:incremental-adoption`` | the single-scenario claim is descriptive now that the last parallel model and wait vocabulary have retired, and one new caller can make it false again |

Citation spine. Cites the specification for the assessment and label interfaces, queue draining, snapshot boundary, replay and precision guarantees, and evidence authority; cites the structural layer for single stewardship, the representation layer for publication and retention, the operational layer for assessment, learning, and diagnostic semantics, the numerics layer for independent arithmetic, and the contracts layer for the public boundary. It cites the test documentation policy (´dec:assayer:test-documentation-policy´) for authored claims and generated projections, and the concept for the shared choices (´sec:assayer:testing-harness-concept-ideas´), rejected alternatives (´sec:assayer:testing-harness-concept-rejected´), and bounded risks (´sec:assayer:testing-harness-concept-risks´). It is cited by the future extensions ``cor:clock:harness-control``, ``cor:concurrency:harness-barriers``, ``cor:construction:harness-declarations``, ``cor:ordering:tape-batches``, ``cor:durability:harness-fork``, ``cav:retention:probe-boundary``, and ``cor:surface:test-support-opacity``.
...
Twelve environments, five layer-projected live decisions. The record carries a finished model, its final host-facing arrangement, and no connection from the Companion into Core; that absence is part of the arrangement rather than a gap in it.
...
| ``dec`` | ``dec:challenge:arrangement-final`` | the published replacement trait and clocked health report are the whole Companion surface, no mapper is added, and challenge metric emission remains with the host under the Assayer prefix |
...
1. **SETTLED — the clock record stays separate.** Folding its three decisions into ``durability`` would recreate the two-owner defect: a statement with two owners is a statement about to be copied (`obs:assayer:adr-duplicate-definition`). ``durability``, ``posterior`` and ``construction`` already cite the clock record's decisions, so the fold would either copy the clock discipline into each record or make ``durability`` own a timestamp type it never persists. The harness corollary (`cor:clock:harness-control`) governs scenario control of both clock domains and has no home in ``durability``. The separate owner therefore stands on the reasoning at (`dec:assayer:record-clock-record`).
