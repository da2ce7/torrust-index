...
The inferred `EntityConcordanceHealth` value exposes exactly `label_count`, `concordance`, and `flagged`. It exposes no cause, diagnosis, corruption verdict, confidence in a cause, or recommended action. The surrounding `ConcordanceHealth` also carries its window and calibration readings plus per-entity capacity and eviction accounting, but no causal classification.
...
Two focused unit witnesses already keep the arithmetic and storage mechanics: `per_entity_concordance_flags_and_evicts_least_recent` proves flagging at the gate together with least-recently-observed eviction (´test:unit:per-entity-concordance-flags-and-evicts-least-recent´), and `per_entity_concordance_honours_configured_gate_and_deficit` proves the configured gate and strict deficit boundary (´test:unit:per-entity-concordance-honours-configured-gate-and-deficit´). Neither asks what a host can learn about cause from the public type.

The crate witness `full_health_report_all_subtypes_instantiated` reaches the outer concordance block only to require a non-empty debug rendering (´test:crate:full-health-report-all-subtypes-instantiated´); it does not read a per-entity value. The nearest label-path integration witness accepts an inverted host valence convention without an integrity check (´test:integration:inverted-valence-signs-accepted-without-corruption´), but it neither raises nor reads per-entity concordance. No crate or integration witness cites this promise.

The finished harness now supplies the complete `WorldBuilder`, the `World::flush_labels` barrier, and the independent `pairwise_rank` oracle that cover the builder and rank mechanisms originally associated with this plan (´tab:assayer:testing-harness-concept-plan-demands´), (´tab:assayer:harness-implementation-library-roster´), (´dec:harness:oracle-tier´). They can support a numerical concordance witness, but none can prove causal neutrality: a runtime scenario would have to invent two causal stories outside the input and could only rediscover that identical inputs produce identical fields. This witness therefore needs no `World`, probe, playback, guarded fixture, or seeded sweep.

The package declares no UI compile-fail runner or compile-failure dependency, and the finished harness roster contains no compiler-test facility (´tab:assayer:harness-implementation-library-roster´). The integration suite contains no UI fixture directory, and the standing testing plan has no entry covering this promise.

## The witness · `sec:assayer:intent-a-concordance-flag-names-a-discrepancy-without-attributing-a-cause-witness`

The witness belongs in the integration target `concordance_surface`, in `concordance_surface::concordance_health_exposes_discrepancy_without_cause`, with one compile-fail fixture beneath the integration suite. Its test documentation cites the intent, and the module index repeats the witness statement.
...
- Assertion: a diagnostic-checking UI runner requires that fixture to fail for the recorded reason. Resolving the earlier field projections before rejecting `.cause` keeps the accessible half of the surface live.
...
Add `trybuild` to the package's dev-dependencies. In the integration target `concordance_surface`, `concordance_surface::concordance_health_exposes_discrepancy_without_cause` runs one compile-fail source fixture whose checked diagnostic has `.cause` as its only intended type error and names the three accessible discrepancy fields. This is the only missing test machinery; it needs no production change, `World` extension, clock, barrier, or liveness support, and the ordinary test-document update carries the new target and witness into the generated indexes and integration matrix.
...
The implementing lane's report identifies the `concordance_surface` integration target, `concordance_surface::concordance_health_exposes_discrepancy_without_cause`, its source fixture, checked diagnostic, and dev dependency; confirms the driver documentation cites the intent; and shows the generated module index, integration matrix, and coverage report resolving the intent to the compile-time witness.
