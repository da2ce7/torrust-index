...
The first three are the durability record's own promises (`dec:durability:checkpoint-journal`) and (`cor:durability:replay-exactness`) taken at their word; the last two are the ones that need instrumentation, because what they assert is the absence of a state that no public surface exposes. Their reusable checkpoint-and-restore infrastructure belongs to the isolated persistence fork (´entry:assayer:harness-persistence-fork´) under its durability corollary (´cor:durability:harness-fork´).
...
The multi-channel boundary suite still needs a measured runtime budget and tolerance where the suite is defined, so growth in the grid or setup cost becomes a repeatable finding. That verification work now lives in the backlog (´entry:assayer:process-grid-runtime´), where it also provides the revisit condition for shared playback (´entry:assayer:harness-tape-runner´); this plan retains only the historical reason that the product of enrichment and policy axes makes per-case cost load-bearing.
...
**DONE.** The `World` and `WorldBuilder` pair with a deterministic `TestRng`, a name registry, a tolerances bundle, the `World::cold()` convenience, lifecycle, assess, derive and label verbs, the deterministic `World::cycle_on(...)` and `World::cycle_default(...)` round-trip helpers, `World::flush_labels()` for post-label observation, the `LabelSpec` builder, and the domain-aware assertions including `assert_health_clean` and `assert_risk_basis_near`. The test-local support module centralises the channel policy and reward builders and the repeated same-core-moved-decision-layer assertions. These landed pieces are the skeleton of the shared testing contract (´rec:harness:shared-testing-contract´), and their declared construction inputs answer to the harness-construction corollary (´cor:construction:harness-declarations´).
...
What this bought is measured rather than assumed. The staleness class of divergence is gone: no reported-only, load-scaled failure has recurred. The scenario-facing consequence is now fixed by the harness-control corollary (´cor:clock:harness-control´). What the injection did not buy is exactness. With every budget set to zero the suite still fails, and the surviving spread has two shapes — a floating-point residue at the scale of a few units in the last place, and a coarser residue in the reported cells that alternates between two stable values rather than jittering. The second shape is an ordering race, not a clock reading, and it is left to (`entry:assayer:harness-stage-clock-residual-race`). The budgets therefore stand, now justified by what actually moves them.

The persistent-clock stage split after the shared seam landed. Its remaining production reads now live in the backlog (´entry:assayer:harness-stage-clock´) under the separated-domain decision (´dec:clock:two-domains´), while the former `World::advance` and `World::travel_to` clause is superseded by scenario-time work (´entry:assayer:harness-scenario-time´) under the harness-control corollary (´cor:clock:harness-control´). The historical split retires no drift budget; that expectation belonged to the monotonic half above.
...
**HISTORICAL.** This umbrella remains because standing plan citations still reach it, but it owns no OPEN work. Its barrier, projection, playback, oracle, guarded-fixture, and persistence pieces are now independently executable backlog entries (´entry:assayer:harness-closed-barriers´) (´entry:assayer:harness-probe-contract´) (´entry:assayer:harness-tape-runner´) (´entry:assayer:harness-oracle-tier´) (´entry:assayer:harness-guarded-fixtures´) (´entry:assayer:harness-persistence-fork´), all governed by the shared harness record (´rec:harness:shared-testing-contract´).

The randomised invariant packs landed from the backlog (´entry:assayer:harness-property-packs´), where twenty-nine declared-seed packs witness every claim the census found sweep-shaped, each rejection reports its seed, case index and complete drawn input, and no dependency was added — the seeded-sweeps decision executed rather than restated (´dec:harness:seeded-sweeps´). The roadmap retains the historical scope: posterior positive-definiteness after rank-one updates and marginalisation, Kraft equality preservation, routed-assembly sums, pure-decay monotonicity, and crossover matching beyond the existing geometry sweep.
...
| ``exact`` | 0.0 | Exact equality, for bit-identical snapshots | Not a tolerance at all. It is a field so that such a test reads as a named requirement rather than as a bare zero among real epsilons. |
