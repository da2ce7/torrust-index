...
impl CellMutableState {
    ...
    #[must_use]
    pub fn new(now: PersistentTimestamp) -> Self {
        Self {
            measurement: MeasurementState::default(),
            outcome: CellOutcomeState::new_neutral(now),
        }
    }
}
...
impl CellOutcomeState {
    ...
    ///
    /// `now` is the persistent present supplied by the engine clock.
    #[must_use]
    pub fn new_neutral(now: PersistentTimestamp) -> Self {
        Self {
...
mod tests {
    ...
    fn cell_outcome_state_new_neutral() {
        let state = CellOutcomeState::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn cell_outcome_state_decayed_view_is_exact_and_pure() {
        ...
        let mut stored = CellOutcomeState::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn cell_mutable_state_new() {
        let state = CellMutableState::new(PersistentTimestamp::new(1_700_000_000, 0));
