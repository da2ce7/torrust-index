...
use crate::types::{DimensionId, PersistentTimestamp};
...
fn apply_competitive_cell_state_changes(
    ...
    for cell in entries {
        cell_state
            .entry(*cell)
            .or_insert_with(|| Mutex::new(CellMutableState::new(now)));
    }
}
...
/// Learned state recovered for an identity dimension whose host declaration
/// has not yet supplied the process-local encoding function.
#[derive(Clone, Debug)]
pub struct RestoredIdentityState {
    /// G-V graph snapshot recovered from the checkpoint.
    graph_snapshot: IdentityGraphSnapshot,
    /// Competitive cells published at checkpoint time.
    competitive_cells: Vec<CompetitiveCellId>,
    /// Outcome state retained for every tracked cell.
    cell_outcome_state: Vec<(CompetitiveCellId, super::CellOutcomeState)>,
}

impl RestoredIdentityState {
    /// Collects the three durable pieces of a restored identity dimension.
    pub const fn new(
        graph_snapshot: IdentityGraphSnapshot,
        competitive_cells: Vec<CompetitiveCellId>,
        cell_outcome_state: Vec<(CompetitiveCellId, super::CellOutcomeState)>,
...
pub enum MaintenanceCommand {
    ...
    /// Prepare for checkpoint, acknowledging only after observations and deferred signal writes are drained, competitive changes are detected and offered to the model-owner queue, and graph snapshots are published.
    ...
    CreateDimension {
        ...
        /// Checkpoint state to seed instead of an empty dimension.
        restored: Option<RestoredIdentityState>,
...
/// Process-local infrastructure and optional durable seed for one dimension.
...
impl DimensionState {
    ...
    fn new(
        ...
        depth_cutoff: u8,
        spatial_decay_rate: f64,
        runtime: DimensionRuntime,
        now: Instant,
        persistent_now: PersistentTimestamp,
    ) -> Self {
        let DimensionRuntime {
            observation_rx,
            infra,
            restored,
        } = runtime;
        let (graph, current_set) = if let Some(restored) = restored {
            let graph = GvGraph::from_observations(config, restored.graph_snapshot.reconstruction_observations());
            debug_assert!(restored.graph_snapshot.validate_total_sum(graph.total_sum()));
            let current_set: HashSet<_> = restored.competitive_cells.into_iter().collect();
            {
                let mut cell_state = infra.cell_state.write().unwrap_or_else(std::sync::PoisonError::into_inner);
                cell_state.extend(restored.cell_outcome_state.into_iter().map(|(cell, outcome)| {
                    (
                        cell,
                        Mutex::new(CellMutableState {
                            measurement: super::MeasurementState::default(),
                            outcome,
                        }),
                    )
                }));
                for cell in &current_set {
                    cell_state
                        .entry(*cell)
                        .or_insert_with(|| Mutex::new(CellMutableState::new(persistent_now)));
                }
            }
            infra.graph_snapshot.store(Arc::new(Some(restored.graph_snapshot)));
            (graph, current_set)
        } else {
            (GvGraph::new(config), HashSet::new())
        };
        ...
        if state.graph.total_sum() > 0 {
            state.publish_competitive_index();
        }
        state
    }
    ...
    fn detect_and_emit_changes(&mut self) {
        for dim in &mut self.dimensions {
            ...
            if set_changed {
                ...
                if let Ok(mut cell_state) = dim.infra.cell_state.write() {
                    apply_competitive_cell_state_changes(&mut cell_state, &exits, &entries, self.clock.now());
                }
...
mod tests {
    ...
    fn competitive_exit_resets_measurement_and_reentry_retains_outcome() {
        ...
        let mut state = CellMutableState::new(PersistentTimestamp::new(1_700_000_000, 0));
        ...
        apply_competitive_cell_state_changes(&mut states, &[cell], &[], PersistentTimestamp::new(1_700_000_000, 0));
        {
    ...
    fn graph_interval_cleanup_removes_only_evicted_state() {
        ...
        let mut states = HashMap::from([
            (
                root,
                Mutex::new(CellMutableState::new(PersistentTimestamp::new(1_700_000_000, 0))),
            ),
            (
                evicted,
                Mutex::new(CellMutableState::new(PersistentTimestamp::new(1_700_000_000, 0))),
