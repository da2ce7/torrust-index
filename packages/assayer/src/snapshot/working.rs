...
pub struct WorkingCopy {
    ...
    /// Engine-clock monotonic reading at construction or the last processed label.
    ///
    /// The caller supplies the initial reading so label decay stays in the engine's clock frame (´dec:clock:two-domains´). With `SystemClock`, this and the former constructor-local read are both the process monotonic clock, read microseconds apart; an injected clock can instead be arbitrarily advanced. Reading the process clock here would charge that offset to the first label, repeating decay already applied at restore (´dec:durability:decay-once´).
...
impl WorkingCopy {
    ...
    ///
    /// `last_label_time` is the caller's reading from the engine's monotonic clock.
    ///
    ...
    #[must_use]
    pub fn cold_start(p: usize, config: &ModelConfig, n_recompute: u32, last_label_time: Instant) -> Self {
    ...
    ///
    /// `last_label_time` is the caller's current engine-clock monotonic reading; stored state never supplies it.
    ///
    /// # Errors
    ///
