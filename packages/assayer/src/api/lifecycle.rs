...
impl Assayer {
    ...
    ///
    /// If a restored working copy already holds `reg.id`, this call rebinds
    /// that existing slot and sends no model extension. A second registration
    /// of the live ID remains a duplicate.
    ///
    ...
    pub fn register_sentinel(&self, reg: SentinelRegistration) -> Result<(), LifecycleError> {
        ...
        match self.sentinel_slots.entry(reg.id) {
            Entry::Occupied(_) => {
                let mut restored = self
                    .restored_sentinels_awaiting_rebind
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner);
                let rebound = restored.remove(&reg.id);
                drop(restored);
                if rebound {
                    debug!(sentinel_id = ?reg.id, "restored Sentinel rebound without model extension");
                    return Ok(());
                }
        ...
        self.outcome_ledger.create_sentinel(reg.id, self.clock.now());
    ...
    pub fn deregister_sentinel(&self, id: SentinelId) -> Result<(), LifecycleError> {
        ...
        if let Err(e) = self.send_lifecycle(LifecycleEvent::DeregisterSentinel(id)) {
            ...
            if was_awaiting_rebind {
                self.restored_sentinels_awaiting_rebind
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner)
                    .insert(id);
            }
    ...
    ///
    /// If a restored working copy already holds `reg.id`, this call rebinds
    /// that existing model and sends no model extension. A second registration
    /// of the live ID remains a duplicate.
    ///
    ...
    pub fn register_outcome_axis(&self, reg: OutcomeAxisRegistration) -> Result<(), LifecycleError> {
        ...
        let rebound = if inserted {
            false
        } else {
            let mut restored = self
                .restored_axes_awaiting_rebind
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            let rebound = restored.remove(&reg.id);
            drop(restored);
        ...
        if rebound {
            debug!(axis_id = ?reg.id, "restored outcome axis rebound without model extension");
            return Ok(());
        }
    ...
    pub fn deregister_outcome_axis(&self, id: OutcomeAxisId) -> Result<(), LifecycleError> {
        ...
        if let Err(e) = self.send_lifecycle(LifecycleEvent::DeregisterOutcomeAxis(id)) {
            ...
            if was_awaiting_rebind {
                self.restored_axes_awaiting_rebind
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner)
                    .insert(id);
            }
    ...
    pub fn register_identity_dimension(&self, reg: IdentityDimensionRegistration) -> Result<(), LifecycleError> {
        ...
        // A checkpoint cannot carry the process-local encoding function, so
        // the first matching registration attaches that function while the
        // recovered payload seeds the owner. Removing the payload here also
        // preserves the ordinary duplicate-registration contract afterwards.
        let restored = self
            .pending_restored_identity
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .remove(&reg.id);
        let is_restored_rebind = restored.is_some();

        // Send CreateDimension to maintenance thread
        if let Err(error) = identity_cmd_tx.try_send(MaintenanceCommand::CreateDimension {
            ...
            if let MaintenanceCommand::CreateDimension {
                restored: Some(restored),
                ..
            } = error.into_inner()
            {
                self.pending_restored_identity
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner)
                    .insert(reg.id, restored);
            }
        ...
        // Phase 2: Send lifecycle event to model owner. A restored dimension's
        // feature positions and model state already exist in the working copy;
        // registration only rebinds its process-local owner infrastructure.
        if is_restored_rebind {
            return Ok(());
        }

        // Send the cold-start lifecycle event to model owner.
