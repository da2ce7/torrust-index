...
//! pass its [`ChallengeEstimate`] output to `derive_landscape()` explicitly.
//!
//! # Model
//!
...
impl ChallengeEffectivenessState {
    ...
    #[must_use]
    pub const fn new(alpha_0: f64, beta_0: f64, gamma_qt: f64, now: PersistentTimestamp) -> Self {
        Self {
            alpha: alpha_0,
            beta: beta_0,
            t_last: now,
    ...
    #[must_use]
    pub const fn with_defaults(now: PersistentTimestamp) -> Self {
        Self::new(Self::DEFAULT_ALPHA_0, Self::DEFAULT_BETA_0, Self::DEFAULT_GAMMA_QT, now)
    }
...
/// `derive_landscape()` as an explicit derivation input
...
impl ChallengeEffectivenessTracker {
    ...
    ///
    /// Unknown channels are initialised from the default priors at `now`.
    pub fn update(&mut self, channel_id: ChannelId, result: ChallengeResult, now: &PersistentTimestamp) -> ChallengeEstimate {
        let state = self
            .states
            .entry(channel_id)
            .or_insert_with(|| ChallengeEffectivenessState::with_defaults(*now));
...
mod tests {
    ...
    fn new_initialises_to_prior() {
        let state = ChallengeEffectivenessState::new(3.0, 2.0, 0.999, PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn update_fail_increments_alpha() {
        let mut state = ChallengeEffectivenessState::new(2.0, 2.0, 0.9998, PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn estimate_returns_posterior_mean() {
        let state = ChallengeEffectivenessState::new(4.0, 2.0, 0.9998, PersistentTimestamp::new(1_700_000_000, 0));
    ...
    #[test]
    fn serde_roundtrip() {
        let state = ChallengeEffectivenessState::new(5.5, 3.3, 0.9995, PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn tracker_unknown_channel_returns_default_estimate() {
        ...
        let estimate = tracker.estimate(ChannelId(7), &PersistentTimestamp::new(1_700_000_000, 0));
    ...
    fn time_decay_preserves_mean_increases_variance_reduces_n_eff() {
        ...
        let mut state = ChallengeEffectivenessState::new(1.0, 1.0, 0.9998, PersistentTimestamp::new(1_700_000_000, 0));
        let start = state.t_last;
        for _ in 0..100 {
            state.update(ChallengeResult::Fail, &start);
            state.update(ChallengeResult::Pass, &start);
    ...
    fn tracker_override_replaces_estimate_until_cleared() {
        ...
        let overridden = tracker.set_override(channel_id, 0.9, None, &now);
    ...
    fn tracker_inject_evidence_clamps_to_ceiling() {
        ...
        let channel_id = ChannelId(3);
        let now = PersistentTimestamp::new(1_700_000_000, 0);
        let mut state = ChallengeEffectivenessState::with_defaults(now);
        state.injection_ceiling = 2.0;
    ...
    fn tracker_inject_evidence_rejects_negative_counts() {
        let mut tracker = ChallengeEffectivenessTracker::new();
        let result = tracker.inject_evidence(ChannelId(3), -1.0, 0.0, &PersistentTimestamp::new(1_700_000_000, 0));
