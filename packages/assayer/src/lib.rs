...
pub struct Assayer {
    ...
    checkpoint_control_tx: Option<Sender<persistence::scheduler::CheckpointTrigger>>,
    /// Ledger collection scheduler control channel sender.
    /// Dropping this signals the scheduler to shut down.
    ledger_gc_control_tx: Option<Sender<ledger::LedgerGcTrigger>>,
    ...
    /// Restored Sentinels whose existing slots await the host's first
    /// matching registration call.
    restored_sentinels_awaiting_rebind: Mutex<HashSet<SentinelId>>,
    ...
    /// Restored identity state awaiting the first matching host registration,
    /// which supplies the process-local encoding function.
    pending_restored_identity: Mutex<HashMap<DimensionId, identity::RestoredIdentityState>>,
    ...
    /// Restored axes whose existing models await the host's first matching
    /// registration call.
    restored_axes_awaiting_rebind: Mutex<HashSet<OutcomeAxisId>>,
...
impl Assayer {
    ...
    pub(crate) fn build(config: AssayerConfig) -> Result<Self, BuildError> {
        ...
        #[cfg(feature = "serde")]
        let (working, replay_entries, restored) = Self::build_working_copy(&config, clock.now(), clock.now_monotonic())?;
        #[cfg(not(feature = "serde"))]
        let (working, ()) = Self::build_working_copy(&config, clock.now_monotonic());
    ...
    fn build_inner(
        ...
        // Runtime registries describe the structures the working copy already
        // holds. On a cold start these sets are empty; after restore they let
        // reports arrive immediately and let the first matching registration
        // attach without extending the models a second time
        // (´dec:durability:checkpoint-journal´).
        let restored_sentinels: HashSet<_> = working.dimension_map.sentinel_slots.keys().copied().collect();
        let sentinel_slots = DashMap::new();
        for id in &restored_sentinels {
            sentinel_slots.insert(*id, report::SentinelSlot::new());
        }
        let registered_axes: HashSet<_> = working.outcome_models.keys().copied().collect();
        let restored_axes = registered_axes.clone();
        #[cfg(feature = "serde")]
        let restored_seq = working.last_processed_label_seq;
        let shared = Arc::new(SharedState::new(initial_snapshot));

        // Each restored piece reaches the infrastructure that owns it below.
        // Identity payloads wait for registration because the host's encoding
        // function is process-local and does not cross the checkpoint.
        #[cfg(feature = "serde")]
        let (restored_concordance_state, restored_owner_state, restored_ledger_state, pending_restored_identity) = match restored
        {
            Some(state) => {
                let identity = state
                    .identity
                    ...
                    .map(|(id, payload)| {
                        (
                            id,
                            identity::RestoredIdentityState::new(
                                payload.graph_snapshot,
                                payload.competitive_cells,
                                payload.cell_outcome_state,
                            ),
                        )
                    })
                    .collect();
                (Some(state.concordance), Some(state.owner), Some(state.ledgers), identity)
            }
            None => (None, None, None, HashMap::new()),
        };
        #[cfg(not(feature = "serde"))]
        let pending_restored_identity = HashMap::new();
        ...
        let (checkpoint_handle, checkpoint_control_tx) = if let Some(ref persistence_config) = config.persistence {
            let (ctl_tx, ctl_rx) = crossbeam_channel::bounded::<persistence::scheduler::CheckpointTrigger>(0);
        ...
        let (ledger_gc_control_tx, ledger_gc_control_rx) = crossbeam_channel::bounded::<ledger::LedgerGcTrigger>(0);
        ...
        Ok(Self {
            ...
            sentinel_slots,
            restored_sentinels_awaiting_rebind: Mutex::new(restored_sentinels),
            pending_buffer,
            signal_cache,
            outcome_ledger,
            ingestion,
            identity_dimensions,
            pending_restored_identity: Mutex::new(pending_restored_identity),
            ...
            drift_reset_counter,
            registered_axes: Mutex::new(registered_axes),
            restored_axes_awaiting_rebind: Mutex::new(restored_axes),
            #[cfg(feature = "serde")]
            journal,
            observation_tx,
        })
    ...
    fn build_working_copy(
        ...
        last_label_time: std::time::Instant,
    ) -> Result<
        (
            WorkingCopy,
    ...
    #[cfg(not(feature = "serde"))]
    fn build_working_copy(config: &AssayerConfig, last_label_time: std::time::Instant) -> (WorkingCopy, ()) {
        (
            WorkingCopy::cold_start_from_schema(
                0,
                Vec::new(),
