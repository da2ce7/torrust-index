...
pub fn attempt_restore(
    ...
    // The initial label reading also comes from the engine's clock, so the
    // first label cannot repeat the interval already covered by step 4.
    let concordance_state = payload.concordance_state.clone();
    let mut working = WorkingCopy::from_checkpoint_payload(
        &payload,
        config.standardisation.n_init as usize,
        config.model.lambda_prior,
        last_label_time,
    )
    .inspect_err(|e| {
