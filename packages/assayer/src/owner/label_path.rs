...
//! | [`elapsed_leverage_selects_the_same_binding_weight`] | labelling | Two model arms with the same elapsed interval select the same binding weight even when restore has already applied part of that decay. An off-diagonal entry initially equal to zero receives exactly the selected weight because both feature coordinates are one; no subtraction or inferred tolerance obscures the policy reading. |
//! | [`temporal_leverage_identity_is_bit_exact`] | labelling | A temporal factor of one preserves every matrix and mean bit produced by the original stored-leverage update. |
//! | [`invalid_temporal_leverage_refuses_before_mutation`] | labelling | Invalid temporal factors and an overflowing corrected leverage refuse exactly once before changing the model's precision, covariance or mean. |
...
fn apply_model_label_update(
    ...
    // Invalid temporal inputs use the same refusal tally as an inadmissible
    // leverage; no model mutation precedes either refusal.
    if !time_factor.is_finite() || time_factor <= 0.0 || time_factor > 1.0 {
        refusals.fetch_add(1, Ordering::Relaxed);
        return;
    }
    let (v, h) = model.covariance().quadratic_form_with_product(phi);
    // Policy reads covariance at processing time without mutating its stored
    // operands. Dividing by one is bit-exact; an overflowing reading refuses
    // the label rather than silently admitting a zero-weight update
    // (´dec:posterior:combined-factor´) (´alg:temporal:lazy-application´).
    let policy_h = h / time_factor;
    if !crate::model::update::leverage_admits_update(policy_h) {
        refusals.fetch_add(1, Ordering::Relaxed);
        return;
    }
...
pub fn process_label(
    ...
    *health_counters.labels_by_action.entry(label.label.action_taken).or_insert(0) += 1;
    ...
    let label_update_started = Instant::now();
    ...
    #[cfg(any(test, feature = "test-support"))]
    let label_update_elapsed = label_update_started.elapsed();
    ...
    #[cfg(any(test, feature = "test-support"))]
    shared.label_update_recorder.record(label_update_elapsed);
...
mod tests {
    ...
    fn bounded_parallel_model_updates_match_sequential_results() {
        ...
        apply_model_label_update(&mut sequential_axis, &phi, 1.0, 0.8, 100.0, 5.0, 0.3, 0.999, 1e-6, &refusals);
        ...
        std::thread::scope(|scope| {
            ...
            apply_model_label_update(&mut parallel_axis, &phi, 1.0, 0.8, 100.0, 5.0, 0.3, 0.999, 1e-6, &refusals);
        });
    ...
    /// Two model arms with the same elapsed interval select the same binding weight even when restore has already applied part of that decay. An off-diagonal entry initially equal to zero receives exactly the selected weight because both feature coordinates are one; no subtraction or inferred tolerance obscures the policy reading.
    ///
    /// ´claim:labelling:elapsed-leverage-selects-the-same-binding-weight-across-restore´
    /// ´test:unit:elapsed-leverage-selects-the-same-binding-weight´
    #[test]
    fn elapsed_leverage_selects_the_same_binding_weight() {
        use crate::model::bayesian::BayesianLinearModel;

        let started = std::time::Instant::now();
        let phi = vec_to_col(&[1.0, 1.0]);
        let mut uninterrupted = BayesianLinearModel::new(2, 0.125, 1000);
        let mut restored = BayesianLinearModel::new(2, 0.125, 1000);
        let hour = crate::numerics::decay_factor(0.5, 1.0);
        let two_hours = crate::numerics::decay_factor(0.5, 2.0);
        restored.apply_time_decay(hour);
        let refusals = AtomicU64::new(0);
        let target_weight = 0.2;
        // The prior is 8I, so the initial leverage is 16 and does not bind
        // at this target. At two hours it is 64 and the cap does bind.
        assert!(5.0 / (16.0 + EPSILON_LEVERAGE) > target_weight);
        let expected = 5.0 / (64.0 + EPSILON_LEVERAGE);
        assert!(expected < target_weight);
        apply_model_label_update(
            &mut uninterrupted,
            &phi,
            two_hours,
        ...
        let left = uninterrupted.precision().as_inner()[(0, 1)];
        let right = restored.precision().as_inner()[(0, 1)];
        std::io::Write::write_fmt(
            &mut std::io::stderr().lock(),
            format_args!(
                "binding_weight uninterrupted={left:?} restored={right:?} expected={expected:?} wall={:?}\n",
                started.elapsed()
            ),
        )
        .unwrap();
        // Binary-exact prior and time scalings give identical policy operands.
        // The sole rounded division is therefore identical in both arms.
        assert_eq!(left.to_bits(), expected.to_bits(), "uninterrupted selected weight");
        assert_eq!(right.to_bits(), expected.to_bits(), "restored selected weight");
        assert_eq!(refusals.load(Ordering::Relaxed), 0);
    }

    /// A temporal factor of one preserves every matrix and mean bit produced by the original stored-leverage update.
    ///
    /// ´claim:labelling:temporal-leverage-preserves-the-identity-bit-for-bit´
    /// ´test:unit:temporal-leverage-identity-is-bit-exact´
    #[test]
    fn temporal_leverage_identity_is_bit_exact() {
        use crate::model::bayesian::BayesianLinearModel;

        let started = std::time::Instant::now();
        let phi = vec_to_col(&[1.0, 1.0]);
        let mut legacy = BayesianLinearModel::new(2, 0.125, 1000);
        let mut corrected = BayesianLinearModel::new(2, 0.125, 1000);
        let refusals = AtomicU64::new(0);
        let (v, h) = legacy.covariance().quadratic_form_with_product(&phi);
        let weight = compute_effective_weight(1.0, 100.0, 5.0, h, EPSILON_LEVERAGE);
        let outcome = legacy.apply_leverage_bounded_update(&phi, &v, h, weight, 1.0, 0.5, 0.001);
        assert_eq!(outcome, LabelUpdateOutcome::Applied);
        apply_model_label_update(&mut corrected, &phi, 1.0, 1.0, 100.0, 5.0, 1.0, 0.5, 0.001, &refusals);
        let bits = |values: &[f64]| values.iter().map(|value| value.to_bits()).collect::<Vec<_>>();
        assert_eq!(bits(&corrected.to_parameters().mu), bits(&legacy.to_parameters().mu));
        assert_eq!(
            bits(&corrected.to_parameters().covariance_data),
            bits(&legacy.to_parameters().covariance_data)
        );
        for row in 0..2 {
            for column in 0..2 {
                assert_eq!(
                    corrected.precision().as_inner()[(row, column)].to_bits(),
                    legacy.precision().as_inner()[(row, column)].to_bits()
                );
            }
        }
        assert_eq!(refusals.load(Ordering::Relaxed), 0);
        std::io::Write::write_fmt(
            &mut std::io::stderr().lock(),
            format_args!("temporal_identity bit-exact wall={:?}\n", started.elapsed()),
        )
        .unwrap();
    }

    /// Invalid temporal factors and an overflowing corrected leverage refuse exactly once before changing the model's precision, covariance or mean.
    ///
    /// ´claim:labelling:invalid-temporal-leverage-refuses-before-mutation´
    /// ´test:unit:invalid-temporal-leverage-refuses-before-mutation´
    #[test]
    fn invalid_temporal_leverage_refuses_before_mutation() {
        use crate::model::bayesian::BayesianLinearModel;
        ...
        for factor in [
            ...
            let mut model = BayesianLinearModel::new(2, 0.125, 1000);
            let before = model.to_parameters();
            let rejected = AtomicU64::new(0);
            apply_model_label_update(&mut model, &phi, factor, 1.0, 100.0, 5.0, 1.0, 0.5, 0.001, &rejected);
            assert_eq!(rejected.load(Ordering::Relaxed), 1, "factor={factor:?}");
            let after = model.to_parameters();
            assert_eq!(bits(&after.mu), bits(&before.mu), "factor={factor:?}");
            assert_eq!(
                bits(&after.covariance_data),
                bits(&before.covariance_data),
                "factor={factor:?}"
            );
            for row in 0..2 {
                for column in 0..2 {
                    let expected: f64 = if row == column { 0.125 } else { 0.0 };
                    assert_eq!(model.precision().as_inner()[(row, column)].to_bits(), expected.to_bits());
                }
            }
        }
        std::io::Write::write_fmt(
            &mut std::io::stderr().lock(),
            format_args!("temporal_refusal cases=8 wall={:?}\n", started.elapsed()),
        )
        .unwrap();
    }
    ...
    fn check_nan_detects_nan_in_mu() {
        ...
        let working = WorkingCopy::cold_start(
            10,
            &ModelConfig::default(),
            1000,
