...
pub struct Tolerances {
    ...
    /// read uniformly: `tol.exact` instead of a bare literal.
    ///
    /// It bounds numeric exactness and nothing more. A zero tolerance
    /// admits `+0.0` against `-0.0`, so a test whose promise is that a
    /// value came back unchanged — a restore, a round trip, a read that
    /// must not mutate what it read — makes that promise with
    /// `assert_bits_equal` instead, and this field bounds the
    /// assertions whose subject is an arithmetic result.
    pub exact: f64,
}
...
/// ´const:assayer:scenario-tolerance-defaults-form-x3bd300bf´
