...
//! Domain-aware assertion helpers for scalar bounds, reckonings, tag profiles, and health reports.
//!
//! # Test index
//!
//! | Test | Area | Claim |
//! |------|------|-------|
//! | [`the_exact_tolerance_admits_the_two_zeros`] | harness | The exact tolerance is numeric equality: it reads the two zeros as one value, which is the whole difference between a bound and an identity. |
//! | [`the_bit_comparison_separates_the_two_zeros`] | harness | cites (´claim:harness:a-zero-tolerance-is-numeric-equality-so-a-bit-identity-promise-is-checked-by-the-bits´) |
//! | [`the_bit_comparison_holds_a_value_against_itself`] | harness | cites (´claim:harness:a-zero-tolerance-is-numeric-equality-so-a-bit-identity-promise-is-checked-by-the-bits´) |
...
/// Assert that two floating-point values are the same value, bit for bit.
///
/// [`assert_near`] with a zero tolerance is numeric equality, which is a
/// weaker claim than it looks: `+0.0` and `-0.0` differ in the sign bit and
/// still compare equal by value, so a test promising that a number survived a
/// restore, a round trip, or a read that must not mutate it passes on a value
/// that came back changed. Comparing [`f64::to_bits`] asks the question the
/// promise makes. It also treats two NaNs with the same payload as equal,
/// which `==` does not, and which a preserved payload is entitled to.
///
/// # Panics
///
/// Panics if the two values differ in any bit.
#[track_caller]
pub fn assert_bits_equal(actual: f64, expected: f64, label: &str) {
    assert!(
        actual.to_bits() == expected.to_bits(),
        "{label}: expected {expected} bit for bit (0x{:016x}), got {actual} (0x{:016x})",
        expected.to_bits(),
        actual.to_bits(),
    );
}
...
mod tests {
    ...
    /// The exact tolerance is numeric equality: it reads the two zeros as one
    /// value, which is the whole difference between a bound and an identity.
    ///
    /// ´claim:harness:a-zero-tolerance-is-numeric-equality-so-a-bit-identity-promise-is-checked-by-the-bits´
    /// ´test:unit:the-exact-tolerance-admits-the-two-zeros´
    #[test]
    fn the_exact_tolerance_admits_the_two_zeros() {
        let tol = crate::testing::DEFAULT_TOLERANCES;
        assert_near(-0.0, 0.0, tol.exact, "a zero tolerance is numeric equality");
    }

    /// The bit comparison reads them as the two values they are.
    ///
    /// (´claim:harness:a-zero-tolerance-is-numeric-equality-so-a-bit-identity-promise-is-checked-by-the-bits´)
    /// ´test:unit:the-bit-comparison-separates-the-two-zeros´
    #[test]
    #[should_panic(expected = "the sign bit is part of the value")]
    fn the_bit_comparison_separates_the_two_zeros() {
        assert_bits_equal(-0.0, 0.0, "the sign bit is part of the value");
    }

    /// And it holds a value against itself, payload and all.
    ///
    /// (´claim:harness:a-zero-tolerance-is-numeric-equality-so-a-bit-identity-promise-is-checked-by-the-bits´)
    /// ´test:unit:the-bit-comparison-holds-a-value-against-itself´
    #[test]
    fn the_bit_comparison_holds_a_value_against_itself() {
        assert_bits_equal(-0.0, -0.0, "the same zero");
        assert_bits_equal(f64::NAN, f64::NAN, "a NaN is the payload it carries");
    }
}
