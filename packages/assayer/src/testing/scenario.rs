...
use super::world::{ConstructionFixture, World, WorldBuildError, WorldBuilder};
...
/// # Panics
///
/// Panics if cold construction fails or if the complete measured construction baseline differs from the scenario declaration.
#[must_use]
pub fn scenario(instance_id: &str, seed: u64) -> Scenario {
    let tracing = init_tracing();
    let world = World::cold(instance_id, seed);
    world
        .guard_construction_fixture(ConstructionFixture::Scenario)
        .expect("scenario construction should reproduce its complete declaration");
    Scenario {
        world,
        _tracing: tracing,
    }
...
/// # Errors
///
/// Returns [`WorldBuildError`] if construction fails or if the complete measured construction baseline differs from the configured scenario declaration.
pub fn scenario_with(
    ...
    scenario.world.guard_construction_fixture(ConstructionFixture::ScenarioWith)?;
    Ok(scenario)
}
...
/// # Errors
///
/// Returns [`WorldBuildError`] if construction fails or if the complete measured construction baseline differs from the explicit-config scenario declaration.
pub fn scenario_with_config(
    ...
    world.guard_construction_fixture(ConstructionFixture::ScenarioWithConfig)?;
    Ok(Scenario {
        world,
        _tracing: tracing,
    })
