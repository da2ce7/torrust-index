...
/// An explicit request for one Ledger collection cycle.
///
/// The completion sender is answered only after every Sentinel visible to the
/// cycle has been swept. Periodic cycles carry no request and therefore need
/// no acknowledgement.
#[derive(Debug)]
pub struct LedgerGcTrigger {
    /// Receives the merged outcome after the requested sweep completes.
    completion: Sender<super::GcOutcome>,
}

impl LedgerGcTrigger {
    /// Creates a trigger whose receiver is the cycle-completion signal.
    #[cfg(any(test, feature = "test-support"))]
    pub(crate) const fn new(completion: Sender<super::GcOutcome>) -> Self {
        Self { completion }
    }

    /// Announces the completed cycle and transfers its outcome.
    fn complete(self, outcome: super::GcOutcome) {
        let _completion_result = self.completion.send(outcome);
    }
}
...
pub fn spawn_ledger_gc_scheduler(
    ...
    sweep: SweepConfig,
    clock: Arc<dyn Clock>,
    control_rx: Receiver<LedgerGcTrigger>,
) -> JoinHandle<()> {
