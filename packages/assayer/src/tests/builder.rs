...
//! | [`world_builder_without_seed_is_rejected`] | builder | A scenario builder with no declared seed is refused at construction with `MissingSeed`. Reproducibility is part of the harness contract, so omission is a typed host error rather than a panic or a hidden default. |
...
//! | [`test_entity_helper_deterministic`] | builder | The shared world's entity helper is a function of its name alone: the same name yields the same key, different names yield different keys, and every key carries bytes. Identity features are computed from those bytes, so a helper that varied between calls would make any test that depends on entity identity irreproducible. |
//! | [`test_report_helper_valid`] | builder | The shared report fixtures always supply the depth-zero root cell covering the whole domain, and the four-cell fixture adds its documented competitive cells on top of it. Report ingestion refuses a report with no root, so a fixture that omitted it would make every test built on it fail for a reason unrelated to what the test was about. |
...
//!
//! Direct production-builder calls remain only where construction itself is the subject: those tests must observe `AssayerBuilder` state or exact `BuildError` variants before a `World` can exist, while every engine used as fixture setup enters through `World`.
//!
//! # Cross-References
//!
...
use crate::testing::{ACK_DEADLINE, World, WorldBuildError, golden_report_4cell, minimal_report};
...
/// A scenario builder with no declared seed is refused at construction with
/// `MissingSeed`. Reproducibility is part of the harness contract, so omission
/// is a typed host error rather than a panic or a hidden default.
///
/// ´claim:builder:a-scenario-builder-with-no-seed-is-refused-at-construction´
/// ´test:crate:world-builder-without-seed-is-rejected´
#[test]
fn world_builder_without_seed_is_rejected() {
    let result = World::builder(AssayerConfig {
    ...
    assert!(matches!(result, Err(WorldBuildError::MissingSeed)));
}
...
fn build_defaults_pass_validation() {
    ...
    let _world = World::builder(AssayerConfig {
        instance_id: "test".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
...
/// The shared world's entity helper is a function of its name alone: the same name yields the same key, different names yield different keys, and every key carries bytes. Identity features are computed from those bytes, so a helper that varied between calls would make any test that depends on entity identity irreproducible.
///
...
fn test_entity_helper_deterministic() {
    ...
    let e1a = World::entity("entity-42");
    let e1b = World::entity("entity-42");
    assert_eq!(e1a, e1b);

    // Different seeds produce different keys
    let e2 = World::entity("entity-43");
    assert_ne!(e1a, e2);
...
fn preseed_basic_round_trip() {
    ...
    let entries: Vec<PreSeedEntry> = (0..5)
        .map(|i| PreSeedEntry {
            entity: World::entity(&format!("entity-{i}")),
            channel: ch,
...
fn preseed_result_fields_populated() {
    ...
    let spam_entries: Vec<PreSeedEntry> = (0..20)
        .map(|i| PreSeedEntry {
            entity: World::entity(&format!("entity-{}", 100 + i)),
            channel: ch,
            action_taken: Action::Allow,
...
fn builder_refuses_out_of_range_template_offsets() {
    ...
    let refused = |template: InteractionTemplate, expected_index: usize| {
        let result = Assayer::builder(AssayerConfig {
            instance_id: "test".to_owned(),
            infrastructure: crate::testing::test_infrastructure(),
            ..Default::default()
        })
...
fn builder_admits_boundary_template_offsets() {
    ...
    let assayer = Assayer::builder(AssayerConfig {
        instance_id: "test".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    ...
    let with_signal = Assayer::builder(AssayerConfig {
        instance_id: "test".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    ...
    let without_signal = Assayer::builder(AssayerConfig {
        instance_id: "test".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    ...
    let aggregate_past_block = Assayer::builder(AssayerConfig {
        instance_id: "test".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
