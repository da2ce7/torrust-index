...
//! | [`published_slot_moments_cross_publication_as_one_owned_slot`] | feature | A named Sentinel's complete mean and variance slices, phase, observation count, version, and layout generation arrive from one settled publication as owned vectors, and a label queued behind a parked model owner is visible when the probe returns. No other slot or model field is needed to state where this slot's coordinate system stands. |
...
use crate::testing::{DEFAULT_TOLERANCES, LabelSpec, TestRng, World, assert_bits_equal, assert_finite, assert_near};
...
fn standardise_zero_mean_unit_variance() {
    ...
    assert_bits_equal(phi[0], 1.0, "bias preserved");
...
fn standardise_shift_and_scale() {
    ...
    assert_bits_equal(phi[0], 1.0, "bias unchanged");
...
fn standardise_bias_excluded() {
    ...
    assert_bits_equal(phi[0], 1.0, "bias excluded");
}
...
fn restandardise_mismatch_bounded_at_fifty_labels() {
    ...
    assert_bits_equal(phi_lab[0], 1.0, "bias unchanged under re-standardisation");
}

/// A named Sentinel's complete mean and variance slices, phase, observation count, version, and layout generation arrive from one settled publication as owned vectors, and a label queued behind a parked model owner is visible when the probe returns. No other slot or model field is needed to state where this slot's coordinate system stands.
///
/// ´claim:feature:a-slot-moments-probe-copies-one-complete-slot-from-one-settled-publication´
/// ´test:crate:published-slot-moments-cross-publication-as-one-owned-slot´
#[test]
fn published_slot_moments_cross_publication_as_one_owned_slot() {
    let mut world = World::cold("slot-probe", 0x534C_4F54_5F50_524F);
    let sentinel = world.register_sentinel("slot").expect("the Sentinel registers");
    let assessment = world.assess(world.request("default", "slot-subject"));
    world
        .flush_observations()
        .expect("the assessment's cold-ramp observation settles before the baseline");
    let before = world
        .published_slot_moments("slot")
        .expect("the initial publication barrier succeeds")
        .expect("the registered Sentinel has a published slot");
    ...
    let after = world
        .published_slot_moments("slot")
        .expect("the slot probe crosses the publication barrier")
        .expect("the registered Sentinel remains published");

    assert_eq!(after.sentinel, sentinel, "the name resolves to the registered slot owner");
    assert!(!after.means.is_empty(), "the published Sentinel slot must have coordinates");
    assert_eq!(
        after.means.len(),
        after.variances.len(),
        "mean and variance slices cover the same slot"
    );
    assert!(
        after.means.iter().all(|value| value.is_finite()),
        "every copied mean is finite"
    );
    assert!(
        after.variances.iter().all(|value| value.is_finite()),
        "every copied variance is finite"
    );
    assert!(
        after.published_version > before.published_version,
        "the barrier must publish the queued label"
    );
    assert_eq!(
        after.layout_generation, before.layout_generation,
        "a label does not silently change the slot layout"
    );
}
