...
fn concordance_initial_thresholds() {
    ...
    assert_eq!(thresholds.map(f64::to_bits), [0.5f64.to_bits(); 4]);
}
...
fn concordance_thresholds_readable_during_observe() {
    ...
    assert_eq!(t1.map(f64::to_bits), [0.5f64.to_bits(); 4]);
    ...
    assert_eq!(t2.map(f64::to_bits), [0.5f64.to_bits(); 4]); // Not recalibrated yet
}
...
fn concordance_window_wraps() {
    ...
    for &t in &thresholds {
        assert_near(t, 10.0, DEFAULT_TOLERANCES.exact, "threshold after wraparound");
    }
}
...
fn identity_entries_exits_change_rate() {
    ...
    // Change rate EWMA updates (exact value depends on alpha), so the claim is
    // that it moved at all rather than where it moved to.
    assert!((tracker.change_rate_ewma - change_rate_before).abs() > 0.0 || change_rate_before.to_bits() == 0.0f64.to_bits());
}
...
fn platt_tracker_serde() {
    ...
    crate::testing::assert_bits_equal(restored.last_delta_cal, tracker.last_delta_cal, "restored last_delta_cal");
    crate::testing::assert_bits_equal(
...
fn concordance_observe_below_interval() {
    ...
    assert_eq!(tracker.current_thresholds().map(f64::to_bits), [0.5f64.to_bits(); 4]);
}
...
fn concordance_checkpoint_state_round_trip() {
    ...
    assert_eq!(
        restored.current_thresholds().map(f64::to_bits),
        original_health.current_thresholds.map(f64::to_bits)
    );
...
fn event_platt_first_fit() {
    ...
    match received {
        HealthEvent::Convergence(ConvergenceEvent::PlattFirstFit {
            ...
            let tol = DEFAULT_TOLERANCES;
            assert_near(kappa_sister, 1.5, tol.exact, "PlattFirstFit kappa_sister");
            assert_near(kappa_anchor, 2.0, tol.exact, "PlattFirstFit kappa_anchor");
            assert_near(delta_cal, 0.42, tol.exact, "PlattFirstFit delta_cal");
        }
...
fn event_platt_converged() {
    ...
    match received {
        HealthEvent::Convergence(ConvergenceEvent::PlattConverged {
            ...
            assert_near(delta_cal, 0.005, DEFAULT_TOLERANCES.exact, "PlattConverged delta_cal");
        }
