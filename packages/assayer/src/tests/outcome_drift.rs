...
use crate::testing::{DEFAULT_TOLERANCES, TestRng, assert_bits_equal, assert_finite, assert_near};
...
fn dimension_mismatch_returns_prior() {
    ...
    let tol = DEFAULT_TOLERANCES;
    assert_near(pred.predicted_raw, 0.0, tol.exact, "mismatched-dim point estimate");
    assert_near(pred.uncertainty, 1.0, tol.exact, "mismatched-dim unit-parameter uncertainty");
    ...
    assert_near(scaled.predicted_raw, 0.0, tol.exact, "scaled prior point estimate");
    assert_near(scaled.uncertainty, 4.0, tol.exact, "κ_a·√(T_corr/λ_prior) = 2·√4");
    assert_near(
...
fn drift_reset_cusums_only() {
    ...
    let tol = DEFAULT_TOLERANCES;
    assert_near(drift.s_plus, 0.0, tol.exact, "S⁺ after reset_cusums");
    assert_near(drift.s_minus, 0.0, tol.exact, "S⁻ after reset_cusums");
    assert_eq!(drift.steps_since_reset, 0, "steps should be 0");
    assert_bits_equal(drift.mean_abs_residual, 0.5, "MAR preserved");
    assert_bits_equal(drift.residual_sign_ewma, 0.3, "sign EWMA preserved");
}
...
fn drift_reset_all() {
    ...
    let tol = DEFAULT_TOLERANCES;
    assert_near(drift.s_plus, 0.0, tol.exact, "S⁺ after reset_all");
    assert_near(drift.s_minus, 0.0, tol.exact, "S⁻ after reset_all");
    assert_near(drift.mean_abs_residual, 0.0, tol.exact, "MAR after reset_all");
    assert_near(drift.residual_sign_ewma, 0.0, tol.exact, "sign EWMA after reset_all");
    assert_eq!(drift.steps_since_reset, 0, "steps should be 0");
}
...
fn drift_state_serde_round_trip() {
    ...
    assert_bits_equal(restored.s_plus, original.s_plus, "S⁺ round-trip");
    assert_bits_equal(restored.s_minus, original.s_minus, "S⁻ round-trip");
    assert_bits_equal(restored.mean_abs_residual, original.mean_abs_residual, "MAR round-trip");
    assert_bits_equal(
