...
//! | [`health_event_barrier_orders_producers_before_drain`] | wellness | The health-event barrier orders the model owner's producer paths before it drains: a queued label and cold-ramp observation yield both of their convergence events on the first drain, and an asynchronously queued lifecycle removal yields its marginalisation event on the next. A final barrier returns an empty vector, proving that each call drains the receiver only after the work it covers rather than sampling an arbitrarily timed prefix. |
...
fn build_health_test_world() -> World {
    let config = AssayerConfig {
        instance_id: "health-test".to_owned(),
...
fn zero_sentinel_assessment_metric_counts_live_results() {
    ...
    assert_eq!(counter.value.to_bits(), 2.0f64.to_bits());
...
fn sentinel_coverage_metric_reads_live_report_presence() {
    ...
    assert_eq!(report.sentinel_coverage.to_bits(), 0.5f64.to_bits());
    ...
    assert_eq!(coverage.value.to_bits(), 0.5f64.to_bits());
}
...
fn signal_cache_metrics_read_live_cache_statistics() {
    ...
    let world = World::builder(AssayerConfig {
        instance_id: "health-signal-cache".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    .channel(CHANNEL, default_policy())
    ...
    assert_eq!(hit_rate.value.to_bits(), report.signal_cache.hit_rate().to_bits());
    assert_eq!(utilisation.value.to_bits(), report.signal_cache.utilisation().to_bits());
}
...
fn positive_class_prior_metrics_read_the_published_snapshot() {
    ...
    world.flush_labels().expect("model owner publishes the queued label");

    let snapshot = world.assayer().shared().published.load();
    let summary = world.assayer().health_summary();
    assert_eq!(summary.p_positive_global.to_bits(), snapshot.p_positive_global.to_bits());
    assert_eq!(summary.p_positive_eligible.to_bits(), snapshot.p_positive_eligible.to_bits());
    ...
    assert_eq!(global.value.to_bits(), snapshot.p_positive_global.to_bits());
    assert_eq!(eligible.value.to_bits(), snapshot.p_positive_eligible.to_bits());
}
...
fn discrimination_auc_above_half_after_200_labels() {
    ...
    let oracle_rows: Vec<_> = entries.iter().map(|(rho, positive, _, _, _)| (*rho, *positive)).collect();
    let expected = pairwise_rank(&oracle_rows, crate::MonitoringConfig::default().min_auc_class_count, 0.0)
        .expect("both classes meet the default gate");
    assert_eq!(expected.to_bits(), 0.70f64.to_bits());
    assert_near(
        auc,
        expected,
        DEFAULT_TOLERANCES.auc,
        "aggregate AUC of the overlapping ladder",
    );
}
...
fn discrimination_auc_reaches_one_under_total_separation() {
    ...
    let expected = pairwise_rank(&oracle_rows, crate::MonitoringConfig::default().min_auc_class_count, 0.0)
        .expect("both classes meet the default gate");
    assert_eq!(expected.to_bits(), 1.0f64.to_bits());
    assert_near(auc, expected, DEFAULT_TOLERANCES.auc, "AUC under total separation");
}
...
fn informativeness_reads_the_operational_slot_weights() {
    ...
    world.flush_labels().expect("model owner publishes the queued labels");
...
fn snapshot_carries_version_age_drift_and_inflation() {
    ...
    assert_eq!(
        age.to_bits(),
        90.0f64.to_bits(),
        "age should follow the virtual clock exactly"
    );
...
fn anchor_freeze_state_reaches_compact_and_full_surfaces() {
    let mut config = AssayerConfig {
        instance_id: "anchor-freeze-boundary".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
...
fn stage_change_event_emitted_on_first_label() {
    ...
    let events = world
        .drain_health_events()
        .expect("health-event producers cross their barriers");
...
fn host_initiated_drift_reset_reaches_the_accumulators() {
    ...
    world
        .flush_labels()
        .expect("model owner publishes the reset and queued label");
...
fn lifecycle_event_resets_drift_accumulators() {
    ...
    world
        .flush_labels()
        .expect("model owner publishes the lifecycle change and queued label");
...
fn label_sanitisation_counts_reach_label_integrity_health() {
    ...
    world
        .label(make_label(reckoning.assessment.id, 1.0))
        .expect("label should succeed");
    world.flush_labels().expect("model owner publishes the clean label");
    ...
    world.flush_labels().expect("model owner publishes the repaired label");
...
fn observability_health_derives_maturity_and_resolution_from_producers() {
    let mut config = AssayerConfig {
        instance_id: "observability-ratios".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
...
fn feedback_latency_decomposes_into_four_stages_from_producers() {
    ...
    let mut world = World::builder(AssayerConfig {
        instance_id: "feedback-latency".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    .channel(CHANNEL, default_policy())
...
/// The health-event barrier orders the model owner's producer paths before it
/// drains: a queued label and cold-ramp observation yield both of their
/// convergence events on the first drain, and an asynchronously queued
/// lifecycle removal yields its marginalisation event on the next. A final
/// barrier returns an empty vector, proving that each call drains the receiver
/// only after the work it covers rather than sampling an arbitrarily timed
/// prefix.
///
/// ´claim:wellness:the-health-event-barrier-orders-every-model-owner-producer-before-draining-the-receiver´
/// ´test:crate:health-event-barrier-orders-producers-before-drain´
#[test]
fn health_event_barrier_orders_producers_before_drain() {
    use crate::health::{ConvergenceEvent, HealthEvent, LifecycleHealthEvent};

    let mut config = AssayerConfig {
        instance_id: "health-event-barrier".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
    config.standardisation.n_init = 1;
    let mut world = World::builder(config)
        .channel(CHANNEL, default_policy())
        .seed(SEED)
        .build()
        .expect("health-event barrier world builds");
    let sentinel_id = world.register_sentinel("health-event-producer").expect("Sentinel registers");

    let reckoning = world
        .derive_for_request(world.request(CHANNEL, "barrier-event"))
        .expect("assess/derive should succeed");
    world
        .label(make_label(reckoning.assessment.id, 1.0))
        .expect("label should enqueue");

    let first = world.drain_health_events().expect("health-event producers settle");
    assert!(
        first.iter().any(|event| matches!(
            event,
            HealthEvent::Convergence(ConvergenceEvent::CompositeStageChanged { .. })
        )),
        "the label producer precedes the drain"
    );
    assert!(
        first
            .iter()
            .any(|event| matches!(event, HealthEvent::Convergence(ConvergenceEvent::BatchInitComplete))),
        "the cold-ramp observation producer precedes the drain"
    );

    world
        .assayer()
        .deregister_sentinel(sentinel_id)
        .expect("lifecycle removal should enqueue");
    let second = world.drain_health_events().expect("lifecycle event producer settles");
    assert!(
        second.iter().any(|event| matches!(
            event,
            HealthEvent::Lifecycle(LifecycleHealthEvent::MarginalisationCompleted { .. })
        )),
        "the command producer precedes the drain"
    );

    let final_drain = world.drain_health_events().expect("an empty producer prefix settles");
    assert!(final_drain.is_empty(), "a completed drain leaves the receiver empty");
}
