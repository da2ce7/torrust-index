...
//! | [`posterior_definiteness_after_updates_and_marginalisation`] | bayes | Marginalisation returns a genuinely smaller model — the removed coordinates are gone from the dimension — and what remains is still a usable posterior, with strictly positive precision and covariance on every surviving diagonal. A shrinking model must stay invertible, or the very next update would have nothing well-defined to work from. The seeded sweep draws 459 posterior histories because that many independent draws give more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this input distribution. Every history varies the width, prior, decay, update weight, feature vectors, update count, and arbitrary non-empty proper removal set; the figure is a starting bound over that declared distribution, not a claim that smaller defect regions cannot exist. The least eigenvalue is checked after every rank-one update and after marginalisation, witnessing the maintained precision invariant (´inv:guarantee:precision´). |
...
use crate::testing::{assert_below, assert_finite, decay_recurrence, run_seeded_sweep};
...
fn positive_definite(matrix: &SymmetricMatrix, label: &str) -> Result<(), String> {
    let (_, lambda_min) = eigenvalue_min_max(matrix).map_err(|error| format!("{label}: eigensolver refused: {error:?}"))?;
    if lambda_min > 0.0 {
        Ok(())
    } else {
        Err(format!("{label}: least eigenvalue {lambda_min} is not positive"))
    }
}
...
fn new_starts_at_zero_mean_under_the_isotropic_prior() {
    ...
    // μ = 0
    for i in 0..100 {
        assert_eq!(m.mu()[i].to_bits(), 0.0f64.to_bits(), "mu[{i}] should be 0");
    }
...
fn new_p1() {
    ...
    assert_eq!(m.dim(), 1);
    assert_eq!(m.mu()[0].to_bits(), 0.0f64.to_bits());
...
/// Marginalisation returns a genuinely smaller model — the removed coordinates are gone from the dimension — and what remains is still a usable posterior, with strictly positive precision and covariance on every surviving diagonal. A shrinking model must stay invertible, or the very next update would have nothing well-defined to work from.
///
/// The seeded sweep draws 459 posterior histories because that many independent draws give more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this input distribution. Every history varies the width, prior, decay, update weight, feature vectors, update count, and arbitrary non-empty proper removal set; the figure is a starting bound over that declared distribution, not a claim that smaller defect regions cannot exist. The least eigenvalue is checked after every rank-one update and after marginalisation, witnessing the maintained precision invariant (´inv:guarantee:precision´).
///
/// ´claim:bayes:marginalisation-yields-a-smaller-model-that-is-still-positive-on-every-surviving-diagonal´
/// ´test:crate:posterior-definiteness-after-updates-and-marginalisation´
#[test]
fn posterior_definiteness_after_updates_and_marginalisation() {
    const CASES: usize = 459;
    const SEED: u64 = 0xB05E_71A1_DE1F_117E;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| {
            let width = usize::try_from(rng.next_range(6)).unwrap_or(0) + 3;
            let update_count = usize::try_from(rng.next_range(7)).unwrap_or(0) + 1;
            let prior_precision = rng.next_f64().mul_add(1.95, 0.05);
            let gamma_eff = rng.next_f64().mul_add(0.1, 0.9);
            let lambda_floor = prior_precision * 0.01;
            let weight = rng.next_f64().mul_add(1.99, 0.01);
            let features = (0..update_count)
                .map(|_| (0..width).map(|_| rng.next_f64().mul_add(2.0, -1.0)).collect())
                .collect();
            let mut remove_indices: Vec<_> = (0..width).filter(|_| rng.coin()).collect();
            if remove_indices.is_empty() {
                remove_indices.push(usize::try_from(rng.next_range(u64::try_from(width).unwrap_or(u64::MAX))).unwrap_or(0));
            } else if remove_indices.len() == width {
                remove_indices.pop();
            }
        ...
        |case| {
            let mut model = BayesianLinearModel::new(case.width, case.prior_precision, 1_000);
            for (update_index, feature) in case.features.iter().enumerate() {
                let phi_hat = vec_to_col(feature);
                let (v, h) = model.covariance().quadratic_form_with_product(&phi_hat);
                let _outcome =
                    model.apply_leverage_bounded_update(&phi_hat, &v, h, case.weight, 0.0, case.gamma_eff, case.lambda_floor);
                positive_definite(model.precision(), "precision after rank-one update")
                    .map_err(|reason| format!("update_index={update_index}: {reason}"))?;
                positive_definite(model.covariance(), "covariance after rank-one update")
                    .map_err(|reason| format!("update_index={update_index}: {reason}"))?;
            }

            model.marginalise(&case.remove_indices, &SchurConfig::default(), ModelId::Operational);
            let expected_width = case.width - case.remove_indices.len();
            if model.dim() != expected_width {
                return Err(format!("marginal dimension {}, expected {expected_width}", model.dim()));
            }
            positive_definite(model.precision(), "precision after marginalisation")?;
            positive_definite(model.covariance(), "covariance after marginalisation")
        },
    );
}
    ...
    for i in 0..10 {
        for j in 10..13 {
            assert_eq!(
                b[(i, j)].to_bits(),
                0.0f64.to_bits(),
                "B[{i},{j}] should be zero (cross-block)"
            );
            assert_eq!(
                b[(j, i)].to_bits(),
                0.0f64.to_bits(),
                "B[{j},{i}] should be zero (cross-block)"
            );
            assert_eq!(
                s[(i, j)].to_bits(),
                0.0f64.to_bits(),
                "Σ[{i},{j}] should be zero (cross-block)"
            );
            assert_eq!(
                s[(j, i)].to_bits(),
                0.0f64.to_bits(),
                "Σ[{j},{i}] should be zero (cross-block)"
            );
        }
    }
}
...
fn new_l024_defaults() {
    ...
    assert_eq!(m.last_diagonal_ratio().to_bits(), 0.0f64.to_bits());
    assert_eq!(m.last_sync_error().to_bits(), 0.0f64.to_bits());
...
fn clean_high_sync_error_updates_model_cadence() {
    ...
    assert_eq!(model.last_sync_error().to_bits(), sync_error.to_bits());
...
fn combined_decay_equivalence() {
    ...
    let gamma_combined = decay_recurrence(1.0, gamma_label, 1, gamma_t, elapsed_hours);
