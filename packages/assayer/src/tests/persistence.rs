...
//! | [`checkpoint_scheduler_barrier_waits_for_trigger_completion`] | persistence | The checkpoint-scheduler barrier returns while the model owner is deliberately parked, proving that its queue-owned completion signal means the checkpoint request has entered the downstream command queue rather than that the file has been written. The separate model-owner barrier writes the checkpoint only after the owner is released, keeping the scheduler trigger and downstream processing as two explicit guarantees. |
...
//! | [`restore_measures_downtime_on_the_injected_clock`] | persistence | A production checkpoint written by one scenario is restored by another at a later scenario reading, and the restored Ledger rate is aged by exactly the interval between those readings. Scenario time and its barriers are already supplied by the harness (´entry:assayer:harness-scenario-time´); this witness covers the production checkpoint and restore path that must consume its persistent domain without consulting the wall clock. |
...
//! | [`checkpoint_restores_runtime_registries_with_rebind_semantics`] | persistence | A restored instance rebuilds its runtime Sentinel and outcome-axis registries from the working copy before a host repeats any declarations. Reports therefore reach a restored Sentinel immediately, and the first matching registrations attach to the existing slot and model without publishing another layout or duplicating feature positions. Destructive deregistration still finds both restored identifiers afterwards. |
//! | [`checkpoint_restores_identity_state_on_registration`] | persistence | An identity payload recovered from a production checkpoint remains pending until the host repeats the matching dimension registration. That call supplies the encode function a checkpoint cannot carry, rebuilds the dedicated owner's graph at its captured energy, publishes the captured competitive set, and restores the already-decayed cell outcome state. It publishes no lifecycle extension because the restored working copy already contains those cells' feature positions. |
...
use crate::config::types::{AssayerConfig, InfrastructureConfig, PersistenceConfig};
...
use crate::testing::registrations::{sentinel_reg, spatial_axis_reg};
use crate::testing::{ACK_DEADLINE, LabelSpec, World};
use crate::types::{Action, AssessmentId, ChannelId, OutcomeAxisId, PersistentTimestamp, SentinelId};
...
fn test_payload(working: &WorkingCopy) -> CheckpointPayload {
    working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), Vec::new())
}
...
pub(super) fn restore_validation_fields() -> BTreeSet<String> {
...
fn test_pending_context(id: AssessmentId) -> PendingContext {
    PendingContext {
        ...
        entity: World::entity(&format!("pending-context-{}", id.0)),
...
fn test_journal_entry(seq: u64) -> JournalEntry {
    JournalEntry {
        ...
        timestamp: PersistentTimestamp::new(1_700_000_000, 0),
    }
}
...
fn checkpoint_round_trip() {
    ...
    assert_eq!(restored.kappa_v.to_bits(), payload.kappa_v.to_bits());
    assert_eq!(restored.kappa_sister.to_bits(), payload.kappa_sister.to_bits());
    assert_eq!(restored.kappa_anchor.to_bits(), payload.kappa_anchor.to_bits());
    assert_eq!(restored.p_positive_global.to_bits(), payload.p_positive_global.to_bits());
    assert_eq!(restored.p_positive_eligible.to_bits(), payload.p_positive_eligible.to_bits());
...
fn checkpoint_restores_marginalisation_error_ledger() {
    ...
    let restored = WorkingCopy::from_checkpoint_payload(
        &restored_payload,
        100,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn checkpoint_crc32_validation() {
    ...
    match checkpoint::read_checkpoint(&path) {
        Err(checkpoint::CheckpointError::IntegrityFailed { .. } | checkpoint::CheckpointError::Deserialise(_)) => {
            // A checksum collision may leave deserialisation to reject the corrupt payload.
        }
...
fn checkpoint_l024_fields_preserved() {
    ...
    for (label, orig, rest) in [
        ...
        assert_eq!(
            rest.last_diagonal_ratio.to_bits(),
            orig.last_diagonal_ratio.to_bits(),
            "{label} last_diagonal_ratio"
        );
        assert_eq!(
            rest.last_sync_error.to_bits(),
            orig.last_sync_error.to_bits(),
...
fn checkpoint_scalars_preserved() {
    ...
    let payload = working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), Vec::new());
    checkpoint::write_checkpoint(&path, &payload).expect("write failed");
    let restored = checkpoint::read_checkpoint(&path).expect("read failed");

    assert_eq!(restored.kappa_v.to_bits(), 2.5_f64.to_bits());
    assert_eq!(restored.kappa_sister.to_bits(), 3.0_f64.to_bits());
    assert_eq!(restored.kappa_anchor.to_bits(), 1.5_f64.to_bits());
    assert_eq!(restored.p_positive_global.to_bits(), 0.42_f64.to_bits());
    assert_eq!(restored.p_positive_eligible.to_bits(), 0.38_f64.to_bits());
...
fn checkpoint_concordance_state_preserved() {
    ...
    assert_eq!(
        restored.concordance_state.thresholds.map(f64::to_bits),
        payload.concordance_state.thresholds.map(f64::to_bits)
    );
...
fn checkpoint_overwrite_existing() {
    ...
    let payload1 = working1.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), Vec::new());
    ...
    let payload2 = working2.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), Vec::new());
...
fn journal_entry_fields_round_trip() {
    ...
    assert_eq!(restored.label.valence.to_bits(), (-1.0_f64).to_bits());
...
fn journal_entry_with_outcomes() {
    ...
    assert_eq!(outcomes[&OutcomeAxisId(1)].to_bits(), 0.75_f64.to_bits());
    assert_eq!(outcomes[&OutcomeAxisId(2)].to_bits(), (-0.5_f64).to_bits());
    assert_eq!(outcomes[&OutcomeAxisId(99)].to_bits(), 1.0_f64.to_bits());
}
...
fn recovery_with_time_decay() {
    ...
    let now_secs = PersistentTimestamp::new(1_700_000_000, 0).seconds;
    ...
    let result = recovery::attempt_restore(
        &cp_path,
        &jl_path,
        &config,
        &[],
        &[],
        PersistentTimestamp::new(1_700_000_000, 0),
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
    )
...
fn recovery_structural_mismatch() {
    ...
    let working = test_working_copy();
    let mut payload = working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), Vec::new());
...
/// The checkpoint-scheduler barrier returns while the model owner is
/// deliberately parked, proving that its queue-owned completion signal means
/// the checkpoint request has entered the downstream command queue rather than
/// that the file has been written. The separate model-owner barrier writes the
/// checkpoint only after the owner is released, keeping the scheduler trigger
/// and downstream processing as two explicit guarantees.
///
/// ´claim:persistence:the-checkpoint-scheduler-barrier-answers-once-its-trigger-has-enqueued-the-request-and-before-the-owner-must-apply-it´
/// ´test:crate:checkpoint-scheduler-barrier-waits-for-trigger-completion´
#[test]
fn checkpoint_scheduler_barrier_waits_for_trigger_completion() {
    let dir = test_dir();
    ...
    .build()
    .expect("persistent barrier world builds");
    ...
    world
        ...
        .expect("checkpoint scheduler acknowledges its trigger");
    assert!(
        !checkpoint_path.exists(),
        "scheduler acknowledgement excludes the parked owner's checkpoint write"
    );

    owner_block.release();
    world.flush_labels().expect("model owner applies the queued checkpoint");
    assert!(
        checkpoint_path.exists(),
        "the downstream barrier completes the checkpoint write"
    );
}
...
fn journal_open_failure_refuses_the_build() {
    ...
    let result = World::builder(config).seed(0x7065_7273_6973_742D).build();
    assert!(
        matches!(
            result,
            Err(crate::testing::WorldBuildError::Build(
                crate::BuildError::PersistenceSetupFailed(_)
            ))
        ),
...
fn test_sentinel_ledger(n_extra: usize) -> SentinelLedger {
    ...
    ledger.ensure_root(PersistentTimestamp::new(1_700_000_000, 0));
    for i in 0..n_extra {
...
fn test_identity_payload(cells: &[CompetitiveCellId]) -> IdentityDimensionPayload {
    let cell_outcomes: Vec<(CompetitiveCellId, CellOutcomeState)> = cells
        .iter()
        .map(|&c| (c, CellOutcomeState::new_neutral(PersistentTimestamp::new(1_700_000_000, 0))))
        .collect();
...
fn checkpoint_with_ledger() {
    ...
    let payload = working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), ledger_state, Vec::new());
...
fn checkpoint_with_identity() {
    ...
    let payload = working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), Vec::new(), identity_state);
...
fn checkpoint_with_cell_outcome_state() {
    ...
    let cell_outcomes: Vec<(CompetitiveCellId, CellOutcomeState)> = cells
        ...
        .enumerate()
        .map(|(i, &cell)| {
            let mut state = CellOutcomeState::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
            #[allow(clippy::cast_precision_loss)]
            {
...
/// A production checkpoint written by one scenario is restored by another at
/// a later scenario reading, and the restored Ledger rate is aged by exactly
/// the interval between those readings. Scenario time and its barriers are
/// already supplied by the harness (´entry:assayer:harness-scenario-time´);
/// this witness covers the production checkpoint and restore path that must
/// consume its persistent domain without consulting the wall clock.
///
...
fn restore_measures_downtime_on_the_injected_clock() {
    ...
    let dir = test_dir();
    let config = AssayerConfig {
        instance_id: "stage-clock-capture".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..AssayerConfig::default()
    };
    let gamma = config.temporal.gamma_t_ledger;
    let mut world = World::builder(config)
        .persistence_dirs(dir.path(), dir.path())
        .seed(0x7374_6167_652D_6370)
        .build()
        .expect("checkpoint scenario builds");
    let sentinel = world.register_sentinel("persisted").expect("Sentinel registration succeeds");
    let captured = world.clock().now();
    world
        .assayer()
        .outcome_ledger
        .get(sentinel)
        .expect("registration creates a Ledger")
        .write()
        .expect("Ledger write lock")
        .root_mut()
        .ewma_bad_rate = 0.5;
    let payload = crate::testing::durable_checkpoint(&world).expect("scenario checkpoint is readable");
    assert_eq!(
        payload.checkpoint_timestamp, captured,
        "checkpoint takes the scenario reading"
    );
    let fork = world
        .fork_persistence_after(Duration::from_secs(24 * 3600), |_| Ok(()))
        .expect("later scenario restores an isolated checkpoint");
    let (_, restored_world) = fork.arms();
    let restored_at = restored_world.clock().now();
    let expected = 0.5 * crate::numerics::decay_factor(gamma, restored_at.hours_since(&captured));
    let ledger = restored_world
        .assayer()
        .outcome_ledger
        .get(sentinel)
        .expect("restored scenario exposes the persisted Ledger");
    let restored_guard = ledger.read().expect("restored Ledger read lock");
    let restored_entry = restored_guard.root();
    assert!(
        (restored_entry.ewma_bad_rate - expected).abs() < 1e-12,
        "scenario interval: expected {expected}, got {}",
        restored_entry.ewma_bad_rate
    );
    assert_eq!(
        restored_entry.last_updated, restored_at,
        "restore restamps with the later scenario reading"
    );
    eprintln!(
        "converted_clock fork_wall={:?} witness_wall={:?}",
        fork.wall(),
        started.elapsed()
    );
}
...
fn restore_refuses_a_journal_corrupt_before_its_tail() {
    ...
    match recovery::attempt_restore(
        &cp_path,
        &jl_path,
        &config,
        &[],
...
fn checkpoint_complete_round_trip() {
    ...
    let payload = working.to_checkpoint_payload(PersistentTimestamp::new(1_700_000_000, 0), ledger_state, identity_state);
    ...
    assert_eq!(restored.kappa_v.to_bits(), 2.5_f64.to_bits());
    assert_eq!(restored.p_positive_global.to_bits(), 0.3_f64.to_bits());
    ...
    let wc2 = crate::snapshot::working::WorkingCopy::from_checkpoint_payload(
        &restored,
        100,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn checkpoint_axis_fields_round_trip() {
    ...
    let restored = crate::snapshot::working::WorkingCopy::from_checkpoint_payload(
        &restored_payload,
        100,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn checkpoint_drift_state_round_trip() {
    ...
    for (model_id, drift) in &working.drift_state {
        ...
        assert_eq!(back.s_plus.to_bits(), drift.s_plus.to_bits());
        assert_eq!(back.s_minus.to_bits(), drift.s_minus.to_bits());
        assert_eq!(back.mean_abs_residual.to_bits(), drift.mean_abs_residual.to_bits());
        assert_eq!(back.residual_sign_ewma.to_bits(), drift.residual_sign_ewma.to_bits());
...
fn checkpoint_ramp_recomputes_against_the_configured_horizon() {
    ...
    // moments it reached.
    let same = crate::snapshot::working::WorkingCopy::from_checkpoint_payload(
        &restored_payload,
        horizon,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
    ...
    let shortened = crate::snapshot::working::WorkingCopy::from_checkpoint_payload(
        &restored_payload,
        horizon / 2,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
    ...
    let lengthened = crate::snapshot::working::WorkingCopy::from_checkpoint_payload(
        &restored_payload,
        horizon * 2,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn checkpoint_standardisation_warm_start_end_to_end() {
    ...
    let (means_at_capture, variances_at_capture) = {
        let config = AssayerConfig {
            instance_id: "warm-start".to_owned(),
            model: ModelConfig::default(),
            infrastructure: InfrastructureConfig {
                label_channel_capacity: 100,
                ..crate::testing::test_infrastructure()
        ...
        let world = World::builder(config)
            .seed(0x7761_726D_2D73_7461)
            .build()
            .expect("warm-start capture world builds");
        let assayer = world.assayer();
        for i in 1..=20_u64 {
        ...
        assayer
            .flush_label_channel()
            .expect("the publication barrier answers for the labels");
        let v = assayer.shared().published.load().version;
        assert_eq!(v, 21, "labels should be processed");

        world.flush_labels().expect("checkpoint should succeed");
    ...
    assert!(
        variances_at_capture.iter().any(|&v| v.to_bits() != 1.0_f64.to_bits()),
    ...
    let config = AssayerConfig {
        instance_id: "warm-start-2".to_owned(),
        model: ModelConfig::default(),
        infrastructure: InfrastructureConfig {
            label_channel_capacity: 100,
            ..crate::testing::test_infrastructure()
    ...
    let rebuilt_world = World::builder(config)
        .seed(0x7761_726D_2D72_6573)
        .build()
        .expect("warm-start restore world builds");
    let rebuilt = rebuilt_world.assayer();
...
fn test_pending_assessment(id: AssessmentId) -> PendingAssessment {
    PendingAssessment {
        ...
        persistent_timestamp: crate::types::PersistentTimestamp::new(1_700_000_000, 0),
        entity: World::entity(&format!("pending-assessment-{}", id.0)),
        sentinel_extractions: HashMap::new(),
        identity_coordinates: HashMap::new(),
        identity_active_cells: HashMap::new(),
        active_sentinels: Vec::new(),
...
fn restored_identity_registration(id: crate::types::DimensionId) -> crate::owner::commands::IdentityDimensionRegistration {
    crate::owner::commands::IdentityDimensionRegistration {
        id,
        name: "restored-identity".to_owned(),
        description: "identity state restore witness".to_owned(),
        coordinate_semantics: "the first byte selects a prefix group".to_owned(),
        domain_bits: 128,
        depth_cutoff: 10,
        budget: crate::types::IdentityBudget::for_depth_cutoff(10),
        encode: |entity| entity.as_bytes().first().map_or(0, |&byte| u128::from(byte) << 100),
    }
}

/// A restored instance rebuilds its runtime Sentinel and outcome-axis
/// registries from the working copy before a host repeats any declarations.
/// Reports therefore reach a restored Sentinel immediately, and the first
/// matching registrations attach to the existing slot and model without
/// publishing another layout or duplicating feature positions. Destructive
/// deregistration still finds both restored identifiers afterwards.
///
/// ´claim:persistence:restored-runtime-registries-describe-the-working-copy-and-rebind-without-extension´
/// ´test:crate:checkpoint-restores-runtime-registries-with-rebind-semantics´
#[test]
fn checkpoint_restores_runtime_registries_with_rebind_semantics() {
    let dir = test_dir();
    let sentinel = SentinelId(41);
    let axis = OutcomeAxisId(42);

    {
        let config = AssayerConfig {
            instance_id: "registry-capture".to_owned(),
            model: ModelConfig::default(),
            infrastructure: InfrastructureConfig {
                label_channel_capacity: 100,
                ..crate::testing::test_infrastructure()
        ...
        let world = World::builder(config)
            .seed(0x7265_6769_7374_7279)
            .build()
            .expect("registry capture world builds");
        let assayer = world.assayer();
        assayer.register_sentinel(sentinel_reg(sentinel)).unwrap();
        assayer.register_outcome_axis(spatial_axis_reg(axis)).unwrap();
        assayer
            .flush_label_channel()
            .expect("the publication barrier answers for both registrations");
        let version = assayer.shared().published.load().version;
        assert!(version >= 3, "both registrations should publish before checkpoint");

        world.flush_labels().expect("checkpoint should succeed");
    }

    let config = AssayerConfig {
        instance_id: "registry-restore".to_owned(),
        model: ModelConfig::default(),
        infrastructure: InfrastructureConfig {
            label_channel_capacity: 100,
            ..crate::testing::test_infrastructure()
    ...
    let restored_world = World::builder(config)
        .seed(0x7265_6769_7374_7279)
        .build()
        .expect("registry restore world builds");
    let restored = restored_world.assayer();
    let before = restored.shared().published.load_full();
    let before_width = before.dimension_map.p;
    let before_slot = before.dimension_map.sentinel_slots[&sentinel].clone();
    let before_axis_width = before.outcome_models[&axis].parameters.p;

    restored
        .receive_sentinel_report(sentinel, crate::testing::minimal_report())
        .expect("a restored Sentinel accepts reports before its declaration is rebound");
    restored
        .register_sentinel(sentinel_reg(sentinel))
        .expect("the first matching Sentinel registration rebinds");
    restored
        .register_outcome_axis(spatial_axis_reg(axis))
        .expect("the first matching outcome-axis registration rebinds");

    let after = restored.shared().published.load_full();
    assert_eq!(after.version, before.version, "a rebind publishes no lifecycle extension");
    assert_eq!(after.dimension_map.p, before_width, "a rebind preserves the restored width");
    assert_eq!(after.dimension_map.sentinel_slots[&sentinel], before_slot);
    assert_eq!(after.outcome_models[&axis].parameters.p, before_axis_width);

    restored
        .deregister_outcome_axis(axis)
        .expect("the restored axis is registered");
    restored
        .deregister_sentinel(sentinel)
        .expect("the restored Sentinel is registered");
}

/// An identity payload recovered from a production checkpoint remains pending
/// until the host repeats the matching dimension registration. That call
/// supplies the encode function a checkpoint cannot carry, rebuilds the
/// dedicated owner's graph at its captured energy, publishes the captured
/// competitive set, and restores the already-decayed cell outcome state. It
/// publishes no lifecycle extension because the restored working copy already
/// contains those cells' feature positions.
///
/// ´claim:persistence:a-restored-identity-registration-seeds-the-owner-without-extending-the-model´
/// ´test:crate:checkpoint-restores-identity-state-on-registration´
#[test]
fn checkpoint_restores_identity_state_on_registration() {
    let dir = test_dir();
    let dimension_id = crate::types::DimensionId(73);
    let registration = restored_identity_registration(dimension_id);
    let captured_cell;
    let captured_energy;

    {
        ...
        config.instance_id = "identity-capture".to_owned();
        let world = World::builder(config)
            .seed(0x6964_656E_7469_7479)
            .build()
            .expect("identity capture world builds");
        let assayer = world.assayer();
        assayer.register_identity_dimension(registration.clone()).unwrap();
        world
            .flush_labels()
            .expect("the publication barrier answers for identity registration");
        {
            let dimensions = assayer.identity_dimensions.read().unwrap();
            let infra = dimensions.get(&dimension_id).expect("dimension registered");
            for byte in 0..32_u128 {
                assert!(infra.observe((byte % 8) << 100));
            }
        }

        world
            .flush_identity_maintenance()
            .expect("the identity and publication barriers answer for the observations");
        world.flush_labels().expect("first checkpoint should succeed");

        {
            let dimensions = assayer.identity_dimensions.read().unwrap();
            let infra = dimensions.get(&dimension_id).expect("dimension registered");
            let index = infra.load_competitive_set();
            captured_cell = *index.cell_ids().next().expect("observations publish a competitive cell");
            let snapshot = infra.graph_snapshot.load();
            captured_energy = snapshot
                .as_ref()
                .as_ref()
                .expect("graph snapshot was published")
                .total_energy();
            let cell_state = infra.cell_state.read().unwrap();
            let mut state = cell_state
                .get(&captured_cell)
                .expect("competitive cell has state")
                .lock()
                .unwrap();
            state.outcome.adverse_rate_ewma = 0.75;
            state.outcome.compressed_valence_ewma = 0.625;
            state.outcome.raw_valence_ewma = 0.5;
        }

        world.flush_labels().expect("second checkpoint should succeed");
    }
    ...
    config.instance_id = "identity-restore".to_owned();
    let restored_world = World::builder(config)
        .seed(0x7265_7374_6F72_6564)
        .build()
        .expect("identity restore world builds");
    let restored = restored_world.assayer();
    assert!(restored.identity_dimensions.read().unwrap().is_empty());
    assert!(restored.pending_restored_identity.lock().unwrap().contains_key(&dimension_id));
    let before = restored.shared().published.load_full();
    restored
        .register_identity_dimension(registration)
        .expect("the matching registration rebinds the restored identity owner");
    let after = restored.shared().published.load_full();

    assert_eq!(
        after.version, before.version,
        "identity rebind publishes no lifecycle extension"
    );
    assert_eq!(
        after.dimension_map.p, before.dimension_map.p,
        "identity rebind preserves model width"
    );
    assert!(!restored.pending_restored_identity.lock().unwrap().contains_key(&dimension_id));
    let dimensions = restored.identity_dimensions.read().unwrap();
    let infra = dimensions.get(&dimension_id).expect("restored dimension is registered");
    let snapshot = infra.graph_snapshot.load();
    assert_eq!(
        snapshot
            .as_ref()
            .as_ref()
            .expect("restored graph is published")
            .total_energy(),
        captured_energy
    );
    assert!(infra.load_competitive_set().cell_ids().any(|cell| *cell == captured_cell));
    let state = infra.cell_state.read().unwrap();
    let cell = state
        .get(&captured_cell)
        .expect("restored competitive cell retains state")
        .lock()
        .unwrap();
    assert!(cell.outcome.adverse_rate_ewma > 0.7, "restore decay retains the adverse rate");
    assert!(
        cell.outcome.compressed_valence_ewma > 0.6,
        "restore decay retains compressed valence"
    );
    assert!(cell.outcome.raw_valence_ewma > 0.49, "restore decay retains raw valence");
    assert_eq!(
        cell.measurement.step_count, 0,
        "measurement state is deliberately not durable"
    );
}
...
fn checkpoint_round_trip_via_assayer() {
    ...
    {
        let config = AssayerConfig {
            instance_id: "cp-roundtrip".to_owned(),
            model: ModelConfig::default(),
            infrastructure: InfrastructureConfig {
                label_channel_capacity: 100,
                ..crate::testing::test_infrastructure()
        ...
        let world = World::builder(config)
            .seed(0x6370_2D72_6F75_6E64)
            .build()
            .expect("checkpoint round-trip world builds");
        let assayer = world.assayer();
        for i in 1..=10_u64 {
            assayer.label_tx().send(dummy_label(AssessmentId(i), Some(i))).unwrap();
        }
...
fn whole_state_identity_registration() -> crate::owner::commands::IdentityDimensionRegistration {
    use crate::owner::commands::IdentityDimensionRegistration;
...
fn whole_state_capture(dir: &std::path::Path, root_rate: f64, n_labels: u64) -> (World, u64) {
    let mut config = AssayerConfig {
        instance_id: "test".to_owned(),
        model: ModelConfig::default(),
        infrastructure: InfrastructureConfig {
    ...
    config.instance_id = "whole-state".to_owned();
    let world = World::builder(config)
        .seed(0x7768_6F6C_652D_7374)
        .build()
        .expect("whole-state capture world builds");
    let assayer = world.assayer();
    assayer.register_sentinel(sentinel_reg(SentinelId(1))).unwrap();
    assayer
        .register_identity_dimension(whole_state_identity_registration())
        .unwrap();
    ...
    world
        .flush_identity_maintenance()
        .expect("identity and publication barriers answer for registrations, observations, and labels");
    let v = assayer.shared().published.load().version;
...
fn checkpoint_whole_state_production_path() {
    ...
    let n_labels = 15_u64;
    let (original, live_energy) = whole_state_capture(dir.path(), root_rate, n_labels);
    // Checkpoint out: the file a deployment writes carries the state.
    let payload = crate::testing::durable_checkpoint(&original).expect("read checkpoint");
    ...
    assert_eq!(lp.ledger.root().ewma_bad_rate.to_bits(), root_rate.to_bits());
    ...
    let fork = original
        .fork_persistence_after(Duration::ZERO, |restored| {
            restored
                .assayer()
                .register_identity_dimension(whole_state_identity_registration())
        })
        .expect("isolated whole-state fork rebinds the identity encoder");
    let (uninterrupted, restored) = fork.arms();
    let rebuilt = restored.assayer();
    let lock = rebuilt
        .outcome_ledger
        .get(SentinelId(1))
    ...
    assert_eq!(
        restored_rate.to_bits(),
        root_rate.to_bits(),
        "shared-clock restore preserves the root rate"
    );
    ...
    let suffix = std::time::Instant::now();
    for world in [uninterrupted, restored] {
        let rebuilt = world.assayer();
        rebuilt
            .label_tx()
        ...
        rebuilt
            .flush_label_channel()
            .expect("the publication barrier answers for the post-restore label");
        let summary = rebuilt.health_summary();
        assert_eq!(
            summary.total_labels,
            n_labels + 1,
            "restored counters should continue after restart"
        );
        assert_eq!(summary.eligible_labels, n_labels + 1);
    }
    eprintln!(
        "converted_whole_state fork_wall={:?} suffix_wall={:?} witness_wall={:?}",
        fork.wall(),
        suffix.elapsed(),
        started.elapsed()
    );
...
fn journal_replay_via_assayer_recovery() {
    ...
    let mut working = WorkingCopy::cold_start(
        10,
        &ModelConfig::default(),
        1000,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn checkpoint_round_trip_end_to_end() {
    ...
    let world = {
        ...
        let world = World::builder(config)
            .seed(0x7770_3435_2D63_6170)
            .build()
            .expect("checkpoint capture world should build");
        let assayer = world.assayer();

        // Register + report
        assayer.register_sentinel(sentinel_reg(SentinelId(1))).unwrap();
        let report = crate::testing::golden_report();
        assayer.receive_sentinel_report(SentinelId(1), report).unwrap();

        // Reckon and label 50 with 20% positive
        for i in 0_u64..50 {
            let results = world.derive_for_requests(&[wp45_make_request(ChannelId(0), i)]);
        ...
        let results = world.derive_for_requests(&[wp45_make_request(ChannelId(0), 9999)]);
        let p_bad_before = results.into_iter().next().unwrap().unwrap().assessment.risk.p_bad;
        assert!(p_bad_before.is_finite());

        world
    };
    let fork = world.fork_persistence().expect("isolated end-to-end fork");
    let worlds: [&World; 2] = fork.arms().into();
    let suffix = std::time::Instant::now();
    for world in worlds {
        let assayer = world.assayer();
        ...
        for i in 0_u64..10 {
            let results = world.derive_for_requests(&[wp45_make_request(ChannelId(0), 50_000 + i)]);
            let reckoning = results.into_iter().next().unwrap().unwrap();
    ...
    eprintln!(
        "converted_end_to_end fork_wall={:?} suffix_wall={:?} witness_wall={:?}",
        fork.wall(),
        suffix.elapsed(),
        started.elapsed()
    );
...
fn checkpoint_restores_concordance_tracker() {
    ...
    let started = std::time::Instant::now();
    let world = World::builder(config)
        .seed(0x636F_6E63_6F72_6461)
        .build()
        .expect("concordance capture world should build");
    let assayer = world.assayer();

    for i in 0_u64..5 {
        drop(assayer.assess(&[wp45_make_request(ChannelId(0), i)]));
    }
    let before = assayer.full_health_report().concordance;
    assert_eq!(before.window_size, 5);
    assert_eq!(before.total_observations, 5);
    assert_eq!(before.calibrations_completed, 1);

    let fork = world.fork_persistence().expect("isolated concordance fork");
    let (_, restored) = fork.arms();
    let assayer = restored.assayer();
    ...
    assert_eq!(
        after.current_thresholds.map(f64::to_bits),
        before.current_thresholds.map(f64::to_bits)
    );

    eprintln!(
        "converted_concordance fork_wall={:?} witness_wall={:?}",
        fork.wall(),
        started.elapsed()
    );
}
...
fn shutdown_after_checkpoint_trigger() {
    ...
    let world = World::builder(config)
        .seed(0x7368_7574_646F_776E)
        .build()
        .expect("checkpoint shutdown world should build");

    // Trigger checkpoint
    world.flush_labels().expect("checkpoint should complete");
...
fn label_channel_full_with_journal() {
    ...
    let world = World::builder(config)
        .seed(0x6A6F_7572_6E61_6C2D)
        .build()
        .expect("journal backpressure world should build");
    let assayer = world.assayer();
...
fn concurrent_label_preserves_journal_channel_order() {
    ...
    let world = Arc::new(
        World::builder(config)
            .seed(0x6A6F_7572_6E61_6C6F)
            .build()
            .expect("concurrent journal world should build"),
    );
...
fn recovery_refuses_an_indefinite_checkpoint() {
    ...
    let Err(refusal) = recovery::attempt_restore(
        &cp_path,
        &jl_path,
        &config,
        &[],
...
fn refusal_after_round_trip(
    ...
    let refusal = WorkingCopy::from_checkpoint_payload(
        &read_back,
        100,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn a_model_state_that_disagrees_with_its_own_width_is_refused() {
    ...
    WorkingCopy::from_checkpoint_payload(
        &read_back,
        100,
        0.1,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
    )
    .expect("a state consistent with itself still restores");
...
fn an_outcome_axis_state_that_disagrees_with_its_own_width_is_refused() {
    ...
    .expect("a consistent axis state still restores");
