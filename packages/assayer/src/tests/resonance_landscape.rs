...
//! | [`landscape_shape_seeded_sweep`] | resonance | cites (´claim:resonance:landscape-shape-is-a-function-of-the-declaration-alone´) |
//! | [`landscape_replay_seeded_sweep`] | resonance | cites (´claim:resonance:one-argument-triple-determines-the-landscape-to-the-bit´) |
//! | [`landscape_translation_and_variance_seeded_sweep`] | resonance | cites (´claim:resonance:the-regime-table-is-independent-of-the-risk-basis´) |
...
struct TranslationCase {
    ...
    second_p_bad: f64,
    first_sigma_eff: f64,
    second_sigma_eff: f64,
    first_kappa_eff: f64,
    second_kappa_eff: f64,
    challenge_mean: f64,
    challenge_variance: f64,
}
...
fn policy_with_action_count(action_count: usize) -> ChannelPolicy {
    let actions = match action_count {
        2 => vec![Action::Allow, Action::Block],
        3 => vec![Action::Allow, Action::Challenge, Action::Block],
        _ => vec![Action::Allow, Action::Challenge, Action::Slow, Action::Block],
    };
    ChannelPolicy {
        actions,
...
fn landscapes_match_bits(first: &DecisionLandscape, second: &DecisionLandscape) -> bool {
    first.u.to_bits() == second.u.to_bits()
        && first.sigma_u.to_bits() == second.sigma_u.to_bits()
        && first.beta_sum.to_bits() == second.beta_sum.to_bits()
        && first.posterior.q_c().to_bits() == second.posterior.q_c().to_bits()
        && first.posterior.variance().to_bits() == second.posterior.variance().to_bits()
        && first.crossovers.len() == second.crossovers.len()
        && first.regimes.len() == second.regimes.len()
        && first.crossovers.iter().zip(&second.crossovers).all(|(left, right)| {
            left.from == right.from
                && left.to == right.to
                && left.logit.to_bits() == right.logit.to_bits()
                && left.posture.to_bits() == right.posture.to_bits()
                && left.offset.to_bits() == right.offset.to_bits()
                && left.sensitivity.to_bits() == right.sensitivity.to_bits()
                && left.delta_good.to_bits() == right.delta_good.to_bits()
                && left.delta_bad.to_bits() == right.delta_bad.to_bits()
                && left.variance.to_bits() == right.variance.to_bits()
                && left.interval_logit.0.to_bits() == right.interval_logit.0.to_bits()
                && left.interval_logit.1.to_bits() == right.interval_logit.1.to_bits()
                && left.interval_posture.0.to_bits() == right.interval_posture.0.to_bits()
                && left.interval_posture.1.to_bits() == right.interval_posture.1.to_bits()
        })
        && first.regimes.iter().zip(&second.regimes).all(|(left, right)| {
            left.action == right.action
                && left.width.map(f64::to_bits) == right.width.map(f64::to_bits)
                && left.width_variance.map(f64::to_bits) == right.width_variance.map(f64::to_bits)
                && left.domination_probability.to_bits() == right.domination_probability.to_bits()
        })
}
...
/// Every valid declaration retains its declared crossover and regime shape under drawn evidence.
///
/// The generator draws 459 risk probabilities from `[0.01, 0.99)`, risk deviations from `[0.1, 2.1)`, calibration scales from `[0.5, 2.5)`, valid challenge moments, and each supported declaration width from two through four actions. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:resonance:landscape-shape-is-a-function-of-the-declaration-alone´)
/// ´test:crate:landscape-shape-seeded-sweep´
#[test]
fn landscape_shape_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1A0D_5CA9_5A9E_0459;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| {
            let challenge_mean = rng.next_f64().mul_add(0.9, 0.05);
            LandscapeCase {
                p_bad: rng.next_f64().mul_add(0.98, 0.01),
                sigma_eff: rng.next_f64().mul_add(2.0, 0.1),
                kappa_eff: rng.next_f64().mul_add(2.0, 0.5),
                challenge_mean,
                challenge_variance: challenge_mean * (1.0 - challenge_mean) * rng.next_f64(),
                action_count: usize::try_from(rng.next_range(3) + 2).unwrap_or(2),
            }
        },
        |case| {
            let policy = policy_with_action_count(case.action_count);
            let posterior = ChallengeEstimate::new(case.challenge_mean, case.challenge_variance)
                .map_err(|error| format!("generator produced invalid challenge estimate: {error}"))?;
            let arguments = basis(case.p_bad, case.sigma_eff, case.kappa_eff);
            let first = derive_landscape_from_basis(&arguments, &policy, posterior);
            if first.crossovers.len() != case.action_count - 1 || first.regimes.len() != case.action_count {
                return Err(format!(
                    "declaration of {} actions produced {} crossovers and {} regimes",
                    case.action_count,
                    first.crossovers.len(),
                    first.regimes.len()
                ));
            }
            if first.regimes.first().is_some_and(|regime| regime.width.is_some())
                || first.regimes.last().is_some_and(|regime| regime.width.is_some())
                || first.regimes[1..case.action_count - 1]
                    .iter()
                    .any(|regime| regime.width.is_none())
            {
                return Err("boundary and interior regime widths do not follow the declaration".to_owned());
            }
            Ok(())
        },
    );
...
/// Two invocations from one complete argument triple replay every landscape field bit for bit.
///
/// The generator draws 459 risk probabilities from `[0.01, 0.99)`, risk deviations from `[0.1, 2.1)`, calibration scales from `[0.5, 2.5)`, valid challenge moments, and each supported declaration width from two through four actions. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:guarantee:derivation-purity´)
/// (´inv:guarantee:replay´)
/// (´claim:resonance:one-argument-triple-determines-the-landscape-to-the-bit´)
/// ´test:crate:landscape-replay-seeded-sweep´
#[test]
fn landscape_replay_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1A0D_5CA9_E2A1_0459;
    ...
    run_seeded_sweep(
        ...
        |case| {
            ...
            let arguments = basis(case.p_bad, case.sigma_eff, case.kappa_eff);
            let first = derive_landscape_from_basis(&arguments, &policy, posterior);
            let second = derive_landscape_from_basis(&arguments, &policy, posterior);
            if !landscapes_match_bits(&first, &second) {
                return Err("replayed landscape differs at one or more fields".to_owned());
            }
            Ok(())
        },
    );
...
/// Holding policy and challenge evidence fixed makes every offset and regime width independent of risk, translates all finite crossovers by the common shared-term change, and gives every crossover exactly the sum of risk and challenge variance sources.
///
/// The generator draws 459 pairs of complete non-degenerate risk bases plus one valid challenge moment pair under the four-action declaration. Independent risk probabilities, deviations, and calibration scales exercise translation and both variance terms without changing the policy-owned structure. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:guarantee:rigid-translation´)
/// (´inv:guarantee:two-source-uncertainty´)
/// (´claim:resonance:the-regime-table-is-independent-of-the-risk-basis´)
/// ´test:crate:landscape-translation-and-variance-seeded-sweep´
#[test]
fn landscape_translation_and_variance_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x7A4A_51A7_10A5_0459;
    ...
    run_seeded_sweep(
        ...
        |rng| {
            let challenge_mean = rng.next_f64().mul_add(0.9, 0.05);
            TranslationCase {
                first_p_bad: rng.next_f64().mul_add(0.98, 0.01),
                second_p_bad: rng.next_f64().mul_add(0.98, 0.01),
                first_sigma_eff: rng.next_f64().mul_add(2.0, 0.1),
                second_sigma_eff: rng.next_f64().mul_add(2.0, 0.1),
                first_kappa_eff: rng.next_f64().mul_add(2.0, 0.5),
                second_kappa_eff: rng.next_f64().mul_add(2.0, 0.5),
                challenge_mean,
                challenge_variance: challenge_mean * (1.0 - challenge_mean) * rng.next_f64(),
            }
        },
        |case| {
            ...
            let first = derive_landscape_from_basis(
                &basis(case.first_p_bad, case.first_sigma_eff, case.first_kappa_eff),
                &policy,
                posterior,
            );
            let second = derive_landscape_from_basis(
                &basis(case.second_p_bad, case.second_sigma_eff, case.second_kappa_eff),
                &policy,
                posterior,
            );
            let translation = second.u - first.u;
            for (index, (left, right)) in first.crossovers.iter().zip(&second.crossovers).enumerate() {
                if left.offset.to_bits() != right.offset.to_bits()
                    || left.sensitivity.to_bits() != right.sensitivity.to_bits()
                    || left.delta_good.to_bits() != right.delta_good.to_bits()
                    || left.delta_bad.to_bits() != right.delta_bad.to_bits()
                {
                    return Err(format!("crossover {index} changed a risk-independent field"));
                }
                if left.logit.is_finite() && right.logit.is_finite() {
                    let translation_error = ((right.logit - left.logit) - translation).abs();
                    if translation_error > 1e-12 {
                        return Err(format!("crossover {index} translation error is {translation_error}"));
                    }
                }
                let first_expected = left
                    .sensitivity
                    .mul_add(left.sensitivity * posterior.variance(), first.sigma_u * first.sigma_u);
                let second_expected = right
                    .sensitivity
                    .mul_add(right.sensitivity * posterior.variance(), second.sigma_u * second.sigma_u);
                if (left.variance - first_expected).abs() > 1e-12 || (right.variance - second_expected).abs() > 1e-12 {
                    return Err(format!(
                        "crossover {index} two-source variance mismatch: first={}, expected={first_expected}, second={}, expected={second_expected}",
                        left.variance, right.variance
                    ));
                }
            }
            for (index, (left, right)) in first.regimes.iter().zip(&second.regimes).enumerate() {
                if left.width.map(f64::to_bits) != right.width.map(f64::to_bits)
                    || left.width_variance.map(f64::to_bits) != right.width_variance.map(f64::to_bits)
                {
                    return Err(format!("regime {index} changed a risk-independent width field"));
                }
            }
            Ok(())
        },
...
fn conjugate_counts_cross_the_boundary_whole() {
    ...
    tracker.insert_channel(
        channel,
        ChallengeEffectivenessState::new(1.0, 1.0, 0.9998, PersistentTimestamp::new(1_700_000_000, 0)),
    );
