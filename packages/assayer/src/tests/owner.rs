...
use crate::testing::{ACK_DEADLINE, DEFAULT_TOLERANCES, World, assert_near};
...
/// A shutdown command is enough on its own to retire the owner thread. The owner is serving when the command is sent — it answers a blocking command first, so what follows is a retirement rather than a thread that never started — and from then on nothing else reaches it: no label is ever offered, and every sender is held alive past the join, so neither work arriving nor a channel disconnecting can be what ended the loop. The command is the only cause left. What the test does not assert is how long the retirement took, because the duration was never the claim: a host tearing down needs the thread to leave when told rather than when something else happens to arrive, and the wait the thread leaves from carries no timer at all — which the audit beside this test reads off the loop's own source — so there is no poll interval a shutdown could sit behind and no wall-clock figure that would mean anything if there were. The thread join is the exact completion signal, with no elapsed-time condition added by the test.
///
...
fn shutdown_command_retires_the_owner() {
    // The handshake bound reports a thread that never began serving.
    const HANDSHAKE: Duration = Duration::from_secs(60);

    let config = AssayerConfig {
        instance_id: "shutdown-owner".to_owned(),
    ...
    // Joining is the thread's completion signal; polling its state would add no evidence about why it retired.
...
fn drop_sends_shutdown_and_joins() {
    let world = World::cold("owner", 0x6F77_6E65_722D_776F);
...
fn label_increments_snapshot_version() {
    ...
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers for the first label");
    let v1 = assayer.shared.published.load().version;
    ...
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers for the second label");
    let v2 = assayer.shared.published.load().version;
...
fn label_tracks_sequence_number() {
    ...
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers for the label");
...
fn commands_preempt_labels() {
    ...
    let config = AssayerConfig {
        instance_id: "command-preemption".to_owned(),
        infrastructure: InfrastructureConfig {
...
fn disconnected_channel_after_thread_exit() {
    let config = AssayerConfig {
        instance_id: "disconnected-owner".to_owned(),
        infrastructure: InfrastructureConfig {
            label_channel_capacity: 10,
            ..crate::testing::test_infrastructure()
        },
...
fn queued_labels_are_drained_and_refused_after_the_stop() {
    ...
    // The publication barrier's handler drains every queued label before acknowledging, including labels refused by the planted stop.
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers after draining the refused labels");
...
fn drain_all_commands_before_label() {
    ...
    let config = AssayerConfig {
        instance_id: "drain-all-commands".to_owned(),
        infrastructure: InfrastructureConfig {
            label_channel_capacity: 100,
            ..crate::testing::test_infrastructure()
        },
...
fn shutdown_after_pending_labels() {
    ...
    // The publication barrier completes every label offered before it.
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers for the backlog");
    let final_version = assayer.shared.published.load().version;
...
fn assayer_shutdown_three_threads() {
    ...
    let world = World::builder(config)
        .seed(0x7468_7265_652D_7468)
        .build()
        .expect("three-thread world builds");
    let assayer = world.assayer();
...
fn concurrency_4_readers_1_writer_model_owner() {
    ...
    let config = AssayerConfig {
        instance_id: "concurrent-owner".to_owned(),
        infrastructure: InfrastructureConfig {
            label_channel_capacity: 2000,
            ..crate::testing::test_infrastructure()
        },
        ..Default::default()
    };
    let world = World::builder(config)
        .seed(0x636F_6E63_7572_7265)
        .build()
        .expect("concurrent owner world builds");
    let assayer = world.assayer();
    ...
    let readers: Vec<_> = (0..NUM_READERS)
        .map(|_| {
            ...
            std::thread::spawn(move || {
                ...
                // Retained concurrency workload: repeated snapshot reads are the measurement, and the publication barrier ends the fixed write phase.
    ...
    assayer
        .flush_label_channel()
        .expect("the publication barrier answers for the backlog");
    let final_v = shared.published.load().version;
