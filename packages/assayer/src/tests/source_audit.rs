...
//! | Test | Area | Claim |
//! |------|------|-------|
//! | [`performance_fixtures_do_not_import_private_engine_constructors`] | audit | Performance fixtures construct only the public scenario: the source cannot name the private model, working-copy, shared-state, label-owner, or dimension-map construction paths that the retired stopwatch witnesses assembled beside the harness. |
...
/// Performance fixtures construct only the public scenario: the source cannot name the private model, working-copy, shared-state, label-owner, or dimension-map construction paths that the retired stopwatch witnesses assembled beside the harness.
///
/// ´claim:audit:performance-fixtures-reach-the-real-engine-only-through-the-shared-scenario´
/// ´test:crate:performance-fixtures-do-not-import-private-engine-constructors´
#[test]
fn performance_fixtures_do_not_import_private_engine_constructors() {
    const PROHIBITED: [&str; 7] = [
        "BayesianLinearModel::new",
        "WorkingCopy::",
        "SharedState::new",
        "process_label(",
    ...
    SourceTreeScanner::new().for_each_line(|visit| {
        if !path_has_component(&visit, "testing") || !path_file_name_is(&visit, "performance.rs") {
            return;
        }
        for token in PROHIBITED {
            if visit.line.contains(token) {
                violations.push(format!(
                    "{}:{}: performance fixture names private engine token `{token}`",
                    visit.path.display(),
                    visit.line_no
                ));
            }
    ...
    assert!(
        violations.is_empty(),
        "performance fixtures must use the shared scenario only:\n{}",
        violations.join("\n")
    );
}
...
const CORE_API_DERIVATION_TOKENS: &[&str] = &["derive_landscape", "render_resonances", "derive_resonance", "ResonanceInput"];
...
/// and stays withholding it: the second and nanosecond fields and the system-time conversion are sealed at crate visibility, and the type carries neither `Default` nor a wall-clock constructor. A test cannot show a field is unreadable from outside, so the seal is checked by reading the source — the same arrangement the resonance allowlist uses for a property the visibility system enforces but a merge could quietly relax.
///
...
fn timestamp_fields_and_default_stay_sealed() {
    ...
    assert!(
        !source.contains("pub fn now() -> Self"),
        "the timestamp carries no wall-clock constructor: the injected clock owns the present"
    );
}
