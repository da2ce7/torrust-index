...
//! | [`corrected_marginal_structure_seeded_sweep`] | bayes | cites (´claim:bayes:the-corrected-marginal-never-claims-more-precision-than-the-bare-submatrix-and-usually-claims-less´) |
...
use crate::testing::{DEFAULT_TOLERANCES, TestRng, assert_below, assert_near, regularised_schur_complement, run_seeded_sweep};
...
fn scalar_case_r1() {
    ...
    let expected = regularised_schur_complement(b.as_inner(), &remove, config.delta(FIXTURE_LAMBDA_PRIOR))
        .expect("the regularised removed block has a usable pivot");
    ...
    // Each route performs fewer than eight arithmetic stages on inputs no larger than four; budgeting eight ulps per stage gives this absolute comparison bound.
    let tolerance = 8.0 * 8.0 * f64::EPSILON * 4.0;
    assert_near(bp.as_inner()[(0, 0)], expected[(0, 0)], tolerance, "B'[0,0]");
    assert_near(bp.as_inner()[(0, 1)], expected[(0, 1)], tolerance, "B'[0,1]");
    assert_near(bp.as_inner()[(1, 1)], expected[(1, 1)], tolerance, "B'[1,1]");
}
...
fn rank1_sigma_prime_is_covariance_submatrix() {
    ...
    let tol = DEFAULT_TOLERANCES.exact;
...
/// A regularised corrected marginal agrees with the independent full-solve oracle, preserves the declared kept order, and never exceeds the bare kept block on a surviving diagonal.
///
/// The generator draws 459 complete dense five-by-five factors over `[0.2, 1.2)`, removes either one or two trailing coordinates, and draws a positive prior precision from `[0.1, 2.1)`. The factor product plus an identity gives a well-conditioned positive-definite parent whose non-zero couplings exercise the correction rather than the fallback. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:guarantee:structural-exactness´)
/// (´claim:bayes:the-corrected-marginal-never-claims-more-precision-than-the-bare-submatrix-and-usually-claims-less´)
/// ´test:crate:corrected-marginal-structure-seeded-sweep´
#[test]
fn corrected_marginal_structure_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x5C4A_BA51_5AFE_0459;
    const WIDTH: usize = 5;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| MarginalStructureCase {
            basis: (0..WIDTH * WIDTH).map(|_| rng.next_f64() + 0.2).collect(),
            remove_count: usize::try_from(rng.next_range(2) + 1).unwrap_or(1),
            lambda_prior: rng.next_f64().mul_add(2.0, 0.1),
        },
        |case| {
            let factors = faer::Mat::from_fn(WIDTH, WIDTH, |row, column| case.basis[column * WIDTH + row]);
            let mut parent = factors.transpose() * &factors;
            for index in 0..WIDTH {
                parent[(index, index)] += 1.0;
            }
            let precision = SymmetricMatrix::from_computation(parent);
            let first_removed = WIDTH - case.remove_count;
            let remove: Vec<_> = (first_removed..WIDTH).collect();
            let keep = compute_keep_indices(WIDTH, &remove);
            let config = SchurConfig::default();
            let delta = config.delta(case.lambda_prior);
            let oracle = regularised_schur_complement(precision.as_inner(), &remove, delta)
                .ok_or_else(|| "independent oracle could not solve the generated removed block".to_owned())?;
            let result = compute_schur_complement(&precision, &remove, &keep, &config, case.lambda_prior);
            if result.diagnostics.fell_back_to_bkk {
                return Err(format!(
                    "production correction fell back: {:?}",
                    result.diagnostics.correction_outcome
                ));
            }
            let bare = precision.extract_symmetric_submatrix(&keep);
            let mut one_strict = false;
            for column in 0..keep.len() {
                for row in 0..keep.len() {
                    let actual = result.b_prime.as_inner()[(row, column)];
                    let expected = oracle[(row, column)];
                    let error = (actual - expected).abs();
                    if error > 1e-10 {
                        return Err(format!(
                            "corrected cell ({row}, {column}) differs from independent oracle: actual={actual}, expected={expected}, error={error}"
                        ));
                    }
                }
                let corrected = result.b_prime.diagonal_element(column);
                let uncorrected = bare.diagonal_element(column);
                if corrected > uncorrected + 1e-12 {
                    return Err(format!(
                        "corrected diagonal {column} claims more precision: corrected={corrected}, bare={uncorrected}"
                    ));
                }
                one_strict |= corrected < uncorrected - 1e-12;
            }
            if !one_strict {
                return Err("generated coupling produced no strict diagonal correction".to_owned());
            }
            Ok(())
        },
    );
