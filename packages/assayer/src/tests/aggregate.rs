...
fn empty_sentinels_returns_zeros() {
    ...
    for (i, &v) in features.iter().enumerate() {
        assert_near(v, 0.0, tol.exact, &format!("features[{i}]"));
    }
}
...
fn single_sentinel_stats() {
    ...
    assert_near(features[0], 2.0, tol.exact, "max max_z");
    // Mean = 2.0 (single value)
    assert_near(features[1], 2.0, tol.exact, "mean max_z");
    // Std = 0.0 (single value)
    assert_near(features[2], 0.0, tol.exact, "std max_z");
    // Per-axis max z
    assert_near(features[3], 1.0, tol.exact, "per-axis max N");
    assert_near(features[4], 2.0, tol.exact, "per-axis max D");
    assert_near(features[5], 0.5, tol.exact, "per-axis max S");
    assert_near(features[6], 0.3, tol.exact, "per-axis max C");
    // Cross-axis product
    assert_near(features[11], 2.0, tol.exact, "cross-axis product"); // 1.0 × 2.0
    // Coverage
    assert_near(features[13], 1.0, tol.exact, "coverage"); // 1/1
    // Max CUSUM = 0.4
    assert_near(features[14], 0.4, tol.exact, "max CUSUM");
}
...
fn three_sentinels_golden() {
    ...
    assert_near(features[0], 1.2, tol.exact, "max max_z");

    // Mean of max_z per Sentinel: (0.4 + 0.8 + 1.2) / 3 = 0.8
    assert_near(features[1], 0.8, tol.default, "mean max_z");

    // Per-axis max z
    assert_near(features[3], 0.8, tol.exact, "per-axis max N"); // max(0.3, 0.8, 0.5)
    assert_near(features[4], 1.2, tol.exact, "per-axis max D"); // max(0.4, 0.6, 1.2)
    assert_near(features[5], 0.7, tol.exact, "per-axis max S"); // max(0.2, 0.7, 0.4)
    assert_near(features[6], 0.6, tol.exact, "per-axis max C"); // max(0.1, 0.3, 0.6)

    // Coverage: 3/5 = 0.6
    assert_near(features[13], 0.6, tol.exact, "coverage");
...
fn axis_breadth_counts() {
    ...
    assert_near(features[12], 4.0, tol.exact, "breadth all-above");
    ...
    assert_near(features2[12], 2.0, tol.exact, "breadth partial");
}
...
fn cross_axis_product() {
    ...
    assert_near(features[11], 6.0, tol.exact, "cross-axis product");
}
...
fn zero_registered_coverage() {
    ...
    assert_near(features[13], 0.0, tol.exact, "coverage (0 registered)");
}
