...
use crate::types::{AssessmentId, LedgerKey, PersistentTimestamp, SentinelId};
...
fn pending_assessment(
    ...
    PendingAssessment {
        ...
        entity: World::entity(&format!("entity-{id}")),
...
fn install_starved_ledger(world: &World) {
    let assayer = world.assayer();
    let now = assayer.clock.now();
    let mut ledger = SentinelLedger::new();
    ledger.ensure_root(now);
    ...
    assayer.outcome_ledger.create_sentinel(STARVED_SENTINEL, now);
...
fn request_labels_empty_budget_returns_empty() {
    let mut config = AssayerConfig {
        instance_id: "guidance-empty-budget".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
    config.model.p_plus_init = 0.05;
    let world = World::builder(config)
        .seed(0x6775_6964_616E_6365)
        .build()
        .expect("empty-budget guidance world should build");
    let assayer = world.assayer();
...
fn request_labels_ranks_seeded_pending_snapshot() {
    let mut config = AssayerConfig {
        instance_id: "guidance-ranking".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
    config.model.p_plus_init = 0.05;
    let world = World::builder(config)
        .seed(0x6775_6964_616E_6365)
        .build()
        .expect("ranking guidance world should build");
    let assayer = world.assayer();
    install_starved_ledger(&world);
...
fn request_labels_does_not_deduplicate_across_categories() {
    let mut config = AssayerConfig {
        instance_id: "guidance-category-overlap".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
    config.model.p_plus_init = 0.05;
    let world = World::builder(config)
        .seed(0x6775_6964_616E_6365)
        .build()
        .expect("category-overlap guidance world should build");
    let assayer = world.assayer();
    install_starved_ledger(&world);
    assayer
        .pending_buffer
...
fn request_labels_respects_scan_limit() {
    let mut config = AssayerConfig {
        instance_id: "guidance-scan-limit".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    };
    config.model.p_plus_init = 0.05;
    let world = World::builder(config)
        .seed(0x6775_6964_616E_6365)
        .build()
        .expect("scan-limit guidance world should build");
    let assayer = world.assayer();
    assayer.pending_buffer.insert(pending_assessment(1, 0.9, 0.8, None));
    assayer.pending_buffer.insert(pending_assessment(2, 0.4, 0.2, None));
