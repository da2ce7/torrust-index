...
//! | [`p_matches_model_assertion`] | feature | The vector's total width equals the sum of every block computed independently from the configuration — bias, aggregates, identity blocks, the cross-dimension block, signals, Sentinel slots, interactions and competitive indicators. The blocks are laid end to end with nothing between them, so the width can be predicted from the registries and declared templates without building the map, and no index in the vector belongs to no block. The seeded sweep draws 459 configurations because that many independent draws give more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of the declared population-and-template grid. The grid varies every term in the dimension formula independently, including the five interaction-template routes; the figure is a starting bound over that grid, while a thinner seam needs a derived partition or error bound before increasing it. |
//! | [`layout_covering_and_contiguity_seeded_sweep`] | feature | Every index belongs to exactly one logical block and the ordered blocks meet without a gap or overlap. The generator draws 459 complete layouts over independent signal, Sentinel, identity-dimension, spatial-axis, outcome-axis, competitive-cell, and five interaction-template populations. Zero-width and populated forms of every conditional block occur in the declared grid. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution. (´inv:dimension:covering´) (´inv:dimension:contiguity´) |
...
use crate::feature::dimension_map::{DimensionMap, IndexRange};
...
use crate::testing::{DimensionBlocks, dimension_width, run_seeded_sweep};
...
#[derive(Debug)]
struct LayoutSweepCase {
    signal_count: usize,
    sentinel_count: usize,
    dimension_count: usize,
    spatial_axis_count: usize,
    outcome_axis_count: usize,
    cells_per_dimension: usize,
    type1_template_count: usize,
    type2_template_count: usize,
    type3_template_count: usize,
    type4_template_count: usize,
    type5_template_count: usize,
}

fn draw_layout_case(rng: &mut crate::testing::TestRng) -> LayoutSweepCase {
    LayoutSweepCase {
        signal_count: usize::try_from(rng.next_range(12)).unwrap_or(0),
        sentinel_count: usize::try_from(rng.next_range(9)).unwrap_or(0),
        dimension_count: usize::try_from(rng.next_range(6)).unwrap_or(0),
        spatial_axis_count: usize::try_from(rng.next_range(5)).unwrap_or(0),
        outcome_axis_count: usize::try_from(rng.next_range(5)).unwrap_or(0),
        cells_per_dimension: usize::try_from(rng.next_range(13)).unwrap_or(0),
        type1_template_count: usize::try_from(rng.next_range(4)).unwrap_or(0),
        type2_template_count: usize::try_from(rng.next_range(4)).unwrap_or(0),
        type3_template_count: usize::try_from(rng.next_range(4)).unwrap_or(0),
        type4_template_count: usize::try_from(rng.next_range(4)).unwrap_or(0),
        type5_template_count: usize::try_from(rng.next_range(4)).unwrap_or(0),
    }
}

fn build_layout(case: &LayoutSweepCase) -> DimensionMap {
    let sentinel_ids: Vec<_> = (1..=case.sentinel_count)
        .map(|id| u32::try_from(id).unwrap_or(u32::MAX))
        .collect();
    let dimension_ids: Vec<_> = (1..=case.dimension_count)
        .map(|id| u32::try_from(id).unwrap_or(u32::MAX))
        .collect();
    let sentinel_map = sentinels(&sentinel_ids);
    let dimension_list = dims(&dimension_ids);
    let competitive_cells: IndexMap<_, _> = dimension_list
        .iter()
        .copied()
        .map(|dimension_id| (dimension_id, make_cells(case.cells_per_dimension)))
        .collect();
    let spatial_axes = n_axes(case.spatial_axis_count);
    let outcome_axes = n_axes(case.outcome_axis_count);
    let mut templates = Vec::new();
    templates.extend((0..case.type1_template_count).map(|_| InteractionTemplate::type1(0, FeatureSelector::AggregateFeature(0))));
    templates.extend((0..case.type2_template_count).map(|_| InteractionTemplate::type2(SentinelId(1), SentinelId(2), 0)));
    templates.extend((0..case.type3_template_count).map(|_| InteractionTemplate::type3(0)));
    templates.extend((0..case.type4_template_count).map(|_| InteractionTemplate::type4(0, FeatureSelector::AggregateFeature(0))));
    templates.extend((0..case.type5_template_count).map(|_| InteractionTemplate::type5(FeatureSelector::AggregateFeature(0))));
    DimensionMap::rebuild_with_axes(
        case.signal_count,
        &sentinel_map,
        &dimension_list,
        &competitive_cells,
...
fn rebuild_eight_sentinels_m1() {
    ...
    let expected = dimension_width(DimensionBlocks {
        identity_dimensions: dim_list.len(),
        outcome_axes: 0,
        spatial_axes: 1,
        signals: 0,
        sentinels: sents.len(),
        interactions: 0,
        competitive_indicators: cells_a.len() + cells_b.len(),
    });
...
fn rebuild_reference_config_p638() {
    ...
    let expected = dimension_width(DimensionBlocks {
        identity_dimensions: dim_list.len(),
        outcome_axes: axis_ids.len(),
        spatial_axes: 1,
        signals: 0,
        sentinels: sents.len(),
        interactions: 5 * sents.len() + 8 + cells_a.len() + cells_b.len(),
        competitive_indicators: cells_a.len() + cells_b.len(),
    });
    assert_eq!(expected, 638);
    assert_eq!(dm.p, expected);
}
...
/// The vector's total width equals the sum of every block computed independently from the configuration — bias, aggregates, identity blocks, the cross-dimension block, signals, Sentinel slots, interactions and competitive indicators. The blocks are laid end to end with nothing between them, so the width can be predicted from the registries and declared templates without building the map, and no index in the vector belongs to no block.
///
/// The seeded sweep draws 459 configurations because that many independent draws give more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of the declared population-and-template grid. The grid varies every term in the dimension formula independently, including the five interaction-template routes; the figure is a starting bound over that grid, while a thinner seam needs a derived partition or error bound before increasing it.
///
/// ´claim:feature:the-vectors-width-is-exactly-the-sum-of-its-blocks-with-nothing-between-them´
...
fn p_matches_model_assertion() {
    ...
    const SEED: u64 = 0xA55E_4B1E_5A55_0001;
    ...
    run_seeded_sweep(
        ...
        |case| {
            ...
            let mut templates = Vec::new();
            templates.extend(
                (0..case.type1_template_count).map(|_| InteractionTemplate::type1(0, FeatureSelector::AggregateFeature(0))),
            );
            templates.extend((0..case.type2_template_count).map(|_| InteractionTemplate::type2(SentinelId(1), SentinelId(2), 0)));
            templates.extend((0..case.type3_template_count).map(|_| InteractionTemplate::type3(0)));
            templates.extend(
                (0..case.type4_template_count).map(|_| InteractionTemplate::type4(0, FeatureSelector::AggregateFeature(0))),
            );
            templates
                .extend((0..case.type5_template_count).map(|_| InteractionTemplate::type5(FeatureSelector::AggregateFeature(0))));
            let map = DimensionMap::rebuild_with_axes(
                case.signal_count,
                &sentinel_map,
                &dimension_list,
                &competitive_cells,
            ...
            let competitive_width = case.dimension_count * case.cells_per_dimension;
            let named_pair_width = case.type2_template_count * usize::from(case.sentinel_count >= 2);
            let wildcard_pair_width = case.type3_template_count * case.sentinel_count.saturating_sub(1) * case.sentinel_count / 2;
            let interaction_width = case.type1_template_count * case.sentinel_count
                + named_pair_width
                + wildcard_pair_width
                + case.type4_template_count
                + case.type5_template_count * competitive_width;
            let expected = dimension_width(DimensionBlocks {
                identity_dimensions: case.dimension_count,
                outcome_axes: case.outcome_axis_count,
                spatial_axes: case.spatial_axis_count,
                signals: case.signal_count,
                sentinels: case.sentinel_count,
                interactions: interaction_width,
                competitive_indicators: competitive_width,
            });

            if map.p == expected {
                Ok(())
            } else {
                Err(format!("routed width {}, oracle block sum {expected}", map.p))
            }
        },
    );
}

/// Every index belongs to exactly one logical block and the ordered blocks meet without a gap or overlap.
///
/// The generator draws 459 complete layouts over independent signal, Sentinel, identity-dimension, spatial-axis, outcome-axis, competitive-cell, and five interaction-template populations. Zero-width and populated forms of every conditional block occur in the declared grid. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:dimension:covering´)
/// (´inv:dimension:contiguity´)
/// ´claim:feature:every-layout-index-belongs-to-exactly-one-contiguous-block´
/// ´test:crate:layout-covering-and-contiguity-seeded-sweep´
#[test]
fn layout_covering_and_contiguity_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0xC0A3_21A0_0A57_0459;

    run_seeded_sweep(SEED, CASES, draw_layout_case, |case| {
        let map = build_layout(case);
        let mut ranges = Vec::new();
        ranges.push(IndexRange::new(map.bias_idx, map.bias_idx + 1));
        ranges.push(map.agg_range.clone());
        ranges.extend(map.id_dim_ranges.values().cloned());
        ranges.extend(map.id_cross_dim_range.iter().cloned());
        ranges.push(map.sig_range.clone());
        ranges.extend(map.sentinel_slots.values().cloned());
        ranges.push(map.interaction_range.clone());
        ranges.push(map.competitive_range.clone());

        for (position, pair) in ranges.windows(2).enumerate() {
            if pair[0].end != pair[1].start {
                return Err(format!(
                    "block boundary {position} does not meet: left={:?}, right={:?}",
                    pair[0], pair[1]
                ));
            }
        }
        if ranges.first().is_none_or(|range| range.start != 0) || ranges.last().is_none_or(|range| range.end != map.p) {
            return Err(format!("block envelope does not cover 0..{}: ranges={ranges:?}", map.p));
        }

        let mut owners = vec![0_u8; map.p];
        for range in &ranges {
            for index in range.to_range() {
                let Some(count) = owners.get_mut(index) else {
                    return Err(format!("block {range:?} routes index {index} outside width {}", map.p));
                };
                *count = count.saturating_add(1);
            }
        }
        if let Some((index, count)) = owners.iter().enumerate().find(|(_, count)| **count != 1) {
            return Err(format!("index {index} has {count} owning blocks, expected one"));
        }
        Ok(())
    });
}
