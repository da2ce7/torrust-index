...
//! | [`pending_entry_view_is_non_consuming_and_keeps_storage_width`] | pending | A pending probe completes while the unrelated model owner is parked, can read the same entry twice without consuming it, preserves both configured feature-storage widths, and reports only active-cell counts rather than exposing the stored cell vectors. The synchronous map lookup therefore acquires no invented queue dependency and widens none of the retained request input. |
...
use crate::pending::{PendingAssessment, PendingBuffer, PendingRiskBasis, SentinelExtraction, StoredFeatures};
use crate::testing::dimensions::make_cells;
use crate::testing::{World, assert_below};
...
fn create_pending_assessment_at_time(id: AssessmentId, timestamp: Instant) -> PendingAssessment {
    ...
    PendingAssessment {
        ...
        id,
        timestamp,
        persistent_timestamp: PersistentTimestamp::new(1_700_000_000, 0),
...
fn lazy_eviction_50ms_expiry() {
    ...
    // Retained measurement delay: expiry is the subject, and this buffer reads the real monotonic clock rather than an asynchronous queue.
...
fn eviction_capped_at_16_per_insert() {
    ...
    // Retained measurement delay: the test must age entries past the real-clock expiry horizon before measuring bounded eviction work.
...
/// A pending probe completes while the unrelated model owner is parked, can read the same entry twice without consuming it, preserves both configured feature-storage widths, and reports only active-cell counts rather than exposing the stored cell vectors. The synchronous map lookup therefore acquires no invented queue dependency and widens none of the retained request input.
///
/// ´claim:pending:a-pending-probe-is-one-non-consuming-precision-preserving-request-view´
/// ´test:crate:pending-entry-view-is-non-consuming-and-keeps-storage-width´
#[test]
fn pending_entry_view_is_non_consuming_and_keeps_storage_width() {
    let world = World::cold("pending-probe", 0x5045_4E44_494E_475F);
    let single_id = AssessmentId(7_001);
    let double_id = AssessmentId(7_002);
    let single = create_pending_assessment(single_id);
    let mut double = create_pending_assessment(double_id);
    double.signal_features = StoredFeatures::Double(vec![0.125, 0.25, 0.5]);
    world.assayer().pending_buffer.insert(single);
    world.assayer().pending_buffer.insert(double);
    let owner_block = world.block_model_owner_for_test();

    let first = world
        .pending_entry_view(single_id)
        .expect("the single-precision entry is present");
    let again = world
        .pending_entry_view(single_id)
        .expect("the first probe does not consume the entry");
    let wide = world
        .pending_entry_view(double_id)
        .expect("the double-precision entry is present");

    assert_eq!(first, again, "two non-consuming views of unchanged pending state agree");
    assert_eq!(first.single_precision_signals(), Some(&[0.5_f32, 1.0, -0.3][..]));
    assert!(first.double_precision_signals().is_none(), "single storage is not widened");
    assert_eq!(wide.double_precision_signals(), Some(&[0.125_f64, 0.25, 0.5][..]));
    assert!(wide.single_precision_signals().is_none(), "double storage is not narrowed");
    assert_eq!(first.active_cell_counts.get(&DimensionId(1)), Some(&1));

    owner_block.release();
    assert!(
        world.assayer().pending_buffer.remove(single_id).is_some(),
        "probing leaves the entry available to its consumer"
    );
    assert!(
        world.assayer().pending_buffer.remove(double_id).is_some(),
        "probing leaves the second entry available too"
    );
}
