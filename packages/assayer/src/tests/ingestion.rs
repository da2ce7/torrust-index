...
use crate::types::{LedgerKey, PersistentTimestamp, SentinelId};
...
fn quality_nan_max_z() {
    ...
    assert_eq!(
        entry.scores.novelty.max_z.to_bits(),
        0.0f64.to_bits(),
        "novelty axis should be zeroed"
    );
    assert_eq!(entry.scores.novelty.cusum.to_bits(), 0.0f64.to_bits());
...
fn quality_inf_cusum() {
    ...
    assert_eq!(
        entry.scores.surprise.max_z.to_bits(),
        0.0f64.to_bits(),
        "surprise axis should be zeroed"
    );
    assert_eq!(entry.scores.surprise.cusum.to_bits(), 0.0f64.to_bits());
}
...
fn quality_noise_influence_above_1() {
    ...
    assert_eq!(entry.noise_influence.to_bits(), 1.0f64.to_bits(), "should clamp to 1.0");
}
...
fn quality_noise_influence_negative() {
    ...
    assert_eq!(entry.noise_influence.to_bits(), 0.0f64.to_bits(), "should clamp to 0.0");
}
...
fn quality_multiple_bad_axes() {
    ...
    assert_eq!(entry.scores.novelty.max_z.to_bits(), 0.0f64.to_bits(), "novelty zeroed");
    assert_eq!(
        entry.scores.displacement.max_z.to_bits(),
        0.0f64.to_bits(),
        "displacement zeroed"
    );
...
fn setup_ingestion() -> (DashMap<SentinelId, SentinelSlot>, OutcomeLedger, IngestionConfig) {
    ...
    outcome_ledger.create_sentinel(sentinel_id, PersistentTimestamp::new(1_700_000_000, 0));
...
fn ingest_concurrent_different_sentinels() {
    ...
    for i in 1..=4 {
        ...
        ledger.create_sentinel(id, PersistentTimestamp::new(1_700_000_000, 0));
    }
