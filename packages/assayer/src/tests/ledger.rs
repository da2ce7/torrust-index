...
//! | [`neutral_ledger_state_takes_travelled_scenario_time`] | ledger | A neutral Ledger root created after scenario travel carries the travelled persistent reading exactly. The harness already moves both typed domains and crosses the required barriers (´entry:assayer:harness-scenario-time´); this witness covers the production constructor path that receives that reading when a later lifecycle event creates state. |
...
//! | [`ledger_gc_barrier_waits_for_cycle_completion`] | ledger | The Ledger-GC barrier returns the scheduler's own completed-cycle outcome: over two Sentinels — one carrying an entry both stale and empty, the other a recent one — the acknowledged cycle removes the eligible entry and leaves the recent entry and both roots standing. The assertion reads state immediately after the acknowledgement, with no poll or elapsed-time inference between the trigger and the observation. |
...
//! | [`ledger_bad_rate_seeded_sweep`] | ledger | cites (´claim:ledger:the-bad-rate-stays-within-the-unit-interval-under-any-history´) |
//! | [`ledger_compressed_valence_seeded_sweep`] | ledger | cites (´claim:ledger:the-compressed-valence-ewma-stays-inside-the-range-of-the-values-it-averages´) |
//! | [`ledger_decayed_read_seeded_sweep`] | ledger | cites (´claim:ledger:a-decayed-reading-is-always-positive-and-never-exceeds-the-stored-value´) |
//! | [`ledger_read_purity_seeded_sweep`] | ledger | cites (´claim:ledger:reading-a-decayed-view-does-not-mutate-the-stored-entry´) |
//! | [`ledger_root_routing_seeded_sweep`] | ledger | cites (´claim:ledger:the-root-receives-every-write-exactly-once-whatever-the-coordinate´) |
...
use crate::testing::{
    Clock, DEFAULT_TOLERANCES, TestRng, Tolerances, VirtualClock, World, assert_bits_equal, assert_near, run_seeded_sweep,
};
...
#[derive(Debug)]
struct LedgerTransitionCase {
    stored_bad_rate: f64,
    stored_compressed_valence: f64,
    stored_raw_valence: f64,
    hourly_rate: f64,
    retention: f64,
    elapsed_hours: i64,
    is_positive: bool,
    observed_compressed_valence: f64,
    observed_raw_valence: f64,
}

#[derive(Debug)]
struct LedgerRoutingCase {
    coordinate: u128,
    depths: Vec<u8>,
    hourly_rate: f64,
    retention: f64,
...
const SWEEP_NOW_SECONDS: i64 = 10_000_000;

fn draw_ledger_transition_case(rng: &mut TestRng) -> LedgerTransitionCase {
    LedgerTransitionCase {
        stored_bad_rate: rng.next_f64().mul_add(1.0 - f64::EPSILON, f64::EPSILON),
        stored_compressed_valence: rng.next_f64().mul_add(2.0, -1.0),
        stored_raw_valence: rng.next_f64().mul_add(10.0, -5.0),
        hourly_rate: rng.next_f64().mul_add(0.0099, 0.99),
        retention: rng.next_f64().mul_add(0.49, 0.5),
        elapsed_hours: i64::try_from(rng.next_range(1_001)).unwrap_or(0),
        is_positive: rng.coin(),
        observed_compressed_valence: rng.next_f64().mul_add(2.0, -1.0),
        observed_raw_valence: rng.next_f64().mul_add(10.0, -5.0),
    }
}

fn ledger_entry_from_case(case: &LedgerTransitionCase, last_updated: PersistentTimestamp) -> LedgerEntry {
    let mut entry = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    entry.ewma_bad_rate = case.stored_bad_rate;
    entry.compressed_valence_ewma = case.stored_compressed_valence;
    entry.raw_valence_ewma = case.stored_raw_valence;
    entry.last_updated = last_updated;
    entry
}

fn ledger_update_from_case(case: &LedgerTransitionCase) -> LedgerUpdate {
    LedgerUpdate::new(
        case.is_positive,
        case.observed_compressed_valence,
        case.observed_raw_valence,
        Action::Allow,
        true,
    )
}

fn ledger_entries_match(first: &LedgerEntry, second: &LedgerEntry) -> bool {
    first.total_assessments == second.total_assessments
        && first.per_action_count == second.per_action_count
        && first.ewma_bad_rate.to_bits() == second.ewma_bad_rate.to_bits()
        && first.compressed_valence_ewma.to_bits() == second.compressed_valence_ewma.to_bits()
        && first.raw_valence_ewma.to_bits() == second.raw_valence_ewma.to_bits()
        && first.per_axis == second.per_axis
        && first.last_updated == second.last_updated
        && first.recent_eligible_count == second.recent_eligible_count
        && first.adverse_eligible_count == second.adverse_eligible_count
        && first.eligible_arrival_load.to_bits() == second.eligible_arrival_load.to_bits()
        && first.consecutive_absent == second.consecutive_absent
}
...
fn new_neutral_zero_ewmas() {
    let entry = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    assert_near(entry.ewma_bad_rate, 0.0, TOL.exact, "ewma_bad_rate");
    assert_near(entry.compressed_valence_ewma, 0.0, TOL.exact, "compressed_valence_ewma");
    assert_near(entry.raw_valence_ewma, 0.0, TOL.exact, "raw_valence_ewma");
...
fn new_neutral_timestamp_approx_now() {
    let before = PersistentTimestamp::new(1_700_000_000, 0);
    let entry = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    let after = PersistentTimestamp::new(1_700_000_000, 0);

    assert!(entry.last_updated >= before);
    assert!(entry.last_updated <= after);
}

/// A neutral Ledger root created after scenario travel carries the travelled
/// persistent reading exactly. The harness already moves both typed domains
/// and crosses the required barriers (´entry:assayer:harness-scenario-time´);
/// this witness covers the production constructor path that receives that
/// reading when a later lifecycle event creates state.
///
/// ´claim:ledger:a-neutral-entry-created-after-scenario-travel-is-stamped-with-the-travelled-persistent-present´
/// ´test:crate:neutral-ledger-state-takes-travelled-scenario-time´
#[test]
fn neutral_ledger_state_takes_travelled_scenario_time() {
    let mut world = World::cold("ledger-stage-clock", 0x7374_6167_652D_636C);
    let target = UNIX_EPOCH + Duration::from_secs(VirtualClock::EPOCH_SECS + 100 * 365 * 24 * 3600);
    world.travel_to(target).expect("scenario travel settles its barriers");
    let travelled = world.clock().now();

    let sentinel = world
        .register_sentinel("after-travel")
        .expect("Sentinel registration succeeds");
    let ledger = world
        .assayer()
        .outcome_ledger
        .get(sentinel)
        .expect("registration creates a Ledger");
    let root_timestamp = ledger.read().expect("Ledger read lock").root().last_updated;

    assert_eq!(root_timestamp, travelled, "neutral state takes the scenario reading");
    assert_eq!(root_timestamp, PersistentTimestamp::from_system_time(target));
}
...
fn make_ledger_with_depths(depths: &[u8]) -> SentinelLedger {
    ...
    for &depth in depths {
        ...
        ledger.insert(key, LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)));
    }
...
fn all_layers_write_ewma_formula() {
    ...
    // Initial state
    assert_near(ledger.root().ewma_bad_rate, 0.0, TOL.exact, "initial bad_rate");
...
fn read_decay_pure() {
    ...
    assert_bits_equal(view1.bad_rate, view2.bad_rate, "read_decayed is pure");
    assert_bits_equal(entry.ewma_bad_rate, original_rate, "entry must not mutate");
}
...
fn read_decay_factor() {
    ...
    let now = PersistentTimestamp::new(1_700_000_000, 0);
...
fn cell_set_appearances() {
    ...
    let changes = apply_cell_set_changes(&mut ledger, &new_cells, 3, PersistentTimestamp::new(1_700_000_000, 0));
...
fn cell_set_absence_increment() {
    ...
    let mut ledger = SentinelLedger::new();
    ledger.ensure_root(PersistentTimestamp::new(1_700_000_000, 0));
    ledger.insert(key4, LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)));

    let empty: HashSet<LedgerKey> = HashSet::new();
    apply_cell_set_changes(&mut ledger, &empty, 3, PersistentTimestamp::new(1_700_000_000, 0));
...
fn cell_set_deletion() {
    ...
    let changes = apply_cell_set_changes(&mut ledger, &empty, 3, PersistentTimestamp::new(1_700_000_000, 0));
...
fn cell_set_root_never_deleted() {
    ...
    for _ in 0..10 {
        apply_cell_set_changes(&mut ledger, &empty, 0, PersistentTimestamp::new(1_700_000_000, 0));
    }
...
fn cell_set_no_merge() {
    ...
    ledger.ensure_root(PersistentTimestamp::new(1_700_000_000, 0));
    ledger.insert(key4, LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)));
    ledger.insert(key8, LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)));
    ...
    for _ in 0..3 {
        apply_cell_set_changes(&mut ledger, &report_with_key4, 3, PersistentTimestamp::new(1_700_000_000, 0));
    }
...
fn gc_batched_deletion_bounds_lock_windows() {
...

    {
        ...
        guard.ensure_root(PersistentTimestamp::new(1_700_000_000, 0));
...
fn gc_batched_survives_concurrent_rehash() {
    ...
    let writer = {
        ...
        thread::spawn(move || {
            let mut idx = 0u128;
            // Retained concurrency workload: repeated writes are the rehash stimulus, and the fixed collection call supplies completion.
            while !stop.load(Ordering::Relaxed) {
                ...
                guard.insert(
                    make_key(idx + 1, 64),
                    LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)),
                );
                idx += 1;
                drop(guard);
                // Retained concurrency pacing: this writer awaits no condition; yielding widens the rehash interleavings exercised by the sweep.
                thread::yield_now();
            }
        })
    };
...
/// The Ledger-GC barrier returns the scheduler's own completed-cycle outcome:
/// over two Sentinels — one carrying an entry both stale and empty, the other
/// a recent one — the acknowledged cycle removes the eligible entry and leaves
/// the recent entry and both roots standing. The assertion reads state
/// immediately after the acknowledgement, with no poll or elapsed-time
/// inference between the trigger and the observation.
///
/// ´claim:ledger:the-ledger-gc-barrier-answers-only-after-its-triggered-cycle-sweeps-every-visible-sentinel´
/// ´test:crate:ledger-gc-barrier-waits-for-cycle-completion´
#[test]
fn ledger_gc_barrier_waits_for_cycle_completion() {
    // Present: 100 days after the persistent epoch; horizon: 60 days.
    let clock = Arc::new(VirtualClock::at_secs(100 * 24 * 3600));
    let world = World::builder(crate::AssayerConfig {
        instance_id: "ledger-gc-barrier".to_owned(),
        infrastructure: crate::testing::test_infrastructure(),
        ..Default::default()
    })
    .clock(Arc::clone(&clock))
    .seed(0x6C65_6467_6572_6763)
    .build()
    .expect("Ledger-GC barrier world builds");
    let container = &world.assayer().outcome_ledger;

    let stale_sentinel = SentinelId(1);
    let fresh_sentinel = SentinelId(2);
    container.create_sentinel(stale_sentinel, PersistentTimestamp::new(1_700_000_000, 0));
    container.create_sentinel(fresh_sentinel, PersistentTimestamp::new(1_700_000_000, 0));
    ...
    let outcome = world.flush_ledger_gc().expect("Ledger-GC cycle acknowledges");
    assert_eq!(outcome.removed, 1, "the completed cycle reports its one removal");
...
fn outcome_ledger_create_sentinel() {
    ...
    ledger.create_sentinel(SentinelId(1), PersistentTimestamp::new(1_700_000_000, 0));
...
fn entry_serde_with_axes() {
    ...
    assert_bits_equal(restored.ewma_bad_rate, 0.123, "bad_rate roundtrip");

    let (c1, r1) = axis_val(&restored.per_axis, OutcomeAxisId(1));
    let (c2, _r2) = axis_val(&restored.per_axis, OutcomeAxisId(2));
    assert_bits_equal(c1, 0.5, "axis 1 compressed");
    assert_bits_equal(c2, 0.7, "axis 2 compressed");
    assert_bits_equal(r1, 1.5, "axis 1 raw");
}
...
fn read_decay_zero_elapsed() {
    ...
    assert_bits_equal(view.bad_rate, 0.75, "bad_rate unchanged");
    assert_bits_equal(view.compressed_valence, 0.5, "compressed unchanged");
    assert_bits_equal(view.raw_valence, 2.0, "raw unchanged");
}
...
fn read_decay_backward_clock() {
    ...
    assert_bits_equal(view.bad_rate, 0.5, "backward clock does not decay");
}
...
fn write_single_negative() {
    ...
    assert_near(entry.ewma_bad_rate, 0.0, TOL.exact, "single negative → 0");
}
...
fn write_raw_valence() {
    ...
    assert_near(entry.raw_valence_ewma, (1.0 - lambda) * 5.0, TOL.exact, "raw_valence formula");
}
...
fn write_per_axis_absent() {
    let now = clock().now();
    let mut entry = LedgerEntry::new_neutral(now);
    let lambda = 0.999;
...
fn appear_already_exists() {
    ...
    let changes = apply_cell_set_changes(&mut ledger, &report, 3, PersistentTimestamp::new(1_700_000_000, 0));
...
fn creation_no_inheritance() {
    ...
    let key4 = make_key(0, 4);
    let report: HashSet<LedgerKey> = vec![key4].into_iter().collect();
    apply_cell_set_changes(&mut ledger, &report, 3, PersistentTimestamp::new(1_700_000_000, 0));

    // New entry should be neutral, NOT inherit from root
    let new_entry = ledger.get(&key4).expect("should exist");
    assert_near(new_entry.ewma_bad_rate, 0.0, TOL.exact, "new entry does not inherit bad_rate");
    assert_near(
...
fn multiple_appearances() {
    ...
    let changes = apply_cell_set_changes(&mut ledger, &keys, 3, PersistentTimestamp::new(1_700_000_000, 0));
...
fn multiple_deletions() {
    ...
    let mut entry4 = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
    entry4.consecutive_absent = 2;
    let mut entry8 = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
...
fn multiple_sentinels() {
    let ledger = OutcomeLedger::new();
    ledger.create_sentinel(SentinelId(1), PersistentTimestamp::new(1_700_000_000, 0));
    ledger.create_sentinel(SentinelId(2), PersistentTimestamp::new(1_700_000_000, 0));
    ledger.create_sentinel(SentinelId(3), PersistentTimestamp::new(1_700_000_000, 0));
...
/// One independently drawn transition keeps the bad-rate EWMA inside the interval bounded by its prior value and the new binary outcome.
///
/// The generator draws 459 complete relevant entry states, valid hourly and observation-retention rates, elapsed intervals through a thousand hours, binary outcomes, bounded compressed valences, and raw valences from `[−5, 5)`. Each case starts from its own state, so a prior prefix cannot hide a bad transition. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:ledger:the-bad-rate-stays-within-the-unit-interval-under-any-history´)
/// ´test:crate:ledger-bad-rate-seeded-sweep´
#[test]
fn ledger_bad_rate_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1ED6_BAD0_5AFE_0459;

    run_seeded_sweep(SEED, CASES, draw_ledger_transition_case, |case| {
        let now = PersistentTimestamp::new(SWEEP_NOW_SECONDS, 0);
        let mut entry = ledger_entry_from_case(case, now);
        let update = ledger_update_from_case(case);
        entry.apply_write_decay_and_update(case.hourly_rate, &now, &update, case.retention);
        let observation = if case.is_positive { 1.0 } else { 0.0 };
        let lower = case.stored_bad_rate.min(observation);
        let upper = case.stored_bad_rate.max(observation);
        if entry.ewma_bad_rate < lower || entry.ewma_bad_rate > upper {
            return Err(format!(
                "bad-rate transition {} left input hull [{lower}, {upper}]",
                entry.ewma_bad_rate
            ));
        }
        Ok(())
...
/// One independently drawn transition keeps the compressed-valence EWMA inside the interval bounded by its prior and observed values.
///
/// The generator draws 459 complete relevant entry states, valid hourly and observation-retention rates, elapsed intervals through a thousand hours, binary outcomes, bounded compressed valences, and raw valences from `[−5, 5)`. Each case starts from its own state, so a prior prefix cannot hide a bad transition. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:ledger:the-compressed-valence-ewma-stays-inside-the-range-of-the-values-it-averages´)
/// ´test:crate:ledger-compressed-valence-seeded-sweep´
#[test]
fn ledger_compressed_valence_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1ED6_EA1E_5AFE_0459;
    ...
    run_seeded_sweep(SEED, CASES, draw_ledger_transition_case, |case| {
        ...
        let update = ledger_update_from_case(case);
        entry.apply_write_decay_and_update(case.hourly_rate, &now, &update, case.retention);
        let lower = case.stored_compressed_valence.min(case.observed_compressed_valence);
        let upper = case.stored_compressed_valence.max(case.observed_compressed_valence);
        if entry.compressed_valence_ewma < lower || entry.compressed_valence_ewma > upper {
            return Err(format!(
                "compressed-valence transition {} left input hull [{lower}, {upper}]",
                entry.compressed_valence_ewma
            ));
        }
        Ok(())
...
/// A decayed reading of a positive stored value remains positive and never exceeds that stored value.
///
/// The generator draws 459 positive stored entries, valid hourly rates, and elapsed intervals through a thousand hours. Each read begins from an independently drawn timestamp and state. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:guarantee:ledger-floor´)
/// (´claim:ledger:a-decayed-reading-is-always-positive-and-never-exceeds-the-stored-value´)
/// ´test:crate:ledger-decayed-read-seeded-sweep´
#[test]
fn ledger_decayed_read_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1ED6_DEC4_5AFE_0459;

    run_seeded_sweep(SEED, CASES, draw_ledger_transition_case, |case| {
        let now = PersistentTimestamp::new(SWEEP_NOW_SECONDS, 0);
        let then = PersistentTimestamp::new(SWEEP_NOW_SECONDS - case.elapsed_hours * 3_600, 0);
        let entry = ledger_entry_from_case(case, then);
        let view = entry.read_decayed(case.hourly_rate, &now);
        if view.bad_rate <= 0.0 || view.bad_rate > entry.ewma_bad_rate {
            return Err(format!(
                "decayed bad rate {} is outside (0, {}]",
                view.bad_rate, entry.ewma_bad_rate
            ));
        }
        Ok(())
    });
...
/// Reading a decayed view leaves every stored field bit-identical.
///
/// The generator draws 459 complete relevant entry states, valid hourly rates, and elapsed intervals through a thousand hours. The before-and-after comparison includes every persisted scalar, counter, axis collection, and timestamp. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:ledger:reading-a-decayed-view-does-not-mutate-the-stored-entry´)
/// ´test:crate:ledger-read-purity-seeded-sweep´
#[test]
fn ledger_read_purity_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1ED6_AEAD_5AFE_0459;
    ...
    run_seeded_sweep(SEED, CASES, draw_ledger_transition_case, |case| {
        ...
        let view = entry.read_decayed(case.hourly_rate, &now);
        if !view.bad_rate.is_finite() {
            return Err(format!("decayed view is not finite: {}", view.bad_rate));
        }
        if !ledger_entries_match(&entry, &stored) {
            return Err("read-decay mutated the stored entry".to_owned());
        }
        Ok(())
    });
}

/// Every routed write reaches the permanent root exactly once across arbitrary coordinates and sparse tracked ancestor sets.
///
/// The generator draws 459 full 128-bit coordinates, a sparse subset of the five nibble-aligned depths through twenty, valid hourly and observation-retention rates, and either outcome. Each ledger is built afresh with the drawn containing cells plus its root. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:ledger:the-root-receives-every-write-exactly-once-whatever-the-coordinate´)
/// ´test:crate:ledger-root-routing-seeded-sweep´
#[test]
fn ledger_root_routing_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x1ED6_E2A1_2007_0459;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| LedgerRoutingCase {
            coordinate: (u128::from(rng.next_u64()) << 64) | u128::from(rng.next_u64()),
            depths: [4_u8, 8, 12, 16, 20].into_iter().filter(|_| rng.coin()).collect(),
            hourly_rate: rng.next_f64().mul_add(0.0099, 0.99),
            retention: rng.next_f64().mul_add(0.49, 0.5),
            is_positive: rng.coin(),
        },
        |case| {
            ...
            for &depth in &case.depths {
                ledger.insert(
                    LedgerKey::from_coordinate(case.coordinate, depth),
                    LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0)),
                );
            }
            let before = ledger.root().total_assessments;
            let update = LedgerUpdate::new(case.is_positive, 0.25, 0.5, Action::Challenge, true);
            let now = PersistentTimestamp::new(10_000_000, 0);
            update_all_layers(&mut ledger, case.coordinate, case.hourly_rate, &now, &update, case.retention);
            let after = ledger.root().total_assessments;
            if after != before + 1 {
                return Err(format!(
                    "root tally moved from {before} to {after}, expected exactly one write"
                ));
            }
            Ok(())
        },
...
fn ledger_decay_twenty_nine_day_half_life_timeline() {
    ...
    let t0 = clock().now();
    let mut peak_entry = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
...
fn label_assess_race_no_torn_reads() {
    ...
    fn seed(sentinel: SentinelId, leaf_key: LedgerKey, t0: PersistentTimestamp) -> OutcomeLedger {
        ...
        let mut leaf = LedgerEntry::new_neutral(PersistentTimestamp::new(1_700_000_000, 0));
