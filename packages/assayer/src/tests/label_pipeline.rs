...
use crate::testing::{DEFAULT_TOLERANCES, LabelSpec, assert_bits_equal, assert_finite, assert_near};
...
fn step0_last_label_time_updated() {
    ...
    // Retained measurement delay: the assertion compares two real-clock instants, so the stimulus is elapsed time rather than queue completion.
...
fn step5_kappa_v_skip_zero_valence() {
    ...
    assert_bits_equal(env.working.kappa_v, initial_kappa, "κ_v unchanged on zero valence");
}
...
fn learning_policy_conforms_row_by_row() {
    for row in LEARNING_POLICY_ROWS {
        ...
        for (population, p_positive) in [
            ...
            assert_eq!(
                expected_weight.to_bits() == env.config.importance_ceiling.to_bits(),
                row.ceiling_binds,
...
fn alarm_outcome_cusums_accumulate_by_sentinel_and_axis() {
    ...
    for axis in [1, 3] {
        assert_eq!(alarm.high_alarm_benign[axis].to_bits(), 0.0f64.to_bits());
        assert_eq!(alarm.quiet_alarm_adverse[axis].to_bits(), 0.0f64.to_bits());
    }
}
...
fn companion_boundary_tracker_updates_outside_core() {
    ...
    let now = crate::types::PersistentTimestamp::new(1_700_000_000, 0);
...
fn cp5_nan_check_clean_working_copy() {
    ...
    let _working = WorkingCopy::cold_start(
        20,
        &ModelConfig::default(),
        1000,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
...
fn cp5_revert_restores_from_snapshot() {
    ...
    let working = WorkingCopy::from_snapshot(
        &snapshot,
        &model_config,
        1000,
        100,
    ...
    let exact = DEFAULT_TOLERANCES.exact;
...
fn a_failed_revert_stops_the_label_path() {
    ...
    env.working = WorkingCopy::from_snapshot(
        &with_nan,
        &model_config,
        env.config.n_recompute,
        100,
        crate::testing::Clock::now_monotonic(&crate::testing::SystemClock),
    )
...
fn the_revert_rebuilds_at_the_deployments_configuration() {
    ...
    assert!(
        (ModelConfig::default().lambda_prior - 0.25).abs() > DEFAULT_TOLERANCES.exact,
...
fn replay_invariant_r17() {
    ...
    let mut reverted = WorkingCopy::from_snapshot(
        &checkpoint,
        &model_config,
        1000,
        100,
