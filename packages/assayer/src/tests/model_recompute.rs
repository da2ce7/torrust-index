...
fn the_spectral_floor_is_the_least_the_success_condition_admits() {
    ...
    assert_near(spectral_floor(1.0, 0), 0.0, DEFAULT_TOLERANCES.exact, "empty width");
    assert_near(
...
fn the_floor_share_is_dimensionless_monotone_and_tends_to_zero() {
    ...
    assert_near(floor_share(1e-8, 0.0), 1.0, DEFAULT_TOLERANCES.exact, "a singular direction");

    // No floor in force, nothing to attribute to it.
    assert_near(floor_share(0.0, 1e-4), 0.0, DEFAULT_TOLERANCES.exact, "no floor in force");
}
...
fn a_rebuild_that_is_not_better_is_refused() {
    ...
    assert_near(spectrum.floor_share, 1.0, DEFAULT_TOLERANCES.exact, "no evidence holds it up");
}
...
fn the_relative_arms_fire_on_their_own_quantities() {
    ...
    match past_factor {
        RecomputeTrigger::ConditioningGrowth {
            ...
            assert_near(baseline_ratio, 100.0, DEFAULT_TOLERANCES.exact, "the baseline ratio");
...
fn a_model_whose_drift_is_all_rounding_shortens_as_before() {
    ...
    assert_eq!(
        baseline.sync_error_prior_induced.to_bits(),
        0.0f64.to_bits(),
        "a model with no clamp mass attributes nothing to the prior"
    );
    assert_eq!(
        baseline.sync_error_residual.to_bits(),
        baseline.sync_error_before.to_bits(),
...
fn a_clamp_contribution_over_the_threshold_calls_for_a_rebuild() {
    ...
    assert_eq!(
        clean.prior_induced.to_bits(),
        0.0f64.to_bits(),
        "a model with no clamp attributes nothing"
    );
...
fn a_refused_factorisation_raises_the_alarm() {
    ...
    assert_eq!(
        baseline.sync_error_after().expect("the visit rebuilt").to_bits(),
        baseline.sync_error_before.to_bits(),
