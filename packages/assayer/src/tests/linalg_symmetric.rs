...
//! | [`symmetric_operations_seeded_sweep`] | linalg | cites (´claim:linalg:an-operation-leaves-its-result-bitwise-symmetric-not-merely-symmetric-to-tolerance´) |
//! | [`extension_structure_seeded_sweep`] | linalg | cites (´claim:linalg:extension-keeps-the-original-block-gives-new-dimensions-an-independent-prior-and-zeroes-the-cross-blocks´) |
//! | [`quadratic_form_non_negative_seeded_sweep`] | linalg | cites (´claim:linalg:the-quadratic-form-stays-non-negative-on-a-positive-definite-matrix´) |
//! | [`quadratic_form_naive_seeded_sweep`] | linalg | cites (´claim:linalg:the-quadratic-form-equals-the-naive-double-sum-over-every-entry´) |
//! | [`quadratic_form_fused_seeded_sweep`] | linalg | cites (´claim:linalg:the-fused-call-returns-the-same-product-and-scalar-as-the-two-separate-ones´) |
...
use crate::testing::{TestRng, assert_finite, run_seeded_sweep};
...
#[derive(Debug)]
struct QuadraticFormCase {
    basis: Vec<f64>,
    direction: Vec<f64>,
}

fn spd_from_basis(basis: &[f64]) -> SymmetricMatrix {
    const WIDTH: usize = 4;
    let factors = Mat::from_fn(WIDTH, WIDTH, |i, j| basis[j * WIDTH + i]);
    let mut matrix = factors.transpose() * &factors;
    for index in 0..WIDTH {
        matrix[(index, index)] += 0.1;
    }
    SymmetricMatrix::from_computation(matrix)
}

fn symmetry_witness(matrix: &SymmetricMatrix) -> Result<(), String> {
    for column in 0..matrix.dim() {
        for row in column..matrix.dim() {
            let lower = matrix.as_inner()[(row, column)];
            let upper = matrix.as_inner()[(column, row)];
            if lower.to_bits() != upper.to_bits() {
                return Err(format!(
                    "mirrored cells ({row}, {column}) differ: lower={lower}, upper={upper}"
                ));
            }
        }
    }
...
fn zeros_is_zero() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(m.as_inner()[(i, j)].to_bits(), 0.0f64.to_bits());
        }
    }
}
...
fn identity_scaled_values() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            let expected: f64 = if i == j { 0.1 } else { 0.0 };
            assert_eq!(m.as_inner()[(i, j)].to_bits(), expected.to_bits());
        }
    }
}
...
/// up, and it does not.
///
/// The matrix carries negative entries, and multiplying one of those by zero
/// yields a negative zero — which is exactly zero, and is the one thing this
/// claim does not fix. The assertion is therefore made on the magnitude's
/// bits: what has to be left in every cell is nothing, not one particular
/// spelling of nothing.
///
/// (´claim:linalg:scaling-multiplies-every-entry-through-by-the-scalar´)
...
fn scale_zero() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(m.as_inner()[(i, j)].abs().to_bits(), 0.0f64.to_bits());
        }
    }
}
...
fn clamp_diagonal_min_no_change() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(m.as_inner()[(i, j)].to_bits(), original.as_inner()[(i, j)].to_bits());
        }
    }
}
...
fn extend_dimensions() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(ext.as_inner()[(i, j)].to_bits(), m.as_inner()[(i, j)].to_bits());
        }
    }
    for i in 5..8 {
        for j in 5..8 {
            let expected: f64 = if i == j { 0.5 } else { 0.0 };
            assert_eq!(ext.as_inner()[(i, j)].to_bits(), expected.to_bits());
        }
    }
    for i in 0..5 {
        for j in 5..8 {
            assert_eq!(ext.as_inner()[(i, j)].to_bits(), 0.0f64.to_bits());
            assert_eq!(ext.as_inner()[(j, i)].to_bits(), 0.0f64.to_bits());
        }
    }
}
...
fn extract_symmetric_submatrix_correct() {
    ...
    for (si, &ri) in indices.iter().enumerate() {
        for (sj, &rj) in indices.iter().enumerate() {
            assert_eq!(sub.as_inner()[(si, sj)].to_bits(), m.as_inner()[(ri, rj)].to_bits());
        }
    }
...
fn extract_symmetric_submatrix_all() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(sub.as_inner()[(i, j)].to_bits(), m.as_inner()[(i, j)].to_bits());
        }
    }
}
...
fn extract_symmetric_submatrix_single() {
    ...
    assert_eq!(sub.dim(), 1);
    assert_eq!(sub.as_inner()[(0, 0)].to_bits(), m.as_inner()[(2, 2)].to_bits());
}
...
fn extract_cross_block_correct() {
    ...
    for (bi, &ri) in rows.iter().enumerate() {
        for (bj, &cj) in cols.iter().enumerate() {
            assert_eq!(block[(bi, bj)].to_bits(), m.as_inner()[(ri, cj)].to_bits());
        }
    }
}
...
fn subtract_self_is_zero() {
    ...
    for j in 0..5 {
        for i in 0..5 {
            assert_eq!(zero.as_inner()[(i, j)].to_bits(), 0.0f64.to_bits());
        }
    }
}
...
fn diagonal_element_correct() {
    ...
    for i in 0..5 {
        assert_eq!(m.diagonal_element(i).to_bits(), 3.0f64.to_bits());
    }
}
...
fn diagonal_iter_correct() {
    ...
    for &d in &diag {
        assert_eq!(d.to_bits(), 7.0f64.to_bits());
    }
}
...
fn as_inner_read_only() {
    ...
    assert_eq!(inner[(0, 0)].to_bits(), 5.0f64.to_bits());
    assert_eq!(inner[(0, 1)].to_bits(), 0.0f64.to_bits());
...
/// Rank-one update, scaling, and extension preserve bitwise symmetry.
///
/// The generator draws 459 complete four-by-four factor matrices, four-element directions, signed rank-one weights, non-negative scales, extension widths from one through four, and positive prior diagonals. The factor product plus a fixed diagonal produces the positive-definite starting population; the full factor and direction vectors remain in every rejection. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:linalg:an-operation-leaves-its-result-bitwise-symmetric-not-merely-symmetric-to-tolerance´)
/// ´test:crate:symmetric-operations-seeded-sweep´
#[test]
fn symmetric_operations_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x5A77_E7A1_C0A5_0459;
    const WIDTH: usize = 4;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| MatrixOperationCase {
            basis: (0..WIDTH * WIDTH).map(|_| signed_unit(rng)).collect(),
            direction: (0..WIDTH).map(|_| signed_unit(rng)).collect(),
            rank_weight: rng.next_f64().mul_add(4.0, -2.0),
            scale: rng.next_f64() * 2.0,
            extension_width: usize::try_from(rng.next_range(4) + 1).unwrap_or(1),
            extension_prior: rng.next_f64().mul_add(9.9, 0.1),
        },
        |case| {
            let mut matrix = spd_from_basis(&case.basis);
            let direction = Col::from_fn(WIDTH, |index| case.direction[index]);
            matrix.symmetric_rank1_update(case.rank_weight, &direction);
            symmetry_witness(&matrix)?;
            matrix.scale(case.scale);
            symmetry_witness(&matrix)?;
            let extended = matrix.extend(case.extension_width, case.extension_prior);
            symmetry_witness(&extended)?;
            Ok::<(), String>(())
        },
    );
}

/// Extension copies every inherited cell bit for bit, gives each new coordinate exactly its declared independent prior, and leaves both cross-blocks at exact zero.
///
/// The generator draws 459 complete four-by-four factor matrices, extension widths from one through four, and positive prior diagonals. The factor product plus a fixed diagonal makes the inherited block dense, so every structural region has a non-trivial contrast. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´inv:guarantee:structural-exactness´)
/// (´claim:linalg:extension-keeps-the-original-block-gives-new-dimensions-an-independent-prior-and-zeroes-the-cross-blocks´)
/// ´test:crate:extension-structure-seeded-sweep´
#[test]
fn extension_structure_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0xE87E_A510_C0A5_0459;
    const WIDTH: usize = 4;
    ...
    run_seeded_sweep(
        ...
        |case| {
            let matrix = spd_from_basis(&case.basis);
            let inherited: Vec<_> = (0..WIDTH * WIDTH)
                .map(|offset| matrix.as_inner()[(offset % WIDTH, offset / WIDTH)].to_bits())
                .collect();
            let extended = matrix.extend(case.extension_width, case.extension_prior);
            for column in 0..extended.dim() {
                for row in 0..extended.dim() {
                    let actual = extended.as_inner()[(row, column)];
                    if row < WIDTH && column < WIDTH {
                        let expected = inherited[column * WIDTH + row];
                        if actual.to_bits() != expected {
                            return Err(format!("extension changed inherited cell ({row}, {column}): actual={actual}"));
                        }
                    } else {
                        let expected = if row == column { case.extension_prior } else { 0.0 };
                        if actual.to_bits() != expected.to_bits() {
                            return Err(format!("extension cell ({row}, {column}) is {actual}, expected {expected}"));
                        }
                    }
                }
            }
...
/// On every drawn positive-definite matrix the quadratic form is non-negative.
///
/// The generator draws 459 complete four-by-four factor matrices and four-element directions over `[−1, 1)`. The factor product plus a fixed positive diagonal spans correlated positive-definite matrices while retaining every numeric input in the rejection envelope. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:linalg:the-quadratic-form-stays-non-negative-on-a-positive-definite-matrix´)
/// ´test:crate:quadratic-form-non-negative-seeded-sweep´
#[test]
fn quadratic_form_non_negative_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x0A4D_2A71_A0A0_0459;
    const WIDTH: usize = 4;

    run_seeded_sweep(
        SEED,
        CASES,
        |rng| QuadraticFormCase {
            basis: (0..WIDTH * WIDTH).map(|_| signed_unit(rng)).collect(),
            direction: (0..WIDTH).map(|_| signed_unit(rng)).collect(),
        },
        |case| {
            let matrix = spd_from_basis(&case.basis);
            let direction = Col::from_fn(WIDTH, |index| case.direction[index]);
            let form = matrix.quadratic_form(&direction);
            if form < 0.0 {
                return Err(format!("positive-definite quadratic form is negative: {form}"));
            }
            Ok(())
        },
    );
...
/// The quadratic form agrees with an independent double sum over every matrix entry.
///
/// The generator draws 459 complete four-by-four factor matrices and four-element directions over `[−1, 1)`. The factor product plus a fixed positive diagonal spans correlated positive-definite matrices while retaining every numeric input in the rejection envelope. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:linalg:the-quadratic-form-equals-the-naive-double-sum-over-every-entry´)
/// ´test:crate:quadratic-form-naive-seeded-sweep´
#[test]
fn quadratic_form_naive_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x0A4D_2A71_D0B1_0459;
    const WIDTH: usize = 4;
        ...
        |case| {
            ...
            for row in 0..WIDTH {
                for column in 0..WIDTH {
                    naive += case.direction[row] * matrix.as_inner()[(row, column)] * case.direction[column];
                }
            }
            let naive_error = (form - naive).abs();
            if naive_error > 1e-12 {
                return Err(format!("quadratic form differs from double sum by {naive_error}"));
            }
            Ok(())
        },
    );
...
/// The fused product-and-form route agrees with the two separate calls.
///
/// The generator draws 459 complete four-by-four factor matrices and four-element directions over `[−1, 1)`. The factor product plus a fixed positive diagonal spans correlated positive-definite matrices while retaining every numeric input in the rejection envelope. The count gives more than a ninety-nine per cent chance of entering any defect region occupying at least one per cent of this distribution.
///
/// (´claim:linalg:the-fused-call-returns-the-same-product-and-scalar-as-the-two-separate-ones´)
/// ´test:crate:quadratic-form-fused-seeded-sweep´
#[test]
fn quadratic_form_fused_seeded_sweep() {
    const CASES: usize = 459;
    const SEED: u64 = 0x0A4D_2A71_F05E_0459;
    const WIDTH: usize = 4;
    ...
    run_seeded_sweep(
        ...
        |case| {
            ...
            let direction = Col::from_fn(WIDTH, |index| case.direction[index]);
            let form = matrix.quadratic_form(&direction);
            let (product, fused) = matrix.quadratic_form_with_product(&direction);
            let separate_product = matrix.symmetric_matvec(&direction);
            let fused_error = (fused - form).abs();
            if fused_error > 1e-10 {
                return Err(format!("fused scalar differs from separate form by {fused_error}"));
            }
            for index in 0..WIDTH {
                let product_error = (product[index] - separate_product[index]).abs();
                if product_error > 1e-14 {
                    return Err(format!("fused product differs at index {index} by {product_error}"));
                }
            }
            Ok(())
        },
