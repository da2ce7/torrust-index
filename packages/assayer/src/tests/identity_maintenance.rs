...
//! | [`setup_dimension_guard_rejects_a_changed_observation_capacity_with_its_measured_baseline`] | identity | The dimension fixture guard refuses an observation capacity different from its declaration and reports the complete measured dimension, encoder, shared-state, and two-view queue baseline beside the declaration. Invalid precondition: the measured observation-channel capacity is one slot short. |
//! | [`maintenance_spawn_guard_rejects_a_changed_thread_name_with_its_measured_baseline`] | identity | The default-capacity harness guard refuses a thread name different from its declaration and reports the complete measured thread and two-queue baseline beside the declaration. Invalid precondition: the measured thread name belongs to a different instance. |
//! | [`maintenance_spawn_with_capacity_guard_rejects_a_changed_model_owner_capacity_with_its_measured_baseline`] | identity | The explicit-capacity harness guard refuses a model-owner queue capacity different from its declaration and reports the complete measured thread and two-queue baseline beside the declaration. Invalid precondition: the measured model-owner queue capacity is one slot wider. |
...
use crate::testing::maintenance::{
    MaintenanceFixture, MaintenanceFixtureBaseline, MaintenanceFixtureDeclaration, MaintenanceFixtureError, MaintenanceHarness,
    aggressive_split_config, collect_lifecycle_events, default_mudlark_config, dimension_fixture_baseline, setup_dimension,
};
...
/// The dimension fixture guard refuses an observation capacity different from its declaration and reports the complete measured dimension, encoder, shared-state, and two-view queue baseline beside the declaration.
///
/// Invalid precondition: the measured observation-channel capacity is one slot short.
///
/// ´claim:identity:the-dimension-fixture-guard-refusal-carries-its-complete-measured-baseline´
/// ´test:crate:setup-dimension-guard-rejects-a-changed-observation-capacity-with-its-measured-baseline´
#[test]
fn setup_dimension_guard_rejects_a_changed_observation_capacity_with_its_measured_baseline() {
    let dim_id = 17;
    let expected = MaintenanceFixtureDeclaration::setup_dimension(dim_id);
    let (infra, observation) = setup_dimension(dim_id);
    let mut measured = dimension_fixture_baseline(&infra, &observation);
    let MaintenanceFixtureBaseline::Dimension(baseline) = &mut measured else {
        panic!("setup_dimension must produce a dimension baseline");
    };
    baseline.observation_capacity -= 1;

    let error = MaintenanceFixture::SetupDimension
        .guard_baseline(&expected, measured.clone())
        .expect_err("a changed observation capacity must be refused as invalid setup");
    let message = error.to_string();

    assert_eq!(
        error,
        MaintenanceFixtureError::BaselineMismatch {
            fixture: MaintenanceFixture::SetupDimension,
            expected: Box::new(expected.clone()),
            measured: Box::new(measured.clone()),
        }
    );
    assert_eq!(
        message,
        format!("setup_dimension measured maintenance baseline {measured:?} does not satisfy declaration {expected:?}")
    );
    eprintln!("refusal: {message}");
}

/// The default-capacity harness guard refuses a thread name different from its declaration and reports the complete measured thread and two-queue baseline beside the declaration.
///
/// Invalid precondition: the measured thread name belongs to a different instance.
///
/// ´claim:identity:the-default-maintenance-harness-guard-refusal-carries-its-complete-measured-baseline´
/// ´test:crate:maintenance-spawn-guard-rejects-a-changed-thread-name-with-its-measured-baseline´
#[test]
fn maintenance_spawn_guard_rejects_a_changed_thread_name_with_its_measured_baseline() {
    let instance_id = "guarded-spawn";
    let expected = MaintenanceFixtureDeclaration::spawn(instance_id);
    let harness = MaintenanceHarness::spawn(instance_id);
    let mut measured = harness.maintenance_baseline();
    let MaintenanceFixtureBaseline::Harness(baseline) = &mut measured else {
        panic!("MaintenanceHarness::spawn must produce a harness baseline");
    };
    baseline.thread_name = Some("another-instance-identity-maintenance".to_owned());

    let error = MaintenanceFixture::Spawn
        .guard_baseline(&expected, measured.clone())
        .expect_err("a changed thread name must be refused as invalid setup");
    let message = error.to_string();

    assert_eq!(
        error,
        MaintenanceFixtureError::BaselineMismatch {
            fixture: MaintenanceFixture::Spawn,
            expected: Box::new(expected.clone()),
            measured: Box::new(measured.clone()),
        }
    );
    assert_eq!(
        message,
        format!("MaintenanceHarness::spawn measured maintenance baseline {measured:?} does not satisfy declaration {expected:?}")
    );
    eprintln!("refusal: {message}");
}

/// The explicit-capacity harness guard refuses a model-owner queue capacity different from its declaration and reports the complete measured thread and two-queue baseline beside the declaration.
///
/// Invalid precondition: the measured model-owner queue capacity is one slot wider.
///
/// ´claim:identity:the-explicit-capacity-maintenance-harness-guard-refusal-carries-its-complete-measured-baseline´
/// ´test:crate:maintenance-spawn-with-capacity-guard-rejects-a-changed-model-owner-capacity-with-its-measured-baseline´
#[test]
fn maintenance_spawn_with_capacity_guard_rejects_a_changed_model_owner_capacity_with_its_measured_baseline() {
    let instance_id = "guarded-capacity";
    let declared_capacity = 2;
    let expected = MaintenanceFixtureDeclaration::spawn_with_capacity(instance_id, declared_capacity);
    let harness = MaintenanceHarness::spawn_with_capacity(instance_id, declared_capacity);
    let mut measured = harness.maintenance_baseline();
    let MaintenanceFixtureBaseline::Harness(baseline) = &mut measured else {
        panic!("MaintenanceHarness::spawn_with_capacity must produce a harness baseline");
    };
    baseline.model_owner_capacity = Some(declared_capacity + 1);

    let error = MaintenanceFixture::SpawnWithCapacity
        .guard_baseline(&expected, measured.clone())
        .expect_err("a changed model-owner queue capacity must be refused as invalid setup");
    let message = error.to_string();

    assert_eq!(
        error,
        MaintenanceFixtureError::BaselineMismatch {
            fixture: MaintenanceFixture::SpawnWithCapacity,
            expected: Box::new(expected.clone()),
            measured: Box::new(measured.clone()),
        }
    );
    assert_eq!(
        message,
        format!(
            "MaintenanceHarness::spawn_with_capacity measured maintenance baseline {measured:?} does not satisfy declaration {expected:?}"
        )
    );
    eprintln!("refusal: {message}");
}
...
fn destroy_dimension_command() {
    ...
    // The barrier follows the destroy command, proving the loop processed it.
    harness.flush_identity_maintenance();
}
        ...
        .map(|_| {
            ...
            std::thread::spawn(move || {
                for _ in 0..200 {
                    ...
                    // Retained concurrency pacing: this fixed-count reader never awaits state; yielding only widens its overlap with republication.
...
fn decay_triggers_exit() {
    ...
    // The identity-maintenance barrier drains observations and detects changes.
    // With all energy zeroed, total_sum()==0 → empty set → exits emitted.
    harness.flush_identity_maintenance();
