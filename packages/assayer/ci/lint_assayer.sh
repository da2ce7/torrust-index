...
#   3. The maintained performance command remains compilable without running
#      its expensive measurements.
#
# Usage:
...
# ─── Lint 3: Performance target compilation ──────────────────────────────────

# Maintained measurement command: cargo bench -p torrust-assayer
# CI callers may lower Criterion's sample size and warm-up after the separator,
# but this lint only compiles the target and never executes its measurements.

echo "=== Lint: performance Criterion target compiles ==="

BENCH_START_NS=$(date +%s%N)
set +e
cargo bench -p torrust-assayer --no-run
BENCH_EXIT_CODE=$?
set -e
BENCH_END_NS=$(date +%s%N)
BENCH_WALL_NS=$((BENCH_END_NS - BENCH_START_NS))

echo "Performance target compile exit: $BENCH_EXIT_CODE"
echo "Performance target compile wall: $BENCH_WALL_NS ns"

if [ "$BENCH_EXIT_CODE" -eq 0 ]; then
    echo "PASS: The performance Criterion target compiles without running measurements."
else
    echo "FAIL: The performance Criterion target did not compile."
    EXIT_CODE=1
fi
