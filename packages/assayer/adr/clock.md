...
It is a small record with a wide reach. Its decisions and the environments that follow from them are the whole of it, and several other records cite it rather than restating the discipline it fixes — which is what a statement with two owners would otherwise become.
...
The separation is structural rather than conventional, and the code holds it that way: no conversion exists in either direction between the two domains, the persistent type's only conversions are to and from wall-clock system time, and no monotonic reading is written to or recovered from durable state. Reconstruction receives the engine's current monotonic reading from its caller solely to initialise the working copy's label clock; the persistent reading alone measures downtime. A convention would have made every call site a place to check; a type makes the mistake unwritable.
...
**Corollary (The harness advances both clocks through their barriers)** · `cor:clock:harness-control`

The separation of (`dec:clock:two-domains`) reaches the harness unchanged: the scenario controls the intra-process and persistent domains separately and moves either only forward. An advance is complete for a test only after the scenario crosses every declared barrier required by the operation whose time moved (`dec:harness:no-ad-hoc-waits`). A test may therefore rely on a time advance having settled the queues it necessarily activates, without converting between domains or authoring a wait of its own.
