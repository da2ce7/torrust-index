...
**Decision (The producing set explains the reported cell set)** · `dec:ownership:producing-set-correspondence`

The structural correspondence places the Sentinel's producing set — competitive targets plus online ancestors — against the Core's reported cell set (`tab:terminology:structural`). The producing sets are the online subsets of the investment set, and the slot-vacancy invariant excludes warming targets until they produce (`[SENTINEL-sec:sentinel:algorithm-producing-sets]`). That structure is why the report contains cells above the contour: the producing set is a strict superset of the contour cells selected for analysis.

Leaving the correspondence out and relying on the abstraction caveat alone (`cav:limitation:abstraction`) is rejected. The caveat prevents a reader from assigning Sentinel-internal reasoning to the Core, but it does not explain why the reported set has this shape. A reader may rely on the narrower boundary fact: the Core reads the reported cells and reasons about no Sentinel internal, as the feed-forward decision (`dec:ownership:feed-forward`) and guarantee (`inv:guarantee:feed-forward`) require.

**Decision (The Sentinel-side inventory belongs with boundary custody)** · `dec:ownership:boundary-inventory`

The Sentinel-side inventory lives in this record (`tab:ownership:sentinel-side-inventory`). The terminology convention carries only the qualified pointers to the upstream ranges that own the mechanisms, because a reading convention says how to understand a crossing while this record decides what remains across it. That division keeps the inventory with the feed-forward custody decision (`dec:ownership:feed-forward`) and the guarantee it realises (`inv:guarantee:feed-forward`).

An expository list in the terminology appendix is rejected because it would make the appendix a second owner of the boundary inventory. Dropping the inventory is rejected as well: its earlier loss went unregistered, leaving no place that could distinguish a deliberate removal from an omission.

**Table (What remains on the Sentinel's side of the wall)** · `tab:ownership:sentinel-side-inventory`

| Sentinel-side inventory |
| --- |
| G-Tree node states: terminal, semi-internal and internal |
| V-Tree depth, position and promotion events |
| Investment-set membership and warm-up state |
| The split threshold and the creation and eviction depth gates |
| Contour structure and plateau organisation |
| Benchmark compounding and frozen benchmark values |
| The owning upstream ranges: the Sentinel corpus's spatial-layer chapter (`[SENTINEL-sec:sentinel:algorithm-spatial-layer]`) and its selection-and-assembly part (`[SENTINEL-sec:sentinel:algorithm-selection-and-assembly]`) |
