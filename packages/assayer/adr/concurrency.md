...
**Corollary (The harness names one barrier per asynchronous queue)** · `cor:concurrency:harness-barriers`

The publication discipline of (`dec:structural:publication`) and the write serialisation of (`dec:structural:write-serialisation`) make completion a property of the asynchronous queue carrying the work rather than of elapsed time. Every such queue exposed to tests therefore has exactly one named barrier, and that barrier states which submitted work and resulting publications it covers and which it excludes. Under (`dec:harness:no-ad-hoc-waits`), a test may rely on that barrier alone for the completion it names and may infer nothing about an excluded queue.
