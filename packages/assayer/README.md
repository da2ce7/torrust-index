...
- **Derivation Function** — pure `derive_landscape()` output from explicit host inputs, with `render_resonances()` the optional rendering layer over it.
...
**Layer 3 (The Operational Cycle) is complete.** All runtime data-flow paths are implemented: `assess()` produces core risk assessments, `derive_landscape()` provides pure decision-layer derivation from explicit inputs and `render_resonances()` renders that landscape for display, `label()` processes the 17-step learning path, and `receive_sentinel_report()` validates, indexes, and maintains the Ledger. The operational-cycle records are fully realised.
...
**Layer 5 (Contracts and Boundaries) is substantially complete.** The `AssayerBuilder` pattern, the full `AssayerConfig` hierarchy, all registration types, the six calls the crate's public API map documents — Core `assess()` and `request_labels()`, host-side `derive_landscape()` and `render_resonances()`, `label()` and `receive_sentinel_report()` — health queries, lifecycle methods, and metrics catalog are implemented.
