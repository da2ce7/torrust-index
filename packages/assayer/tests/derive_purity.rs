...
//! (´test:integration:public-derive-landscape-and-render-resonances-bit-identical´)
//! holds one assessment and repeats the pure landscape and rendering
//! transforms, pinning every public number as raw bits. Assessment-facing
//! scenarios use the injected virtual clock, so elapsed host time cannot
//! enter their expectations.
//!
...
//! # Cross-References
//!
//! - (´test:integration:public-derive-landscape-and-render-resonances-bit-identical´) — bit-identical output under a thousand repetitions
...
/// ´test:integration:public-derive-landscape-and-render-resonances-bit-identical´
#[test]
fn public_derive_landscape_and_render_resonances_bit_identical() {
...
fn cold_ramp_trajectory_stays_inside_the_falsifier() {
    ...
    for _ in 0..REQUESTS {
        ...
        // The observation barrier publishes this request's ramp step before the next sample is taken.
        s.flush_observations()
            .expect("the observation barrier publishes each measured ramp step");
    }
...
fn identical_inputs_across_entities_well_formed() {
    ...
    // (´test:integration:public-derive-landscape-and-render-resonances-bit-identical´):
    // purity is a contract *relative to the inputs*. Distinct entity keys
    // feed into the identity layer and MAY produce different assessments —
    // this test only asserts that both sides remain finite and in range,
    // not that they are unequal (a cold engine with no identity
