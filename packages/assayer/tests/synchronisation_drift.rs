...
//! but under configurations chosen so that one candidate mechanism moves at a time, and it prints every reading it takes rather than reducing them to a verdict. Nothing here is a guard: each test asserts only that its complete declared run reached the geometry its readings are about.
//!
...
//! nothing to adopt.
//!
//! # Why the geometry helpers remain local
//!
//! Liveness arming comes directly from `torrust_assayer::testing`, where its heartbeat sleep and stall policy belong. The geometry, avalanche and label cycle remain smaller local forms of the factorisation reproduction (´claim:wellness:a-dense-competitive-schedule-leaves-every-precision-matrix-factorisable´): every label here is `Action::Allow`, so copying the larger challenge-result branch would hide the complete stimulus behind code this instrument cannot reach.
//!
//! # Cross-References
//!
...
use torrust_assayer::testing::{LabelSpec, Progress, World, advanced, fail_fast_on_model_owner_panic, finished, watch_progress};
...
/// Drives the declared `count` labels from `from`; the shared progress watcher owns detection of a stalled queue.
fn drive(world: &World, axis: OutcomeAxisId, from: u64, count: u64, progress: &Progress) {
...
///
/// Answers the widest geometry the complete declared run reached.
fn run_setting(setting: &Setting, run: usize) -> usize {
    ...
    while at < setting.grow_cap && grown.cells < setting.target_cells {
        ...
        drive(world, axis, at, want, &progress);
        at += want;
...
/// targets. A complete declared run that does not clear it has not reached the geometry its readings require.
...
fn the_synchronisation_error_grows_with_the_width_and_not_with_the_floored_count() {
    ...
    // cutoff sweep remains below the required geometry whatever the oracle does.
