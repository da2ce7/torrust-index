...
use torrust_assayer::testing::{
    DIVERGENT_OUTCOME_REPLAY_DRIFT, GOLDEN_COORD, LIFECYCLE_SUFFICIENCY_DRIFT, PURITY_DRIFT, PlaybackBarrier,
    PlaybackBarrierPolicy, PlaybackBatchSize, PlaybackProgress, PlaybackRow, Scenario, World, WorldBuilder,
    assert_core_and_resonance_profile_near_with_drift, assert_health_clean, attach_golden_reporting_sentinel, cycle_request,
    playback, refresh_golden_report, scenario_with, score_verified_request_schema, with_degraded_score_verified,
...
/// Bounds allocation while leaving enough rows per chunk to amortise iterator setup in the canonical enrichment histories.
const ENRICHED_LABEL_PLAYBACK_BATCH_SIZE: NonZeroUsize = NonZeroUsize::new(8).unwrap();
...
impl Enrichment {
    ...
    #[must_use]
    pub fn maybe_signal_schema(&self, builder: WorldBuilder) -> WorldBuilder {
...
/// request shape.
struct EnrichedLabelPlaybackRow<'a> {
    enrichment: &'a Enrichment,
    channel: &'static str,
    cycle: usize,
    case: LabelCycleCase<'static>,
    name: String,
}

impl PlaybackRow for EnrichedLabelPlaybackRow<'_> {
    type Error = String;
    ...
    fn play(&self, world: &World) -> Result<(), Self::Error> {
        let request = self.enrichment.decorate_training(
            self.enrichment.base_request(world, self.channel, self.case.entity),
            self.cycle,
        );
        let reckoning = world
            .derive_for_request(request)
            .map_err(|error| format!("assess failed: {error}"))?;
        world
            .label(self.case.label_spec(reckoning.assessment.id).build())
            .map(drop)
            .map_err(|error| format!("label failed: {error}"))
    }
}

/// Train the canonical alternating entity history through an enrichment's request shape under the shared playback contract.
#[track_caller]
pub fn train_enriched_label_history(
    ...
    let rows = alternating_entity_label_stream(cycles, adverse_every)
        .enumerate()
        .map(|(cycle, case)| EnrichedLabelPlaybackRow {
            enrichment,
            channel,
            cycle,
            case,
            name: format!("{label}: label-history cycle {cycle}"),
        });
    playback(
        world,
        rows,
        ...
        PlaybackBatchSize::new(ENRICHED_LABEL_PLAYBACK_BATCH_SIZE),
    )
    .unwrap_or_else(|error| panic!("{label}: enriched label playback failed: {error}"));
    enrichment.refresh_if_reported(world);
}
