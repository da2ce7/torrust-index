...
///
/// This is the common final assertion (´claim:channel:a-channels-policy-shape-moves-only-its-action-tags-and-never-the-shared-core´) for two-world probes whose public request shape carries more than just `(channel, entity)`: request-scoped signals and live Sentinel coordinates reuse this one comparison once the worlds have seen matched core histories.
#[track_caller]
