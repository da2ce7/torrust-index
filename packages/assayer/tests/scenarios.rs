...
use torrust_assayer::testing::{TestRng, World, assert_finite, assert_in_unit_interval, assert_near, assert_positive};
use torrust_assayer::types::{PersistentTimestamp, dyadic_ancestor_hi, dyadic_ancestor_lo, is_dyadic};
...
fn time_travellers_regret_cannot_decay_past() {
    ...
    assert_eq!(factor.to_bits(), 1.0f64.to_bits(), "Backward time must not decay");
}
...
fn hourglass_inverts_decay() {
    ...
    for _ in 0..100 {
        ...
        let target = 0.99f64.mul_add(rng.next_f64(), 0.01);
...
fn sigmoid_and_logit_are_mirror_twins() {
    ...
    for _ in 0..1000 {
        ...
        let eps = 1e-6;
        let p = 2.0f64.mul_add(-eps, 1.0).mul_add(rng.next_f64(), eps);
...
fn atanh_undoes_tanh_everywhere() {
    ...
    for _ in 0..1000 {
        // x ∈ (-4, 4)
        let x = rng.next_f64().mul_add(8.0, -4.0);
        let th = x.tanh();
...
fn pigeonhole_forces_hash_collision() {
    ...
    for s in strings {
        ...
        assert_eq!(encoded.iter().filter(|&&v| v.to_bits() == 1.0f64.to_bits()).count(), 1);
        let idx = encoded.iter().position(|&v| v.to_bits() == 1.0f64.to_bits()).unwrap();
        indices.push(idx);
    }
...
fn poisoned_apple_sanitised() {
    ...
    assert_eq!(encoded[0].to_bits(), 0.0f64.to_bits(), "NaN must be sanitised");
    assert_eq!(encoded[1].to_bits(), 0.5f64.to_bits(), "clean value preserved");
    assert_eq!(encoded[2].to_bits(), 0.0f64.to_bits(), "infinity must be sanitised");
}
...
fn the_forgetful_oracle_keeps_entity_drops_request() {
    ...
    let who = World::entity("oracle-client");
...
fn phoenix_rises_after_eviction() {
    ...
    let phoenix = World::entity("phoenix");
    ...
    // it is made.
    for i in 0..5 {
        let other = World::entity(&format!("mortal-{i}"));
...
fn polyglot_signal_schema_speaks_every_shape() {
    ...
    assert_eq!(features[0].to_bits(), 0.5f64.to_bits()); // score
    assert_eq!(features[1].to_bits(), 0.0f64.to_bits()); // age_days ln(1+0)=0
    assert_eq!(features[2].to_bits(), 1.0f64.to_bits()); // verified
    assert_eq!(features[3].to_bits(), 1.0f64.to_bits()); // tier clamped to max
    ...
    assert_eq!(country_slice.iter().filter(|&&v| v.to_bits() == 1.0f64.to_bits()).count(), 1);
...
fn request_context_is_a_builder_not_a_bookkeeper() {
    ...
    let dup = RequestContext::new(World::entity("duplicate"))
    ...
    let dup_s = RequestContext::new(World::entity("duplicate"))
