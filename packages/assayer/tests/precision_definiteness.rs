...
//! negative.
//!
//! This harness is that scan cut down to the minimum that reaches the failure. Its declared stretch count makes the stimulus finite, the shared liveness watcher reports a stalled queue, and every checkpoint prints as it is taken so an externally interrupted run remains evidence up to its last completed stretch.
//!
...
/// anything.
///
/// The failure needed the geometry, so a complete declared run that does not grow it has not exercised what it is guarding and says so rather than passing quietly.
...
/// Drives one complete declared stretch; the shared progress watcher owns detection of a stalled queue.
fn drive(world: &World, axis: OutcomeAxisId, from: u64, progress: &Progress) {
...
fn a_dense_competitive_schedule_leaves_every_precision_matrix_factorisable() {
    ...
    assert!(
        ...
        "the complete schedule reached {cells_reached} competitive cells against the {REQUIRED_CELLS} the failure needed"
    );
}
