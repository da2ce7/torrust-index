...
use torrust_assayer::testing::{
    LabelSpec, assert_finite, assert_health_clean, assert_reckoning_well_formed, cycle_request, scenario_with,
...
fn scalar_signal_population_split_converges_directionally() {
    ...
    for cycle in 0..TRAINING_CYCLES {
        ...
        let request = if adverse {
            with_score_verified(scenario.request("default", "training-adverse"), 0.92, true)
        } else {
            with_score_verified(scenario.request("default", "training-benign"), 0.08, false)
        };
    ...
    let benign_holdout = scenario
        .derive_for_request(with_score_verified(
            scenario.request("default", "holdout-benign"),
            0.08,
            false,
        ))
        .expect("benign holdout should derive");
    assert_reckoning_well_formed(&benign_holdout, "holdout-benign");
    let adverse_holdout = scenario
        .derive_for_request(with_score_verified(
            scenario.request("default", "holdout-adverse"),
            0.92,
            true,
        ))
        .expect("adverse holdout should derive");
    assert_reckoning_well_formed(&adverse_holdout, "holdout-adverse");
    ...
    let benign_risks: Vec<f64> = (0..20)
        .map(|sample| {
            ...
            let entity = format!("auc-benign-{sample}");
            let reckoning = scenario
                .derive_for_request(with_score_verified(scenario.request("default", &entity), score, false))
                .expect("benign AUC sample should derive");
    ...
    let adverse_risks: Vec<f64> = (0..20)
        .map(|sample| {
            ...
            let entity = format!("auc-adverse-{sample}");
            let reckoning = scenario
                .derive_for_request(with_score_verified(scenario.request("default", &entity), score, true))
                .expect("adverse AUC sample should derive");
            assert_reckoning_well_formed(&reckoning, &entity);
            reckoning.assessment.risk.p_bad
        })
        .collect();
