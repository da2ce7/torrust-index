...
mod encode_hashed_categorical {
    ...
    fn produces_one_hot_vector() {
        ...
        let ones_count = encoded.iter().filter(|&&v| v.to_bits() == 1.0f64.to_bits()).count();
        assert_eq!(ones_count, 1);

        // All others should be 0.0
        let zeros_count = encoded.iter().filter(|&&v| v.to_bits() == 0.0f64.to_bits()).count();
    ...
    fn different_strings_different_indices() {
        ...
        // Find the indices
        let idx1 = encoded1.iter().position(|&v| v.to_bits() == 1.0f64.to_bits()).unwrap();
        let idx2 = encoded2.iter().position(|&v| v.to_bits() == 1.0f64.to_bits()).unwrap();
...
mod cache_operations {
    ...
    fn miss_returns_zeros_without_signals() {
        ...
        let entity = World::entity("user-1");
    ...
    fn different_entities_are_independent() {
        ...
        let entity_a = World::entity("user-a");
        let entity_b = World::entity("user-b");
...
mod eviction {
    ...
    fn evicts_oldest_at_capacity() {
        ...
        // it is made.
        for i in 0..3 {
            let entity = World::entity(&format!("user-{i}"));
            let mut signals = HashMap::new();
        ...
        let features = cache.get_and_merge(&World::entity("user-0"), &HashMap::new());
    ...
    fn evicts_lru_not_just_oldest() {
        ...
        drop(cache.get_and_merge(&World::entity("A"), &HashMap::new()));
        cache.drain_deferred_writes();

        // Insert D - should evict B (oldest untouched), not A
        let mut signals = HashMap::new();
        signals.insert("auth".to_string(), SignalValue::Numeric(1.0));
        drop(cache.get_and_merge(&World::entity("D"), &signals));
        cache.drain_deferred_writes();

        // A should still be cached (was accessed)
        let features_a = cache.get_and_merge(&World::entity("A"), &HashMap::new());
        assert_eq!(features_a, vec![1.0], "A should be retained (recently used)");

        // B should be evicted (LRU)
        let features_b = cache.get_and_merge(&World::entity("B"), &HashMap::new());
...
mod health_tracking {
    ...
    fn tracks_hits_and_misses() {
        ...
        for i in 0..5 {
            let new_entity = World::entity(&format!("miss-{i}"));
    ...
    fn concurrent_get_merge_no_panics() {
        ...
        for thread_id in 0..4 {
            ...
            let handle = thread::spawn(move || {
                for i in 0..100 {
                    let entity = World::entity(&format!("thread-{thread_id}-entity-{i}"));
                    let mut signals = HashMap::new();
                    signals.insert(
                        "auth".to_string(),
            ...
            let handle = thread::spawn(move || {
                for round in 0..25 {
                    for entity_id in 0..10 {
                        let entity = World::entity(&format!("shared-{entity_id}"));
                        let mut signals = HashMap::new();
                        signals.insert(
                            "auth".to_string(),
