...
fn label_with_unknown_assessment_id_is_rejected() {
    // The expiry case (´claim:pending:entries-past-the-expiry-horizon-are-dropped-even-well-under-capacity´) drives the TTL path through injected scenario time. The error surface the host must handle is identical when the assessment ID was simply never issued by this engine: an unknown id is the generalisation of an evicted one.
    //
