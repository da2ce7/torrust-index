...
//! | [`timestamp_now_positive`] | types | The production system clock returns a persistent reading after the epoch. The clock owns access to the wall clock, while callers receive only the typed reading they can pass into timestamp-bearing constructors. |
...
use torrust_assayer::testing::{Clock, DEFAULT_TOLERANCES, SystemClock, assert_near, assert_positive};
...
/// The production system clock returns a persistent reading after the epoch. The clock owns access to the wall clock, while callers receive only the typed reading they can pass into timestamp-bearing constructors.
///
...
fn timestamp_now_positive() {
    ...
    assert_positive(ts.hours_since(&epoch), "hours from the epoch to the system-clock reading");
}
